...
public class KakaoPlacesClient {
    ...
    /** Runtime guard: only enable external call path when API key is provided. */
    private static boolean apiEnabled() {
        String key = System.getenv("KAKAO_REST_KEY");
        return key != null && !key.isEmpty();
    }

    /** Placeholder search method; returns empty list to avoid external dependency. */
    public List<Map<String, Object>> search(String query, Double x, Double y, Integer radius) {
        log.info("[KakaoPlacesClient] search query={} x={} y={} radius={} (apiEnabled={})",
                 query, x, y, radius, apiEnabled());
