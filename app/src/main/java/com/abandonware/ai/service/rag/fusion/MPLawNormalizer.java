...
/** Marchenko-Pastur like soft clamp for spike noise. */
/**
 * [GPT-PRO-AGENT] 파일 수준 주석 - 기능 연결을 돕기 위한 설명
 * 클래스: MpLawNormalizer
 * 역할(Role): Class
 * 관련 기능(Tags): RAG Fusion
 * 소스 경로: app/src/main/java/com/abandonware/ai/service/rag/fusion/MpLawNormalizer.java
 *
 * 연결 포인트(Hooks):
...
// /
/* agent-hint:
id: com.abandonware.ai.service.rag.fusion.MpLawNormalizer
role: config
*/
class MpLawNormalizer {
    public static double clamp(double x, double beta){
        // beta in [0,1]: higher -> stronger shrink
        double s = 1.0 + beta * Math.tanh(Math.abs(x));
        return x / s;
    }
}