...
/** Low-rank ZCA whitening adapter. If delegate (legacy) is absent, passthrough. */
/**
 * [GPT-PRO-AGENT] 파일 수준 주석 - 기능 연결을 돕기 위한 설명
 * 클래스: LegacyLowRankWhiteningAdapter
 * 역할(Role): Class
 * 소스 경로: app/src/main/java/com/nova/protocol/whiten/LegacyLowRankWhiteningAdapter.java
 *
 * 연결 포인트(Hooks):
...
role: config
*/
class LegacyLowRankWhiteningAdapter implements Whitening {
  private final AtomicReference<Object> delegate = new AtomicReference<>(null);
  public void setDelegate(Object legacyTransform){ delegate.set(legacyTransform); }
  @Override public float[] apply(float[] v){
    if (v==null) return null;
    Object d = delegate.get();
    if (d==null) return v;
    try {
      return (float[]) d.getClass().getMethod("transform", float[].class).invoke(d, (Object)v);
