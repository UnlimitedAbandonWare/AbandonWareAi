...
public class Bm25LocalRetriever {
    ...
    public List<ContextSlice> retrieve(String query, int k) {
        try {
            ...
            traceSuppressed("retrieve", query, e);
    ...
    private static void traceSuppressed(String stage, String query, Throwable error) {
        TraceStore.put("agent.bm25.retrieve.suppressed", true);
        TraceStore.put("agent.bm25.retrieve.suppressed.stage",
                SafeRedactor.traceLabelOrFallback(stage, "unknown"));
        TraceStore.put("agent.bm25.retrieve.suppressed.errorType",
                error == null ? "unknown" : error.getClass().getSimpleName());
        TraceStore.put("agent.bm25.retrieve.suppressed.queryHash", SafeRedactor.hashValue(query));
        TraceStore.put("agent.bm25.retrieve.suppressed.queryLength", query == null ? 0 : query.length());
    }
}
