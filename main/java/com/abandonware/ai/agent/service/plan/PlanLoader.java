...
public class PlanLoader {
            ...
            for (Resource r : res) {
                try (InputStream in = r.getInputStream()) {
                    ...
                    if (p.id() == null || p.id().isBlank()) {
                        traceSuppressed("plan.missingId", locationPattern, null);
                        continue;
                    }
                    map.put(p.id(), p);
                } catch (Exception e) {
                    traceSuppressed("loadAll", locationPattern, e);
                }
            }
            return map;
        } catch (Exception e) {
    ...
    private static void traceSuppressed(String stage, String locationPattern, Throwable error) {
        TraceStore.put("agent.planLoader.suppressed", true);
        TraceStore.put("agent.planLoader.suppressed.stage",
                SafeRedactor.traceLabelOrFallback(stage, "unknown"));
        TraceStore.put("agent.planLoader.suppressed.errorType",
                error == null ? "unknown" : error.getClass().getSimpleName());
        TraceStore.put("agent.planLoader.suppressed.patternHash", SafeRedactor.hashValue(locationPattern));
        TraceStore.put("agent.planLoader.suppressed.patternLength",
                locationPattern == null ? 0 : locationPattern.length());
    }
}
