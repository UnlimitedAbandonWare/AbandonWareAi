...
public class AdminController {
    ...
    @GetMapping("/rooms")
    public ResponseEntity<List<RoomController.RoomView>> all(@RequestParam(name = "type", defaultValue = "all") String type) {
        return ResponseEntity.ok(rooms.listAdmin(type).stream()
                .map(RoomController.RoomView::from)
                .toList());
    }
}
