...
public class IdentityInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        String gid = usableGid(readCookie(request, "gid"));
        if (gid == null) {
    ...
    private static String usableGid(String candidate) {
        ...
        try {
            ...
            traceSuppressed("gid.parse", e);
            return null;
        }
    }

    private static void traceSuppressed(String stage, Throwable error) {
        TraceStore.put("agent.identity.suppressed", true);
        TraceStore.put("agent.identity.suppressed.stage", stage);
        TraceStore.put("agent.identity.suppressed.errorClass",
