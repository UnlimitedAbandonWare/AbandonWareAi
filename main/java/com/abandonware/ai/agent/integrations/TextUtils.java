...
public final class TextUtils {
    ...
    public static double recencyBoost(String text) {
        ...
        if (m.find()) {
            try {
                ...
                traceSuppressed("recency", "date.parseFallback", ex);
                TraceStore.inc("agent.text.recency.date.parseFallback.count");
                TraceStore.put("agent.text.recency.date.parseFallback.errorType", "invalid_date");
            }
        }
        // no date -> default 0.5
        return 0.5;
    ...
    static String sha1(String s, String algorithm) {
        try {
            ...
            traceSuppressed("sha1", "digest", e);
            return Integer.toHexString(Objects.hashCode(s));
        }
    }

    private static void traceSuppressed(String family, String stage, Throwable error) {
        String prefix = "agent.text." + SafeRedactor.traceLabelOrFallback(family, "unknown");
