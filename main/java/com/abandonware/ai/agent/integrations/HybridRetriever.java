...
public class HybridRetriever {
    ...
    public List<Map<String,Object>> retrieve(String query, Integer topK, String domain) {
        try {
            ...
            traceSuppressed("index.ensureBuilt", e, query, domain);
        }
        ...
        try {
            ...
            traceSuppressed("tesseract.rerank", t, query, domain);
        }
...
// 3) second-pass rerank (optional)
        ...
        switch (mode) {
            ...
            case "onnx-ce": {
                try {
                    ...
                    traceSuppressed("secondPass.onnx", t, query, mode);
                    CrossEncoder ce = new HeuristicCrossEncoder();
                    return applyCe(query, items, ce);
                }
            }
    ...
    private static double toDouble(Object o) {
        ...
        try {
            ...
            traceSuppressed("score.parseFallback", e, o);
            return 0.0;
        }
    }

    private static void traceSuppressed(String stage, Throwable error) {
        TraceStore.put("agent.hybridRetriever.suppressed", true);
        TraceStore.put("agent.hybridRetriever.suppressed.stage",
                SafeRedactor.traceLabelOrFallback(stage, "unknown"));
        TraceStore.put("agent.hybridRetriever.suppressed.errorType",
                error == null ? "unknown" : error.getClass().getSimpleName());
    }

    private static void traceSuppressed(String stage, Throwable error, String query, String context) {
        traceSuppressed(stage, error);
        if (query != null) {
            TraceStore.put("agent.hybridRetriever.suppressed.queryHash", SafeRedactor.hashValue(query));
            TraceStore.put("agent.hybridRetriever.suppressed.queryLength", query.length());
        }
        if (context != null) {
            TraceStore.put("agent.hybridRetriever.suppressed.contextHash", SafeRedactor.hashValue(context));
            TraceStore.put("agent.hybridRetriever.suppressed.contextLength", context.length());
        }
    }

    private static void traceSuppressed(String stage, Throwable error, Object value) {
        traceSuppressed(stage, error);
        TraceStore.put("agent.hybridRetriever.suppressed.errorType", "invalid_number");
        if (value != null) {
            String raw = String.valueOf(value);
            TraceStore.put("agent.hybridRetriever.suppressed.valueHash", SafeRedactor.hashValue(raw));
            TraceStore.put("agent.hybridRetriever.suppressed.valueLength", raw.length());
        }
    }
...
// [ADD] 링크 추출 헬퍼 (HTML href 또는 첫 번째 http 링크)
