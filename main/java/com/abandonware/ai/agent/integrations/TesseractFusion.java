...
public class TesseractFusion {
    ...
    private static double toDouble(Object o) {
        ...
        try { return Double.parseDouble(String.valueOf(o)); } catch(NumberFormatException ex){
            traceSuppressed("score.parseFallback", ex);
            return 0.0;
        }
    }
    ...
    private double recencyBoost(Map<String,Object> m) {
        ...
        else if (tsObj != null) try { ts = Long.parseLong(String.valueOf(tsObj)); } catch(NumberFormatException ex){
            traceSuppressed("timestamp.parseFallback", ex);
        }
        if (ts<=0) return 0.5;
    ...
    private static void traceSuppressed(String stage, NumberFormatException ex) {
        String prefix = "agent.tesseract.numeric." + stage;
        String errorType = "invalid_number";
        TraceStore.put("agent.tesseract.numeric.suppressed", true);
        TraceStore.put("agent.tesseract.numeric.suppressed.stage",
                SafeRedactor.traceLabelOrFallback(stage, "unknown"));
        TraceStore.put("agent.tesseract.numeric.suppressed.errorType", errorType);
        TraceStore.inc(prefix + ".count");
        TraceStore.put(prefix + ".errorType", errorType);
    }
