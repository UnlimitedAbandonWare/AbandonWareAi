...
public class RoomService {
    ...
    public Message createMessage(String roomId, String identity, String content) {
        requireOwnedRoom(roomId, identity);
    ...
    /** Get messages after verifying the requested room belongs to the caller identity. */
    public List<Message> getMessagesForIdentity(String roomId, String identity, int limit) {
        requireOwnedRoom(roomId, identity);
    ...
    public Map<String, Object> linkGuestToUser(String sessionId, String userId) {
        ...
        res.put("linked", true);
        res.put("fromHash", com.example.lms.trace.SafeRedactor.hashValue(from));
        res.put("fromLength", from.length());
        res.put("toHash", com.example.lms.trace.SafeRedactor.hashValue(to));
        res.put("toLength", to.length());
    ...
    private Room requireOwnedRoom(String roomId, String identity) {
        if (roomId == null || roomId.isBlank()) {
            throw new IllegalArgumentException("room_id_required");
        }
        Room room = rooms.get(roomId);
        if (room == null) {
            throw new IllegalArgumentException("room_not_found");
        }
        if (!Objects.equals(identity, room.getOwnerIdentity())) {
            throw new SecurityException("room_access_denied");
        }
        return room;
    }
}
