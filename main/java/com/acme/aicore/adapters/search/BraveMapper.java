...
final class BraveMapper {
    ...
    static SearchBundle toBundle(String json) {
        ...
        try {
            ...
            traceSuppressed(json, e);
            return SearchBundle.empty();
        }
    }

    private static void traceSuppressed(String json, Exception error) {
        TraceStore.put("web.brave.mapper.parseFailed", true);
        TraceStore.put("web.brave.mapper.parseFailureClass",
                error == null ? "unknown" : error.getClass().getSimpleName());
        TraceStore.put("web.brave.mapper.responseLength", json == null ? 0 : json.length());
        TraceStore.put("web.brave.mapper.responseHash", SafeRedactor.hashValue(json));
        TraceStore.inc("web.brave.mapper.parseFailed.count");
    }
}
