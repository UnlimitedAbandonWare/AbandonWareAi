    ...
    public Mono<List<RankedDoc>> fuseAndRank(List<SearchBundle> bundles, RankingParams params) {
        ...
        if (stats != null) {
            try {
                ...
                traceSuppressed("statsFallback", ex);
            }
        }
        ...
        return Mono.fromCallable(() -> {
            ...
            try {
                ...
                traceSuppressed("rrfKFallback", ex);
            }
            int window = 50;
            try {
                if (params != null && params.windowM() > 0) window = params.windowM();
            } catch (RuntimeException ex) {
                traceSuppressed("windowFallback", ex);
            }
            ...
            for (SearchBundle b : bundles) {
                ...
                try {
                    ...
                    traceSuppressed("weightFallback", ex);
                }
                double w;
                if (mpReadyFinal) {
...
public Mono<List<RankedDoc>> rerank(List<RankedDoc> topN, RerankParams params) {
      ...
      .doOnSuccess(list -> {
          try {
              ...
              log.info("RANK_LABEL authority={} freshness={} rerank={} evidenceCount={} sessionHash={} xridHash={}",
                      "rrf", "n/a", "mp-aware", (list != null ? list.size() : 0), SafeRedactor.hashValue(sid), SafeRedactor.hashValue(xrid));
          } catch (RuntimeException ex) {
              traceSuppressed("labelLogFallback", ex);
          }
      });
    }

    private static void traceSuppressed(String stage, RuntimeException ex) {
        String prefix = "ranking.mpAware." + stage;
        TraceStore.inc(prefix + ".count");
        TraceStore.put(prefix + ".errorType", ex == null ? "RuntimeException" : ex.getClass().getSimpleName());
    }
}
