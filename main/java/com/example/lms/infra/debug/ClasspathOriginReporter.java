...
public class ClasspathOriginReporter implements ApplicationRunner {
    ...
    private void logDuplicates(String label, Class<?> clazz) {
        try {
            ...
            if (all.size() > 1) {
                log.warn("[ClasspathOrigin] DUPLICATE {} duplicateCount={} urlHashes={}",
                        label, all.size(), all.stream().map(ClasspathOriginReporter::urlFingerprint).toList());
            } else {
                URL url = all.isEmpty() ? null : all.get(0);
                log.info("[ClasspathOrigin] unique {} urlPresent={} urlHash={} urlLength={}",
                        label, url != null, urlHash(url), urlLength(url));
            }
        } catch (Exception e) {
            log.info("[ClasspathOrigin] {} -> <dup-scan-error errorHash={} errorLength={}>",
                    label, SafeRedactor.hashValue(messageOf(e)), messageLength(e));
        }
    }
    ...
    private void logOrigin(String label, Class<?> clazz) {
        try {
            ...
            log.info("[ClasspathOrigin] {} urlPresent={} urlHash={} urlLength={}",
                    label, url != null, urlHash(url), urlLength(url));
        } catch (Exception e) {
            log.info("[ClasspathOrigin] {} -> <error errorHash={} errorLength={}>",
                    label, SafeRedactor.hashValue(messageOf(e)), messageLength(e));
        }
    }
    ...
    private void logStereotype(String label, Class<?> clazz) {
        try {
            ...
            log.info("[ClasspathOrigin] stereotype {} -> <error errorHash={} errorLength={}>",
                    label, SafeRedactor.hashValue(messageOf(e)), messageLength(e));
        }
    }

    private static String urlFingerprint(URL url) {
        return urlHash(url) + ":len=" + urlLength(url);
    }

    private static String urlHash(URL url) {
        return url == null ? null : SafeRedactor.hashValue(String.valueOf(url));
    }

    private static int urlLength(URL url) {
        return url == null ? 0 : String.valueOf(url).length();
    }
