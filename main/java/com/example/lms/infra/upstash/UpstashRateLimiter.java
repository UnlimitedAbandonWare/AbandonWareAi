...
public class UpstashRateLimiter implements RateLimiterPort {
    ...
    public Mono<Boolean> allow(String bucket, long limit, Duration window) {
        ...
        long effectiveLimit = Math.max(1L, limit);
        Duration effectiveWindow = (window == null || window.isZero() || window.isNegative())
                ? Duration.ofSeconds(1)
                : window;
        return upstash.incrExpire("rl:" + bucketKey(bucket), effectiveWindow)
                .map(count -> count <= effectiveLimit);
    }
    ...
    private static String bucketKey(String bucket) {
        return bucket == null || bucket.isBlank() ? "default" : bucket.trim();
    }
}
