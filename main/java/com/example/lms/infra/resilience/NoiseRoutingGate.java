...
public final class NoiseRoutingGate {
    ...
    public static GateDecision decideEscape(String gateKey, double escapeP, GuardContext ctx) {
        try {
            ...
            try {
                ...
                traceSuppressed("noiseGate.trace", ignore);
            }

            return new GateDecision(escape, p, roll, seed);
        } catch (Throwable ignore) {
            traceSuppressed("noiseGate.decide", ignore);
    ...
    private static String sanitizeKey(String k) {
        ...
        // Keep trace keys stable + filesystem/JSON safe without promoting raw free-form labels.
        String label = com.example.lms.trace.SafeRedactor.traceLabel(k.trim());
    ...
    private static void traceSuppressed(String stage, Throwable failure) {
        ...
        TraceStore.put("orch.noiseGate.suppressed." + safeStage, true);
        TraceStore.put("orch.noiseGate.suppressed." + safeStage + ".errorType",
