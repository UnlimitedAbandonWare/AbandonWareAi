    ...
    public ModelsManifest modelsManifest() {
        try {
            ...
            try (InputStream in = getClass().getClassLoader()
                ...
                if (in == null) throw new IllegalStateException("Manifest not found " + safeManifestPath());
                return new Yaml().loadAs(in, ModelsManifest.class);
            } catch (Exception ee) {
                throw new IllegalStateException("Failed to load models manifest " + safeManifestPath(), ee);
            }
        }
    }

    private String safeManifestPath() {
        String path = String.valueOf(manifestPath);
        return "pathHash=" + com.example.lms.trace.SafeRedactor.hashValue(path)
