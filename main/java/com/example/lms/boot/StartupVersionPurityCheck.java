 ...
 * - dev.langchain4j 계열 전 모듈의 Implementation-Version이 EXPECTED_PREFIX(1.0.1)와 정확히 다르면 즉시 실패.
...
public class StartupVersionPurityCheck {
        ...
        try {
            ...
            while (resources.hasMoreElements()) {
                ...
                try (InputStream in = url.openStream()) {
                    ...
                    // 혼재 판정
                    if (ver != null && !EXPECTED_PREFIX.equals(ver)) {
                        badSet.add(entry);
                    }
                }
            }
    ...
    public String dumpInconsistentArtifacts() {
        try {
            ...
            traceSuppressed("startupVersion.dumpInconsistentArtifacts", e);
    ...
    private static void traceSuppressed(String stage, Exception failure) {
        if (log.isDebugEnabled()) {
            log.debug("LangChain4j purity dump fallback stage={} errorType={}",
                    stage,
