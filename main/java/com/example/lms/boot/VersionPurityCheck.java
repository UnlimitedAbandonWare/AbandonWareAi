 ...
 * package has an implementation version that is not exactly the
 * expected version (1.0.1), an IllegalStateException is thrown to abort
...
public class VersionPurityCheck {
    ...
    /**
     * The required exact version for all LangChain4j modules.  Should align
    ...
    public void verifyPackages() {
        ...
        for (Package p : packages) {
            ...
            if (name != null && name.startsWith("dev.langchain4j")) {
                ...
                if (implVer != null && !EXPECTED_PREFIX.equals(implVer)) {
                    mismatchFound = true;
