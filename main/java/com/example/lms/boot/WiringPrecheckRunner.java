...
public class WiringPrecheckRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        checkSingle("DebugEventStore", DebugEventStore.class, false);
        checkSingle("TraceSnapshotStore", TraceSnapshotStore.class, false);
        ...
        checkSingle("NightmareBreaker", NightmareBreaker.class, false);
        checkSingle("QueryTransformer", QueryTransformer.class, false);

        // Optional provider/RAG/GraphRAG seams: surface missing beans without failing boot.
        checkSingle("NaverSearchService", NaverSearchService.class, false);
        checkSingle("BraveSearchService", BraveSearchService.class, false);
        checkSingle("DynamicRetrievalHandlerChain", DynamicRetrievalHandlerChain.class, false);
        checkSingle("KnowledgeGraphHandler", KnowledgeGraphHandler.class, false);
        checkSingle("BrainStateService", BrainStateService.class, false);
        checkSingle("RagGraphExecutor", RagGraphExecutor.class, false);
        checkSingle("EvidenceRepairHandler", EvidenceRepairHandler.class, false);
        ...
        try {
            ...
            traceSuppressed("wiring.braveKeySnapshot", ignore);
        }
        ...
        try {
            ...
            traceSuppressed("wiring.hybridAwaitSnapshot", ignore);
        }
    ...
    private <T> void checkSingle(String label, Class<T> type, boolean required) {
        ...
        if (n == 1) {
            ...
            log.info("[Precheck] OK: {} -> beanHash={} beanLength={} ({})",
                    label, SafeRedactor.hashValue(name), lengthOf(name), type.getName());
            return;
        }
        ...
        // n > 1
        String beanSummary = beans.keySet().stream()
                .map(WiringPrecheckRunner::summarizeName)
                .sorted()
                .toList()
                .toString();
        log.warn("[Precheck] MULTIPLE: {} ({}) count={} beanSummaries={}",
                label, type.getName(), n, beanSummary);
    }
    ...
    private static void traceSuppressed(String stage, Throwable failure) {
        if (log.isDebugEnabled()) {
            log.debug("[Precheck] suppressed stage={} errorType={}",
                    stage,
                    failure == null ? "unknown" : failure.getClass().getSimpleName());
        }
    }

    private static String summarizeName(String value) {
        return "hash=" + SafeRedactor.hashValue(value) + ",length=" + lengthOf(value);
    }
}
