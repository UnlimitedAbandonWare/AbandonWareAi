...
public final class LlmErrorClassifier {
    ...
    public static Result classify(Throwable t) {
        try {
            ...
            if (lowerAll.contains("blank response") || lowerAll.contains("empty response")) {
                return new Result("BLANK_RESPONSE", true, null, shortMsg(t == null ? null : t.getMessage()));
            }
            ...
            String safeErrorType = SafeRedactor.traceLabelOrFallback(ignored.getClass().getSimpleName(), "unknown");
            TraceStore.put("llm.errorClassifier.suppressed.stage", "classify");
            TraceStore.put("llm.errorClassifier.suppressed.errorType", safeErrorType);
            TraceStore.put("llm.errorClassifier.suppressed.classify", true);
            TraceStore.put("llm.errorClassifier.suppressed.classify.errorType", safeErrorType);
    ...
    private static String shortMsg(String s) {
        ...
        Object safe = SafeRedactor.diagnosticValue("llm.error.message.payload", s, 160);
