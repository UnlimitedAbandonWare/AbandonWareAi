...
public class AdaptiveTranslateController {
    ...
    public Mono<Map<String, String>> translate(@RequestBody Map<String, String> payload) {
        String sourceText = payload == null ? null : clean(payload.get("text"));
        if (sourceText == null) {
            return missingText();
        }
        String srcLang = payload == null ? "ko" : clean(payload.getOrDefault("sourceLang", "ko"));
        String tgtLang = payload == null ? "en" : clean(payload.getOrDefault("targetLang", "en"));

        return adaptiveService.translate(sourceText, srcLang == null ? "ko" : srcLang, tgtLang == null ? "en" : tgtLang)
                .map(translatedText -> Map.of("translatedText", translatedText));
    }

    private static Mono<Map<String, String>> missingText() {
        return Mono.just(Map.of("error", "missing_text"));
    }
