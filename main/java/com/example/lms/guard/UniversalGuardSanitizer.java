    ...
    public String sanitize(String answer, PromptContext ctx) {
        ...
        for (String badToken : policy.discouraged()) {
            ...
            if (lowerAnswer.contains(badToken.toLowerCase(Locale.ROOT))) {
                log.warn("[Guard] Domain confusion detected. queryHash={}, queryLength={}, badTokenHash={} badTokenLength={}",
                        SafeRedactor.hashValue(query), query.length(), SafeRedactor.hashValue(badToken), badToken.length());
