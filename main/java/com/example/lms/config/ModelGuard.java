...
public class ModelGuard {
    ...
    public void verify() {
        ...
        base.put("baseUrl.present", StringUtils.hasText(p.baseUrl()));
        base.put("baseUrlHost", LocalLlmGatewaySecurity.endpointHost(p.baseUrl()));
        base.put("baseUrlHash", SafeRedactor.hashValue(p.baseUrl()));
        base.put("chatModel", p.chatModel());
        boolean apiKeyPresent = !ConfigValueGuards.isMissing(p.apiKey());
        base.put("apiKey.present", apiKeyPresent);
        base.put("env.OPENAI_API_KEY.present", !ConfigValueGuards.isMissing(env.getProperty("OPENAI_API_KEY")));
        base.put("env.openai.api.key.present", !ConfigValueGuards.isMissing(env.getProperty("openai.api.key")));
        base.put("env.GEMINI_API_KEY.present", !ConfigValueGuards.isMissing(env.getProperty("GEMINI_API_KEY")));
        base.put("env.gemini.api.key.present", !ConfigValueGuards.isMissing(env.getProperty("gemini.api.key")));
        ...
        if (!"local".equalsIgnoreCase(p.provider()) && !apiKeyPresent) {
            String provider = p.provider() == null ? "" : p.provider();
            IllegalStateException ex = new IllegalStateException("[LLM] api-key missing (llm.api-key) providerHash="
                    + SafeRedactor.hashValue(provider) + " providerLength=" + provider.length());
            debugEventStore.emit(DebugProbeType.MODEL_GUARD, DebugEventLevel.ERROR,
        ...
        boolean openaiKeyPresent = !ConfigValueGuards.isMissing(env.getProperty("OPENAI_API_KEY"))
                || !ConfigValueGuards.isMissing(env.getProperty("openai.api.key"));
