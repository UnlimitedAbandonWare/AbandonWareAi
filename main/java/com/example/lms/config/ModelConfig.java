...
public class ModelConfig {
  ...
  public ChatModel chatModel(Environment env) {
    ...
    boolean groqProvider = provider != null && provider.equalsIgnoreCase("groq");
    boolean localProvider = provider != null && provider.equalsIgnoreCase("local");
    if (groqProvider || localProvider) {
      ...
      String selectedApiKey = localProvider ? usableLocalKeyOrDefault(apiKey) : usableExternalKey(apiKey);
    ...
    String openApiKey  = usableExternalKey(apiKey);
    if (openApiKey == null) {
      openApiKey = usableExternalKey(env.getProperty("openai.api.key"));
    }
  ...
  private static String usableExternalKey(String apiKey) {
    return ConfigValueGuards.isMissing(apiKey) ? null : apiKey.trim();
  }

  private static String usableLocalKeyOrDefault(String apiKey) {
    return ConfigValueGuards.isMissingLocalOpenAiCompatKey(apiKey) ? "ollama" : apiKey.trim();
  }
}
