...
public class UpstashKeyValidator implements EnvironmentAware {
    ...
    private boolean isMeaningfulPropertyDefined(String key) {
        ...
        try {
            ...
            if (v == null)
                return false;
            v = v.trim();
            if (ConfigValueGuards.isMissing(v))
                return false;
            return true;
        } catch (Exception e) {
            traceSuppressed("upstash.propertyProbe", e);
            return false;
        }
    }
    ...
    private static void traceSuppressed(String stage, Throwable ignored) {
        ...
        TraceStore.put("upstash.config.suppressed." + safeStage, true);
        TraceStore.put("upstash.config.suppressed." + safeStage + ".errorType",
                ignored == null ? "unknown"
