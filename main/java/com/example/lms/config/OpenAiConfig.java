...
public class OpenAiConfig {
    ...
    public Object openAiService() {
        ...
        if (localEnabled && localBaseUrl != null && !localBaseUrl.isBlank()) {
            ...
            String token = ConfigValueGuards.isMissingLocalOpenAiCompatKey(openAiKey) ? "ollama" : openAiKey.trim();
        ...
        if (ConfigValueGuards.isMissing(openAiKey)) {
            throw new IllegalStateException("OpenAI API key is missing");
        }
        String normalizedOpenAiKey = openAiKey.trim();
        if (!normalizedOpenAiKey.startsWith("sk-")) {
            throw new IllegalStateException("OpenAI API key must start with 'sk-'");
        }

        OkHttpClient client = makeClient(normalizedOpenAiKey, Duration.ofSeconds(60));
        ObjectMapper mapper = makeMapper();
