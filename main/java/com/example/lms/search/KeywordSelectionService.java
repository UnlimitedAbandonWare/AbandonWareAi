...
public class KeywordSelectionService {
    ...
    private static void traceRule(String msg) {
        ...
        if (m.isEmpty())
            return;
        m = SafeRedactor.safeMessage(m, 220);
        if (m == null || m.isBlank())
            return;
        if (m.length() > 220)
            m = m.substring(0, 220) + "…";
    ...
    private static void traceSuppressed(String stage, Throwable failure) {
        ...
        try {
            TraceStore.put("keywordSelection.suppressed." + safeStage, true);
            TraceStore.put("keywordSelection.suppressed." + safeStage + ".errorType", errorType);
        } catch (RuntimeException traceFailure) {
            log.debug("[KeywordSelection] suppressed trace failed stage={} errorType={}",
    ...
    private static int positiveIntProperty(String key, int fallback, String stage) {
        ...
        try {
            ...
            traceSuppressed("config." + stage + ".parse", ignore);
            return fallback;
        }
    }
    ...
    public Optional<SelectedTerms> select(String conversation, String domainProfile, int maxMust) {
        ...
        try {
            ...
            try {
                ...
                if (qtxFailureCooldown) {
                    TraceStore.putIfAbsent("aux.keywordSelection.qtxSoftCooldown.hint", true);
                }
            } catch (Throwable ignore) {
                traceSuppressed("qtxSoftCooldown.globalProbe", ignore);
            }
            if (!breakerOpen && !ctxSaysBlock && !blockByOtherAuxOpen && qtxFailureCooldown) {
                boolean blockOnQtxSoftCooldown = Boolean.parseBoolean(System.getProperty(
                        "orch.keywordSelection.qtxSoftCooldown.block",
                        System.getProperty("orch.keywordSelection.blockOnQtxSoftCooldown", "false")));
                boolean preferCacheOnQtxSoftCooldown = Boolean.parseBoolean(System.getProperty(
                        "orch.keywordSelection.qtxSoftCooldown.preferCache", "true"));
                boolean allowOneShotOnQtxSoftCooldown = Boolean.parseBoolean(System.getProperty(
                        "orch.keywordSelection.qtxSoftCooldown.allowOneShot", "true"));

                try {
                    // New keyspace (preferred): qtxSoftCooldown
                    TraceStore.put("keywordSelection.qtxSoftCooldown", true);
                    TraceStore.put("keywordSelection.qtxSoftCooldown.policy",
                            blockOnQtxSoftCooldown ? "block" : "soft_allow");
                    TraceStore.put("keywordSelection.qtxSoftCooldown.reason", AuxBlockedReason.FAILURE_COOLDOWN.code());
                    if (qtxSignal != null) {
                        TraceStore.put("keywordSelection.qtxSoftCooldown.qtx", qtxSignal);
                    }

                    // Legacy compatibility: qtxGate
                    TraceStore.put("keywordSelection.qtxGate", true);
                    TraceStore.put("keywordSelection.qtxGate.policy", blockOnQtxSoftCooldown ? "block" : "soft_allow");
                    ...
                    traceSuppressed("qtxSoftCooldown.breadcrumb", suppressed);
                }

                // Prefer cached keywords during QTX soft-cooldown to keep latency low.
                if (preferCacheOnQtxSoftCooldown && cached != null) {
                    try {
                        TraceStore.put("keywordSelection.mode", "cache_rescue_qtxSoftCooldown");
                    } catch (Throwable suppressed) {
                        traceSuppressed("qtxSoftCooldown.cacheMode", suppressed);
                    }
                    traceRule("keywordSelection.mode=cache_rescue_qtxSoftCooldown");
                    return Optional.of(ensureMinMust(deepCopy(cached), conversation, domainProfile, 2,
                            "cache_rescue_qtxSoftCooldown"));
                }

                // Old behavior (hard block) is still available via property.
                if (blockOnQtxSoftCooldown) {
                    try {
                        TraceStore.put("keywordSelection.mode", "blocked_qtxSoftCooldown");
                        ...
                        TraceStore.putIfAbsent("aux.keywordSelection.blocked", true);
                        TraceStore.putIfAbsent("aux.keywordSelection.blocked.reason", "failure-cooldown");
                        TraceStore.putIfAbsent("aux.keywordSelection.blocked.by", "qtxSoftCooldown");
                        ...
                        AuxBlockTracker.markStageBlocked(
                                ...
                                "KeywordSelectionService.select(qtxSoftCooldown)",
                                qtxSignal);
                    } catch (Throwable suppressed) {
                        traceSuppressed("qtxSoftCooldown.blockMode", suppressed);
                    }
                    traceRule("keywordSelection.mode=blocked_qtxSoftCooldown (failure-cooldown)");
                    log.debug("[Aux] keywordSelection blocked by qtxSoftCooldown (qtx={})", qtxSignal);
                    if (cached != null) {
                        try {
                            TraceStore.put("keywordSelection.mode", "cache_rescue_qtxSoftCooldown");
                        } catch (Throwable suppressed) {
                            traceSuppressed("qtxSoftCooldown.blockCacheMode", suppressed);
                        }
                        traceRule("keywordSelection.mode=cache_rescue_qtxSoftCooldown");
                        return Optional.of(ensureMinMust(deepCopy(cached), conversation, domainProfile, 2,
                                "cache_rescue_qtxSoftCooldown"));
                    ...
                    SelectedTerms fb = fallbackTerms(conversation, domainProfile, mustLim);
                    try {
                        TraceStore.put("keywordSelection.mode", "fallback_qtxSoftCooldown");
                    } catch (Throwable suppressed) {
                        traceSuppressed("qtxSoftCooldown.fallbackMode", suppressed);
                    }
                    traceRule("keywordSelection.mode=fallback_qtxSoftCooldown");
                    return Optional.of(ensureMinMust(fb, conversation, domainProfile, 2, "fallback_qtxSoftCooldown"));
                }

                // Soft allow: do not fully block keywordSelection just because QTX is cooling down.
                // To avoid repeated LLM calls while QTX is unstable, allow at most one LLM attempt per request.
                if (allowOneShotOnQtxSoftCooldown) {
                    Object prev = null;
                    try {
                        // New breadcrumb: one-shot gate is in play for this request.
                        TraceStore.putIfAbsent("keywordSelection.qtxSoftCooldown.oneShot.used", true);

                        // Legacy compatibility: keep the old dedup key too.
                        prev = TraceStore.putIfAbsent("keywordSelection.qtxGate.softAllow.used", true);
                    } catch (Throwable suppressed) {
                        traceSuppressed("qtxSoftCooldown.oneShot", suppressed);
                    }
                    if (prev != null) {
                        // Dedup: do not hit LLM again. Prefer cache -> fallback.
                        if (cached != null) {
                            try {
                                TraceStore.put("keywordSelection.mode", "cache_rescue_qtxSoftCooldown_dedup");
                            } catch (Throwable suppressed) {
                                traceSuppressed("qtxSoftCooldown.dedupCacheMode", suppressed);
                            }
                            traceRule("keywordSelection.mode=cache_rescue_qtxSoftCooldown_dedup");
                            return Optional.of(ensureMinMust(deepCopy(cached), conversation, domainProfile, 2,
                                    "cache_rescue_qtxSoftCooldown_dedup"));
                        }
                        SelectedTerms fb = fallbackTerms(conversation, domainProfile, mustLim);
                        try {
                            TraceStore.put("keywordSelection.mode", "fallback_qtxSoftCooldown_dedup");
                        } catch (Throwable suppressed) {
                            traceSuppressed("qtxSoftCooldown.dedupFallbackMode", suppressed);
                        }
                        traceRule("keywordSelection.mode=fallback_qtxSoftCooldown_dedup");
                        return Optional.of(ensureMinMust(fb, conversation, domainProfile, 2,
                                "fallback_qtxSoftCooldown_dedup"));
                    }

                    // First LLM attempt under QTX soft-cooldown.
                    try {
                        TraceStore.putIfAbsent("keywordSelection.qtxGate.softAllow.oneShotAttempted", true);
                    } catch (Throwable suppressed) {
                        traceSuppressed("qtxSoftCooldown.oneShotAttempted", suppressed);
                    }
                }

                try {
                    TraceStore.put("keywordSelection.mode", "soft_allow_qtxSoftCooldown");
                } catch (Throwable suppressed) {
                    traceSuppressed("qtxSoftCooldown.softAllowMode", suppressed);
                }
                traceRule("keywordSelection.mode=soft_allow_qtxSoftCooldown");
                // fall through: run LLM keyword selection normally
            }
            ...
            if (breakerOpen || ctxSaysBlock || blockByOtherAuxOpen) {
                ...
                if (!noiseEscaped) {
                    try {
                        ...
                        traceSuppressed("blocked.mode", suppressed);
                    }
                    ...
                    if (cached != null) {
                        try {
                            ...
                            traceSuppressed("blocked.cacheMode", suppressed);
                        }
                ...
                try {
                    ...
                    traceSuppressed("noiseEscape.trace", suppressed);
                }
                try {
                    ...
                    traceSuppressed("noiseEscape.blockTracker", suppressed);
                }
            }
        } catch (Throwable ignore) {
        ...
        try {
            ...
            int maxConvLines = positiveIntProperty("keywordSelection.maxConversationLines", 80,
            ...
            try {
                TraceStore.put("keywordSelection.domainProfile", SafeRedactor.hashValue(dp));
            } catch (Throwable suppressed) {
                traceSuppressed("domainProfile.trace", suppressed);
            }
            try {
                TraceStore.put("keywordSelection.maxMust", mustLim);
            } catch (Throwable suppressed) {
                traceSuppressed("maxMust.trace", suppressed);
            }
            traceRule("keywordSelection.start domainProfileHash=" + SafeRedactor.hashValue(dp)
                    + " domainProfileLength=" + dp.length() + " maxMust=" + mustLim);

            String selectedTermsPrompt = prompts.buildSelectedTermsJsonPrompt(
                    convForPrompt,
                    dp,
                    mustLim);
            ...
            if (json == null || json.isBlank()) {
                ...
                try {
                    ...
                    traceSuppressed("blank.auxSoft", suppressed);
                }
                try {
                ...
                if (cached != null) {
                    try {
                        ...
                        traceSuppressed("blank.cacheMode", suppressed);
                    }
                ...
                try {
                    ...
                    traceSuppressed("blank.forceMinMust", suppressed);
                }
                try {
            ...
            if (terms == null) {
                ...
                try {
                    ...
                    traceSuppressed("jsonNull.degradedTrace", suppressed);
                }
                ...
                try {
                    ...
                    traceSuppressed("jsonNull.cacheSeed", suppressed);
                }
                return Optional.of(fb);
            }
            ...
            try {
                ...
                traceSuppressed("parse.auxSoft", suppressed);
            }
            try {
            ...
            if (cached != null) {
                try {
                    ...
                    traceSuppressed("parse.cacheMode", suppressed);
                }
            ...
            try {
                ...
                traceSuppressed("exception.auxDegraded", suppressed);
            }
            try {
            ...
            if (cached != null) {
                try {
                    ...
                    traceSuppressed("exception.cacheMode", suppressed);
                }
    ...
    private static String cacheKey(String conversation, String domainProfile, int mustLim) {
        ...
        try {
            ...
            traceSuppressed("cacheKey.trace", suppressed);
        }
    ...
    private static List<String> copyList(List<String> in) {
        ...
        for (String s : in) {
            if (s == null)
                continue;
            String t = safeSelectedTerm(s, 160);
            if (t.isBlank())
                continue;
            out.add(t);
        }
    ...
    private static String safeSelectionSeed(String raw, int maxLen) {
        if (raw == null || raw.isBlank()) {
            return "";
        }
        String safe = SafeRedactor.safeMessage(raw, Math.max(32, maxLen));
        if (safe == null || safe.isBlank()) {
            return "";
        }
        return cleanupRedactedSelectedTerm(safe, false);
    }

    private static String safeSelectedTerm(String raw, int maxLen) {
        if (raw == null || raw.isBlank()) {
            return "";
        }
        String original = raw.trim();
        String safe = SafeRedactor.safeMessage(original, Math.max(32, maxLen));
        if (safe == null || safe.isBlank()) {
            return "";
        }
        String cleaned = cleanupRedactedSelectedTerm(safe, true);
        if (cleaned.isBlank() || looksLikeOpaqueSecretToken(cleaned)) {
            return "";
        }
        return cleaned;
    }

    private static String cleanupRedactedSelectedTerm(String value, boolean dropSensitiveLabels) {
        if (value == null || value.isBlank()) {
            return "";
        }
        String cleaned = value
                .replace("***", " ")
                .replaceAll("(?i)\\b(owner\\s*token|ownertoken|api[_-]?key|apikey|client[_-]?secret|authorization|bearer|cookie)\\b\\s*[:=]?", " ")
                .replaceAll("\\s+", " ")
                .trim();
        if (dropSensitiveLabels && cleaned.matches("(?i).*(owner\\s*token|ownertoken|api[_-]?key|apikey|client[_-]?secret|authorization|bearer|cookie).*")) {
            return "";
        }
        return cleaned;
    }

    private static boolean looksLikeOpaqueSecretToken(String value) {
        if (value == null || value.isBlank()) {
            return false;
        }
        String t = value.trim();
        ...
        if ((lower.startsWith("sk-") && t.length() >= 12)
                || (lower.startsWith("gsk_") && t.length() >= 12)
                || (lower.startsWith("pcsk_") && t.length() >= 12)
                || (lower.startsWith("sb_secret_") && t.length() >= 20)
                || (lower.startsWith("sb_publishable_") && t.length() >= 20)
                || (t.startsWith("AIza") && t.length() >= 12)) {
            return true;
        }
        boolean hasLetter = t.matches(".*[A-Za-z].*");
        boolean hasDigit = t.matches(".*\\d.*");
        boolean compact = t.matches("[A-Za-z0-9_-]+");
        return hasLetter && hasDigit && compact;
    }
    ...
    private static SelectedTerms ensureMinMust(
        ...
        if (in == null) {
            ...
            try {
                ...
                traceSuppressed("forceMinMust.nullInput", suppressed);
            }
            ...
            try {
                ...
                traceSuppressed("forceMinMust.nullInputMustCount", suppressed);
            }
            return fb;
        }
        ...
        if (must.size() >= min) {
            ...
            try {
                ...
                traceSuppressed("forceMinMust.existingMustCount", suppressed);
            }
            return in;
        }
        ...
        if (must.size() < min) {
            ...
            if (must.size() >= 1) {
                ...
                try {
                    ...
                    traceSuppressed("forceMinMust.partial", suppressed);
                }
            }
            try {
                TraceStore.put("aux.keywordSelection.must.count", must.size());
                ...
                traceSuppressed("forceMinMust.partialMustCount", suppressed);
            }
            return in;
        }
        ...
        try {
            ...
            traceSuppressed("forceMinMust.applied", suppressed);
        }
        try {
            TraceStore.put("aux.keywordSelection.must.count", must.size());
            TraceStore.put("aux.keywordSelection.must.nonBlank.count", must.size());
            ...
            traceSuppressed("forceMinMust.appliedMustCount", suppressed);
        }
        return in;
    }
    ...
    private static SelectedTerms fallbackTerms(String conversation, String domainProfile, int maxMust) {
        ...
        if (maxMust < 2) {
            try {
                ...
                traceSuppressed("fallbackTerms.forceMinMust", suppressed);
            }
        }
        String conv = Objects.toString(conversation, "");
        String dp = Objects.toString(domainProfile, "general");

        String base = safeSelectionSeed(extractLikelyUserQuery(conv), 160);
        ...
        try {
            var gctx = GuardContextHolder.getOrDefault();
            String uq = safeSelectionSeed((gctx == null) ? null : gctx.getUserQuery(), 160);
        ...
        if (anchorCandidate.length() < 3) {
            String alt = safeSelectionSeed(scanConversationForAnchor(conv, 3), 160);
        ...
        try {
            ...
            if (entityPhrase != null && !entityPhrase.isBlank()) {
                ...
                try {
                    TraceStore.put("keywordSelection.fallback.entityPhrase.hash12", SafeRedactor.hash12(entityPhrase));
                    TraceStore.put("keywordSelection.fallback.entityPhrase.len", entityPhrase.length());
                } catch (Throwable ignore) {
            ...
            if (exactPhrase != null && exactPhrase.length() >= 2 && exactPhrase.length() <= 80
                ...
                try {
                    TraceStore.put("keywordSelection.fallback.exact.hash12", SafeRedactor.hash12(exactPhrase));
                    TraceStore.put("keywordSelection.fallback.exact.len", exactPhrase.length());
        ...
        st.setMust(copyList(must));
        st.setExact(copyList(exact));
        st.setShould(copyList(should));
        st.setMaybe(List.of());
        ...
        try {
            TraceStore.put("keywordSelection.fallback.anchor.hash12", SafeRedactor.hash12(anchor));
            TraceStore.put("keywordSelection.fallback.anchor.len", anchor.length());
            ...
            TraceStore.put("keywordSelection.fallback.must.count", must.size());
            TraceStore.put("keywordSelection.fallback.must.hashes", SelectedTermsDebug.sampleTokens(must, 8));
            TraceStore.put("keywordSelection.fallback.should.count", should.size());
            TraceStore.put("keywordSelection.fallback.should.hashes", SelectedTermsDebug.sampleTokens(should, 8));
            if (secondMust != null && !secondMust.isBlank()) {
                TraceStore.put("keywordSelection.fallback.secondMust.hash12", SafeRedactor.hash12(secondMust));
                TraceStore.put("keywordSelection.fallback.secondMust.len", secondMust.length());
            }
        ...
        try {
            ...
            eventData.put("seedSource", seedSource);
            eventData.put("seedHash12", Objects.toString(SafeRedactor.hash12(seedPreview), ""));
            eventData.put("seedLen", seedPreview == null ? 0 : seedPreview.length());
            eventData.put("intent", intent.name());
            eventData.put("anchorHash12", Objects.toString(SafeRedactor.hash12(anchor), ""));
            eventData.put("anchorLen", anchor == null ? 0 : anchor.length());
            eventData.put("secondMustHash12", Objects.toString(SafeRedactor.hash12(secondMust), ""));
            eventData.put("secondMustLen", secondMust == null ? 0 : secondMust.length());
            eventData.put("exactHash12", exact.isEmpty() ? "" : Objects.toString(SafeRedactor.hash12(exact.get(0)), ""));
            eventData.put("exactLen", exact.isEmpty() || exact.get(0) == null ? 0 : exact.get(0).length());
            eventData.put("mustSize", must.size());
            eventData.put("shouldSize", should.size());
