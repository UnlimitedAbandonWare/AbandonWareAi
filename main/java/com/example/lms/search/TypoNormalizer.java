...
public class TypoNormalizer {
    ...
    public TypoNormalizer(Environment env) {
        ...
        try {
            ...
            log.warn("Typo map bind failed (tree). Proceeding with fallbacks. errorHash={} errorLength={}",
        ...
        if (m.isEmpty()) {
            ...
            if (raw != null && !raw.isBlank()) {
                ...
                try {
                    ...
                    log.warn("Typo map parse failed (rawHash={} rawLength={}). errorHash={} errorLength={}",
                            SafeRedactor.hashValue(raw),
                            raw.length(),
                            SafeRedactor.hashValue(messageOf(ex)), messageLength(ex));
                }
            }
