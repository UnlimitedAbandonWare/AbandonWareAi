...
public class WorkflowOrchestrator {
    ...
    @Value("${plans.auto-select.document:document_evidence.v1}")
    private String documentPlanId;

    public String ensurePlanSelected(GuardContext ctx, AnswerMode answerMode,
                                    QueryDomain domain, String userQuery) {
        return ensurePlanSelected(ctx, answerMode, domain, userQuery, false);
    }
    ...
    public String ensurePlanSelected(GuardContext ctx, AnswerMode answerMode,
        ...
        try {
            ...
            LOG.log(System.Logger.Level.DEBUG,
                    "Workflow plan normalization skipped stage=plan_hint_load errorType="
    ...
    private static boolean looksBraveHint(GuardContext ctx) {
        ...
        return lower.contains("brave") || lower.contains("creative") || lower.equals("s2");
    }

    private static boolean looksFreeCostHint(GuardContext ctx) {
        if (ctx == null) return false;
        String hm = ctx.getHeaderMode();
        if (hm == null || hm.isBlank()) return false;
        String lower = hm.toLowerCase(Locale.ROOT);
