 ...
 * token is configured, reload remains available only to authenticated
 * administrators.
 */
@RestController
...
public class DomainProfileController {
    ...
    private static final System.Logger LOG = System.getLogger(DomainProfileController.class.getName());
     ...
     * token is configured in the application properties, the caller must
     * already be authenticated with ROLE_ADMIN.
     *
    ...
    public ResponseEntity<Map<String, Object>> reload(@RequestHeader(value = "X-Admin-Token", required = false) String token,
        ...
        try {
            ...
            traceSuppressed("domainProfile.authLookup", ignore);
        }
        ...
        if (!allowed) {
            ...
            if (tokenMatches(configured, token)) {
                allowed = true;
            }
        }
        if (!allowed) {
    ...
    private static boolean tokenMatches(String expected, String candidate) {
        ...
        byte[] right = candidate.trim().getBytes(StandardCharsets.UTF_8);
    ...
    private static void traceSuppressed(String stage, Exception failure) {
        if (LOG.isLoggable(System.Logger.Level.DEBUG)) {
            LOG.log(System.Logger.Level.DEBUG,
                    "Domain profile authorization fallback stage={0} errorType={1}",
                    stage,
