...
public class ClasspathDiagnosticsController {
    ...
    private static final System.Logger LOG = System.getLogger(ClasspathDiagnosticsController.class.getName());
    ...
    private Map<String, Object> probe(String fqcn) {
        ...
        try {
            ...
            if (loc != null) putLocationMeta(m, String.valueOf(loc));
        } catch (Throwable t) {
            traceSuppressed("classpath.probe", t);
            m.put("present", false);
            m.put("error", classpathProbeError(t));
        }
        return m;
    }
    ...
    private static void traceSuppressed(String stage, Throwable failure) {
        if (LOG.isLoggable(System.Logger.Level.DEBUG)) {
            LOG.log(System.Logger.Level.DEBUG,
                    "Classpath diagnostic probe skipped stage={0} errorType={1}",
                    stage,
                    failure == null ? "unknown" : failure.getClass().getSimpleName());
        }
    }

    private static String classpathProbeError(Throwable t) {
        String reason = "classpath_probe_failed";
        if (t instanceof ClassNotFoundException) {
            reason = "class_not_found";
        } else if (t instanceof NoClassDefFoundError) {
            reason = "no_class_def";
        }
        String message = t == null ? "" : java.util.Objects.toString(t.getMessage(), "");
        return reason
                + " messageHash=" + com.example.lms.trace.SafeRedactor.hashValue(message)
                + " messageLength=" + message.length();
    }

    private static void putLocationMeta(Map<String, Object> m, String location) {
        String value = location == null ? "" : location;
        m.put("locationHash", com.example.lms.trace.SafeRedactor.hashValue(value));
        m.put("locationLength", value.length());
    }
