...
public class UawDatasetTrainingDataFilter {
    ...
    private static final Pattern LEGACY_DEGRADED_MODE_BANNER = legacyDegradedBannerPattern(
    ...
    private static Pattern legacyDegradedBannerPattern(String ignoredPattern, int ignoredFlags) {
        return Pattern.compile("^\\s*※\\s*\\[DEGRADED\\s+MODE\\]",
                Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
    }
    ...
    private static String sampleHash(String question, String answer, String modelUsed) {
        try {
            ...
            log.debug("[AWX][uaw][training-filter] sample hash fallback errorType={}",
    ...
    private static String safeTagValue(String value) {
        ...
        var v = SafeRedactor.traceLabelOrFallback(value.trim(), "none");
        if (v.isEmpty()) {
            return "none";
        }

        // Keep label values bounded and readable.
        v = v.replace("hash:", "hash_").replaceAll("[^A-Za-z0-9_.:-]+", "_");
        if (v.length() > 120) {
            v = v.substring(0, 120);
        }
    ...
    private CompiledRule compileRule(UawDatasetFilterProperties.Rule rule) {
        ...
        for (String rawPattern : rule.getPatterns()) {
            if (rawPattern == null || rawPattern.isBlank()) {
                continue;
            }
            try {
                patterns.add(Pattern.compile(rawPattern, Pattern.CASE_INSENSITIVE | Pattern.MULTILINE));
            } catch (PatternSyntaxException e) {
                log.warn(
                        "[AWX][yaml] invalid dataset-filter regex skipped rule={} scope={} group={} patternHash={} patternLength={} errorClass={} errorMessage={}",
                        safeTagValue(rule.getName()),
                        rule.getScope() == null ? "none" : rule.getScope().key(),
                        safeTagValue(rule.getGroup()),
                        patternHash(rawPattern),
                        rawPattern.length(),
                        e.getClass().getSimpleName(),
                        patternSyntaxDescription(e));
            }
        }
    ...
    private static String patternHash(String pattern) {
        return sampleHash(pattern, "", "");
    }

    private static String patternSyntaxDescription(PatternSyntaxException e) {
        if (e == null || e.getDescription() == null || e.getDescription().isBlank()) {
            return "unknown";
        }
        var msg = e.getDescription().replace('\n', ' ').replace('\r', ' ').trim();
