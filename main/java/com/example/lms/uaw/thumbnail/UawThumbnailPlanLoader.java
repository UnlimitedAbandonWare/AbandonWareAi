...
public class UawThumbnailPlanLoader {
    ...
    private UawThumbnailPlanSpec loadInternal(String normalizedPlanId, UawThumbnailProperties props) {
        ...
        if (!res.exists()) {
            log.warn("[UAW_THUMB] plan not found pathHash={} pathLength={} -> using defaults",
                    com.example.lms.trace.SafeRedactor.hashValue(path), path.length());
        ...
        try (InputStream in = res.getInputStream()) {
            ...
            log.warn("[UAW_THUMB] plan load failed pathHash={} pathLength={} -> using defaults. errorHash={} errorLength={}",
