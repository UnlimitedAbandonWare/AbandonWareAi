...
public class LocationService {
    ...
    public void saveEvent(String userId, LocationEventDto dto) {
        ...
        try {
            ...
            traceLocationSuppressed("saveEvent.reverseGeocode", ignore);
    ...
    public Optional<String> answerWhereAmI(String userId) {
        ...
        if (!isEnabled(userId)) {
            log.info("answerWhereAmI: userHash={} has not enabled location consent", userHash(userId));
            return Optional.empty();
        }
        var lastOpt = locationRepo.findByUserId(userId);
        if (lastOpt.isEmpty()) {
            log.info("answerWhereAmI: no last location recorded for userHash={}", userHash(userId));
            return Optional.empty();
        }
        ...
        try {
            ...
            traceLocationSuppressed("answerWhereAmI.staleAge", ignore);
        ...
        try {
            ...
            traceLocationSuppressed("answerWhereAmI.reverseGeocode", ignore);
    ...
    private static String userHash(String userId) {
        String hash = SafeRedactor.hashValue(userId);
        return hash == null ? "" : hash;
    }

    private static void traceLocationSuppressed(String stage, Throwable failure) {
        String safeStage = SafeRedactor.traceLabelOrFallback(stage, "unknown");
        String safeErrorType = failure == null
                ? "unknown"
                : SafeRedactor.traceLabelOrFallback(failure.getClass().getSimpleName(), "unknown");
        TraceStore.put("location.suppressed.stage", safeStage);
        TraceStore.put("location.suppressed.errorType", safeErrorType);
        TraceStore.put("location.suppressed." + safeStage, true);
        TraceStore.put("location.suppressed." + safeStage + ".errorType", safeErrorType);
    }
}
