...
public class LocationController {
    ...
    public ResponseEntity<?> setConsent(@PathVariable String onOff,
        ...
        if (uid == null || uid.isBlank()) {
            return ResponseEntity.badRequest().body(errorBody("missing_user_identifier"));
        }
    ...
    public ResponseEntity<?> ingest(@RequestBody LocationEventDto dto,
        ...
        if (!svc.isEnabled(uid)) {
            return ResponseEntity.status(412).body(errorBody("location_consent_disabled"));
        }
    ...
    private static Map<String, Object> errorBody(String code) {
        return Map.of("accepted", false, "error", code);
    }
}
