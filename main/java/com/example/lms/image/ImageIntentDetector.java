...
public class ImageIntentDetector {
    ...
    public boolean isImageIntent(String query) {
        ...
        if (isSlashCommand(q, "/image") || isSlashCommand(q, "/img")) {
            return true;
        }
        return IMAGE_HINT.matcher(q).find();
    }

    private static boolean isSlashCommand(String query, String command) {
        if (!query.regionMatches(true, 0, command, 0, command.length())) {
            return false;
        }
        return query.length() == command.length()
                || Character.isWhitespace(query.charAt(command.length()));
    }
}
