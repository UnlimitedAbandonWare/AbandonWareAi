...
public class AutonomousExplorationService {
    ...
    public void explore() {
        try {
            ...
            log.info("[AutonomousExploration] Exploring knowledge gap: queryHash={}, queryLength={}, domainHash={} domainLength={} subjectHash={} subjectLength={}",
                    SafeRedactor.hashValue(gap.getQuery()), gap.getQuery().length(),
                    SafeRedactor.hashValue(gap.getDomain()), gap.getDomain().length(),
                    SafeRedactor.hashValue(gap.getSubject()), gap.getSubject().length());
            ...
            for (String q : coreQueries) {
                try {
                    ...
                    log.warn("[AutonomousExploration] Query expansion failed queryHash12={} queryLength={} errorHash={} errorLength={}",
                            com.example.lms.trace.SafeRedactor.hash12(q), q == null ? 0 : q.length(),
            ...
            try {
                ...
                log.warn("[AutonomousExploration] Retrieval failed. errorHash={} errorLength={}",
                        SafeRedactor.hashValue(messageOf(e)), messageLength(e));
                return;
            }
            if (results == null || results.isEmpty()) {
                log.info("[AutonomousExploration] No evidence found queryHash12={} queryLength={}", com.example.lms.trace.SafeRedactor.hash12(gap.getQuery()), gap.getQuery() == null ? 0 : gap.getQuery().length());
                return;
            }
            ...
            for (Content c : results) {
                try {
                    ...
                    AgentTraceSuppressions.traceSuppressed("autonomousExploration.evidenceSnippet", e);
            ...
            try {
                ...
                log.warn("[AutonomousExploration] Curation failed. errorHash={} errorLength={}",
                        SafeRedactor.hashValue(messageOf(e)), messageLength(e));
            }
        } catch (Exception ex) {
            log.error("[AutonomousExploration] Unexpected error type={} errorHash={} errorLength={}",
                    ex.getClass().getSimpleName(), SafeRedactor.hashValue(messageOf(ex)), messageLength(ex));
        }
    }
