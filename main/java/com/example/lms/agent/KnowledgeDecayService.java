            ...
            for (DomainKnowledge dk : list) {
                ...
                if (dk.getEntityName() != null && !dk.getEntityName().isBlank()) {
                    try {
                        ...
                        AgentTraceSuppressions.traceSuppressed("knowledgeDecay.synergyLookup", ignore);
                    }
                }
                ...
                if (Math.abs(newConf - oldConf) > 1e-6) {
                    ...
                    try {
                        ...
                        log.warn("[KnowledgeDecay] Failed to save updated confidence domainHash={} domainLength={} entityHash={} entityLength={}",
                                SafeRedactor.hashValue(dk.getDomain()), lengthOf(dk.getDomain()),
                                SafeRedactor.hashValue(dk.getEntityName()), lengthOf(dk.getEntityName()));
                    }
                    log.debug("[KnowledgeDecay] Updated confidence domainHash={} domainLength={} entityHash={} entityLength={} from={} to={}",
                            SafeRedactor.hashValue(dk.getDomain()), lengthOf(dk.getDomain()),
                            SafeRedactor.hashValue(dk.getEntityName()), lengthOf(dk.getEntityName()),
                            String.format("%.3f", oldConf), String.format("%.3f", newConf));
                }
            }
        } catch (Exception e) {
