...
public class CuriosityTriggerService {
    ...
    public Optional<KnowledgeGap> findKnowledgeGap() {
        ...
        try {
            String raw = chatModel.generate(curiosityGapPrompt, 0.2, 400);
            ...
            log.debug("[Curiosity] parsing failed. errorHash={} errorLength={}",
    ...
    private KnowledgeGap parseGapLenient(String raw) {
        ...
        try {
            ...
            log.debug("[Curiosity] JSON parse error, candidateHash={} candidateLength={}",
                    SafeRedactor.hashValue(candidate), candidate.length());
            ObjectNode fallback = om.createObjectNode();
            fallback.put("description", SafeRedactor.safeMessage(candidate, 200));
    ...
    private JsonNode tryParseJson(String candidate) throws Exception {
        try {
            ...
            AgentTraceSuppressions.traceSuppressed("curiosity.tryParseJson", e);
    ...
    private static String truncate(String s, int max) {
        if (s == null)
            return "";
        String t = SafeRedactor.safeMessage(s, Math.max(0, max));
        return t == null ? "" : t;
    }
}
