...
public class SoakQuickRunner implements ApplicationRunner {
    ...
    public void scheduled() {
        ...
        try {
            ...
            log.error("[SOAK] scheduled quick_report failed failureClass={} errorType={}",
                    quickFailureClass(e),
    ...
    public SoakQuickBundleReport runOnce(String trigger) throws Exception {
        ...
        try (AutoCloseable __ = TraceContext.attach(sid, traceId)) {
            ...
            String outPath = String.valueOf(out.toAbsolutePath());
            String outHash = SafeRedactor.hashValue(outPath);
            int outLength = outPath.length();
            if ("FAIL".equalsIgnoreCase(overall)) {
                log.error("[SOAK] quick_report done. trigger={} overall={} exitCode={} outHash={} outLength={}",
                        trigger, bundle.summary.overallStatus, bundle.summary.exitCode, outHash, outLength);
            } else if ("WARN".equalsIgnoreCase(overall)) {
                log.warn("[SOAK] quick_report done. trigger={} overall={} exitCode={} outHash={} outLength={}",
                        trigger, bundle.summary.overallStatus, bundle.summary.exitCode, outHash, outLength);
            } else {
                log.info("[SOAK] quick_report done. trigger={} overall={} exitCode={} outHash={} outLength={}",
                        trigger, bundle.summary.overallStatus, bundle.summary.exitCode, outHash, outLength);
            }
    ...
    private static String quickFailureClass(Throwable e) {
        ...
        return SafeRedactor.traceLabelOrFallback(e == null ? "" : e.getClass().getSimpleName(), "unknown");
    }
