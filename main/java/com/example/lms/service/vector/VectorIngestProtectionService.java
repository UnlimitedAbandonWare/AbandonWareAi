...
public class VectorIngestProtectionService {
    ...
    private static String classifyReason(Throwable t) {
        if (t == null) return "";
        String l = String.valueOf(t).toLowerCase(java.util.Locale.ROOT);
        if (l.contains("invalid vectors") || l.contains("all-zero") || l.contains("empty embedding")) {
            return "invalid-vector";
        }
        if (l.contains("timed out") || l.contains("timeout")) {
            return "timeout";
        }
        if (l.contains("connectexception") || l.contains("connection refused")) {
            return "connection-error";
        }
        if (l.contains("eofexception")) {
            return "connection-eof";
        }
        if (l.contains("503") || l.contains("502") || l.contains("504")) {
            return "provider-unavailable";
        }
        if (l.contains("pinecone") && l.contains("validation")) {
            return "pinecone-validation";
        }
        if ((l.contains("vector upsert") && l.contains("degraded"))
                || (l.contains("upsert") && l.contains("error"))) {
            return "vector-upsert-error";
        }
        return SafeRedactor.traceLabelOrFallback(t.getClass().getSimpleName(), "unknown");
    }
