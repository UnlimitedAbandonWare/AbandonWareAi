...
public class RerankerSelector {
    ...
    /** Whether the native ONNX runtime is explicitly allowed to load. */
    private final boolean onnxRuntimeEnabled;
    ...
    public RerankerSelector(Map<String, CrossEncoderReranker> rerankers,
                            ...
                            @Value("${onnx.enabled:false}") boolean onnxEnabled,
                            @Value("${abandonware.reranker.onnx.runtime-enabled:false}") boolean onnxRuntimeEnabled) {
        this.rerankers = rerankers;
        this.onnxEnabled = onnxEnabled;
        this.onnxRuntimeEnabled = onnxRuntimeEnabled;
        String b = (backend == null || backend.isBlank())
                ? null
                : backend.trim().toLowerCase(Locale.ROOT);
        this.backend = (b == null) ? "embedding-model" : b;
        log.info("[AWX2AF2][gpu][reranker] backendHash={} backendLength={} onnxEnabled={} onnxRuntimeEnabled={}",
                SafeRedactor.hashValue(this.backend), lengthOf(this.backend), this.onnxEnabled, this.onnxRuntimeEnabled);
        log.info("[AWX2AF2][gpu][onnx] runtimeEnabled={} backendHash={} backendLength={} action={}",
                this.onnxRuntimeEnabled, SafeRedactor.hashValue(this.backend), lengthOf(this.backend),
                this.onnxRuntimeEnabled ? "armed" : "skip");
    }
    ...
    public CrossEncoderReranker select() {
        ...
        log.info("BACKEND_PATH: rerankerBackendHash={} rerankerBackendLength={} onnx.enabled={}",
                SafeRedactor.hashValue(backend), lengthOf(backend), onnxEnabled);
        ...
        switch (backend) {
            case "onnx-runtime", "onnx" -> {
                if (onnxRuntimeEnabled) {
                    beanName = "onnxCrossEncoderReranker";
                } else {
                    log.warn("[AWX2AF2][gpu][reranker] backendHash={} backendLength={} requested but onnxRuntimeEnabled=false; using embedding-model",
                            SafeRedactor.hashValue(backend), lengthOf(backend));
                    beanName = "embeddingCrossEncoderReranker";
                }
            }
        ...
        if (embedding != null) {
            log.info("Reranker backendHash={} backendLength={} not found, falling back to embedding backend",
                    SafeRedactor.hashValue(backend), lengthOf(backend));
            return embedding;
        }
