...
public interface KnowledgeBaseService {
    ...
    /**
     * 특정 도메인 내의 모든 엔티티 타입 목록을 조회합니다.
     * 
     * @param domain 도메인 이름
     * @return 엔티티 타입 목록
     */
    List<String> getEntityTypes(String domain);

    /**
     * 특정 도메인과 타입에 속하는 엔티티 이름 목록을 List 형태로 반환합니다.
     * 
     * @param domain     도메인 이름
     * @param entityType 엔티티 타입
     * @return 엔티티 이름 목록
     */
    List<String> listEntities(String domain, String entityType);

    /**
     * 도메인과 엔티티 이름으로 지식 객체를 조회합니다.
     * 
     * @param domain     도메인 이름
     * @param entityName 엔티티 이름
     * @return DomainKnowledge의 Optional
     */
    Optional<com.example.lms.domain.knowledge.DomainKnowledge> find(String domain, String entityName);

    /**
     * Returns short, redaction-safe evidence snippets for RAG guard use.
     * Implementations should derive this from their typed storage contract, not reflection.
     */
    default List<String> evidenceSnippets(String domain, String subject, int max) {
        return List.of();
    }
    ...
    default String inferDomain(String userPrompt) {
        ...
        if (s.contains("학원") || s.contains("아카데미") || s.contains("academy"))
            return "EDU";
        if (s.contains("가격") || s.contains("스펙") || s.matches(".*\\b[a-z]{1,4}\\d+[a-z]*\\b.*"))
            return "PRODUCT";
        return "GENERAL";
    }
    ...
    default boolean isHeuristicallySynergetic(String domain, String subject, String partner) {
        ...
        boolean roleOk = (("healer".equals(r1) && "tank".equals(r2)) || ("tank".equals(r1) && "healer".equals(r2))) ||
        ...
        boolean elementOk = (("pyro".equals(e1) && "hydro".equals(e2)) || ("hydro".equals(e1) && "pyro".equals(e2))) ||
