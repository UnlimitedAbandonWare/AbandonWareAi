...
public class VectorStoreService {
    ...
    public void enqueue(String explicitId, String sessionId, String text, Map<String, Object> extraMeta) {
        ...
        if (documentChunkingService != null
            ...
            if (chunks != null && chunks.size() > 1) {
                for (DocumentChunkingService.Chunk chunk : chunks) {
                    ...
                    Object chunkId = chunkMeta.get(VectorMetaKeys.META_CHUNK_ID);
        ...
        Map<String, Object> meta = incomingMeta;
        ...
        if (vectorPoisonGuard != null) {
            try {
                ...
                meta.put("poison_guard_error_message", SafeRedactor.traceLabelOrFallback(ex.getMessage(), ""));
                log.warn("[VectorStore] poison guard failed; routing to quarantine. sidHash={} errorHash={} errorLength={}",
                        SafeRedactor.hashValue(String.valueOf(logicalSid)), SafeRedactor.hashValue(messageOf(ex)), messageLength(ex));
            }
        }
        ...
        if (vectorScopeGuard != null) {
            try {
                ...
                VectorStoreTraceSuppressions.trace("scope.inspectIngest", ex);
                log.debug("[VectorStore] fail-soft stage={}", "scope.inspectIngest");
        ...
        try {
            ...
            VectorStoreTraceSuppressions.trace("ingestProtection.quarantineActive", ignore);
            log.debug("[VectorStore] fail-soft stage={}", "ingestProtection.quarantineActive");
        }
        ...
        if (routedToQuarantine) {
            ...
            try {
                ...
                VectorStoreTraceSuppressions.trace("sidRotationAdvisor.recordQuarantine", ignore);
                log.debug("[VectorStore] fail-soft stage={}", "sidRotationAdvisor.recordQuarantine");
            }
        }
            ...
            if (shouldShadow) {
                ...
                if (!shadowRequireExplicitId || hasStableId) {
                    ...
                    try {
                        ...
                        log.warn("[VectorShadowDLQ] record staged failed (fail-soft). errorHash={} errorLength={}",
        ...
        if (forcedByIngestProtection && vectorQuarantineDlqService != null) {
            try {
                ...
                log.warn("[VectorDLQ] record quarantined failed (fail-soft). errorHash={} errorLength={}",
                        SafeRedactor.hashValue(messageOf(ex)), messageLength(ex));
            }
        }
        ...
        try {
            TraceStore.put("ml.vector.ingest.id", diagnosticVectorId(id));
            TraceStore.put("ml.vector.ingest.sid", diagnosticSid(sid));
            TraceStore.put("ml.vector.ingest.sidHash", DigestUtils.sha1Hex(nullSafe(sid)).substring(0, 12));
            TraceStore.put("ml.vector.ingest.traceId", SafeRedactor.hashValue(traceId0));
            TraceStore.put("ml.vector.ingest.route", routedToQuarantine ? "quarantine" : (shadowed ? "shadow" : "primary"));
            if (shadowed && shadowReason != null) TraceStore.put("ml.vector.ingest.shadow.reason", safeShadowReason);
        } catch (Throwable ignore) {
            VectorStoreTraceSuppressions.trace("ingest.trace", ignore);
            log.debug("[VectorStore] fail-soft stage={}", "ingest.trace");
        }
        ...
        if (dbg0 || routedToQuarantine || shadowed) {
            try {
                TraceLogger.emit("vector_enqueue", "vector",
                        java.util.Map.of(
                                "id", diagnosticVectorId(id),
                                "sid", diagnosticSid(sid),
                                "sidHash", DigestUtils.sha1Hex(nullSafe(sid)).substring(0, 12),
                                "route", routedToQuarantine ? "quarantine" : (shadowed ? "shadow" : "primary"),
                                "shadowReason", safeShadowReason,
                                "payloadLen", payload == null ? 0 : payload.length()
                        ));
            } catch (Throwable ignore) {
                VectorStoreTraceSuppressions.trace("ingest.traceLogger", ignore);
                log.debug("[VectorStore] fail-soft stage={}", "ingest.traceLogger");
            }
        }
        ...
        try {
            ...
            recordAudit(new IngestAuditEvent(
                    ...
                    diagnosticSid(logicalSid),
                    diagnosticSid(sid),
                    docType0,
                    diagnosticVectorId(id),
                    diagnosticVectorId(explicitId),
                    allow,
                    diagnosticLabel(poisonReason),
                    diagnosticLabel(scopeReason),
                    diagnosticLabel(meta.get(VectorMetaKeys.META_SCOPE_ANCHOR_KEY)),
                    diagnosticLabel(meta.get(VectorMetaKeys.META_SCOPE_KIND)),
                    diagnosticLabel(meta.get(VectorMetaKeys.META_SCOPE_PART_KEY)),
                    (meta.get(VectorMetaKeys.META_SCOPE_CONF) instanceof Number n ? n.doubleValue() : null)
            ));
        } catch (Exception ignore) {
            VectorStoreTraceSuppressions.trace("ingest.audit", ignore);
    ...
    public record VectorFlushOutcome(
        ...
        public VectorFlushOutcome {
            ...
            reasonCode = switch (normalizedReason) {
                case "unknown", "backoff", "empty", "complete", "store_failure" -> normalizedReason;
                default -> "unknown";
            };
        }
    }
    ...
    public synchronized VectorFlushOutcome flush() {
        ...
        try {
            for (Map.Entry<FlushGroupKey, List<Map.Entry<String, BufferEntry>>> grp : groups.entrySet()) {
                ...
                try (TraceContext ignored = TraceContext.attach(k.sessionId(), groupTraceId)) {
                    ...
                    try {
                        ...
                        TraceStore.put("ml.vector.flush.traceId", SafeRedactor.hashValue(groupTraceId));
                        TraceStore.put("ml.vector.flush.requestId", SafeRedactor.hashValue(groupRequestId));
                        ...
                        if (!"trace".equals(grouping)) {
                            ...
                            if (!reqIds.isEmpty()) {
                                TraceStore.put("ml.vector.flush.requestIds.sample", hashList(reqIds));
                            }
                            if (!traceIds.isEmpty()) {
                                TraceStore.put("ml.vector.flush.traceIds.sample", hashList(traceIds));
                            }
                        }
                    } catch (Throwable ignore) { VectorStoreTraceSuppressions.trace("flush.traceIdsSample", ignore); log.debug("[VectorStore] fail-soft stage={}", "flush.traceIdsSample"); }
                    ...
                    } catch (Throwable ignore) { VectorStoreTraceSuppressions.trace("flush.traceJsonStart", ignore); log.debug("[VectorStore] fail-soft stage={}", "flush.traceJsonStart"); }
                    ...
                    try {
                        for (int i = 0; i < items.size(); i += batchSize) {
                            ...
                            if (k.debug()) {
                                ...
                                } catch (Throwable ignore) { VectorStoreTraceSuppressions.trace("flush.embeddingIds", ignore); log.debug("[VectorStore] fail-soft stage={}", "flush.embeddingIds"); }
                            }
                        }
                        ...
                        } catch (Throwable ignore) { VectorStoreTraceSuppressions.trace("flush.traceJsonDone", ignore); log.debug("[VectorStore] fail-soft stage={}", "flush.traceJsonDone"); }
                    } catch (Exception e) {
                        VectorStoreTraceSuppressions.trace("flush.group", e);
                        log.debug("[VectorStore] fail-soft stage={}", "flush.group");
                        ...
                        try {
                            ...
                            VectorStoreTraceSuppressions.trace("flush.ingestProtection", ignore);
                            log.debug("[VectorStore] fail-soft stage={}", "flush.ingestProtection");
                        }

                        // Breadcrumb + snapshot for post-mortem.
                        try {
                            TraceStore.put("ml.vector.flush.error", String.format("errorHash=%s errorLength=%d",
                                    SafeRedactor.hashValue(String.valueOf(e)), String.valueOf(e).length()));
                        } catch (Throwable ignore) { VectorStoreTraceSuppressions.trace("flush.errorBreadcrumb", ignore); log.debug("[VectorStore] fail-soft stage={}", "flush.errorBreadcrumb"); }
                        try {
                            if (traceSnapshotStore != null) {
                                traceSnapshotStore.captureCurrent("vector_flush_error", "SCHED", "vector.flush", null, e);
                            }
                        } catch (Throwable ignore) { VectorStoreTraceSuppressions.trace("flush.errorSnapshot", ignore); log.debug("[VectorStore] fail-soft stage={}", "flush.errorSnapshot"); }
            ...
            return new VectorFlushOutcome(true, okIds.size(), pendingSize(), "complete");
        } catch (Exception e) {
    ...
    private static boolean isGraphDbManualVectorMeta(Map<String, Object> meta, String sessionId) {
        if (sessionId != null && sessionId.startsWith("graphdb-manual:")) {
            return true;
        }
        if (meta == null || meta.isEmpty()) {
            return false;
        }
        String lane = String.valueOf(meta.getOrDefault("ingest_lane", ""));
        String docType = String.valueOf(meta.getOrDefault(VectorMetaKeys.META_DOC_TYPE, ""));
        return "graphdb_manual_learning".equals(lane)
                || "GRAPHDB_MANUAL_LEARNING".equalsIgnoreCase(docType);
    }
    ...
    private static Object sanitizeMetaValue(Object v) {
        ...
        if (v instanceof String || v instanceof java.util.UUID
                || v instanceof Integer || v instanceof Long) return v;
    ...
    private static List<String> hashList(java.util.Collection<String> values) {
    ...
    private static String diagnosticSid(String sid) {
        String s = nullSafe(sid).trim();
        if (s.isBlank()) {
            return "";
        }
        if (isAlreadyHashedLmsValue(s)) {
            return s;
        }
        if ("lms:admin".equalsIgnoreCase(s)) {
            return "lms:admin";
        }
        if (s.startsWith("lms:")) {
            String kind = s.split(":", 3).length >= 2 ? s.split(":", 3)[1] : "private";
            return "lms:" + kind + ":hash:" + DigestUtils.sha1Hex(s).substring(0, 12);
        }
        return SafeRedactor.traceLabelOrFallback(s, "");
    }

    private static String diagnosticVectorId(String id) {
        String value = nullSafe(id).trim();
        if (value.isBlank()) {
            return "";
        }
        if (isAlreadyHashedLmsValue(value)) {
            return value;
        }
        if (value.startsWith("lms:")) {
            String[] parts = value.split(":", 3);
            String kind = parts.length >= 2 && !parts[1].isBlank() ? parts[1] : "private";
            return "lms:" + kind + ":hash:" + DigestUtils.sha1Hex(value).substring(0, 12);
        }
        return SafeRedactor.traceLabelOrFallback(value, "");
    }

    private static String diagnosticLabel(Object value) {
        return SafeRedactor.traceLabelOrFallback(value, "");
    }

    private static boolean isAlreadyHashedLmsValue(String value) {
        String s = nullSafe(value).trim();
        if (!s.startsWith("lms:")) {
            return false;
        }
        ...
        return parts.length == 4
                && !parts[1].isBlank()
                && "hash".equalsIgnoreCase(parts[2])
                && parts[3].matches("[A-Za-z0-9_-]{8,}");
    }
    ...
    private static Long tryParseVflushBucket(String traceId) {
        ...
        try {
            ...
            VectorStoreTraceSuppressions.trace("vflushBucket.parse", ignore);
            log.debug("[VectorStore] fail-soft stage={}", "vflushBucket.parse");
            return null;
        }
    }
    ...
    private static int safeInt(Object v, int def) {
        ...
        try {
            ...
            VectorStoreTraceSuppressions.trace("safeInt.parse", ignore);
            log.debug("[VectorStore] fail-soft stage={}", "safeInt.parse");
            return def;
        }
    }
