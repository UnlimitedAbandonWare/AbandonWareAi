...
public class VectorQualityGuard {
    ...
    public <T extends TextSegment> List<EmbeddingMatch<T>> filterMatches(List<EmbeddingMatch<T>> matches, String queryText, String stage) {
        ...
        if (dropped > 0) {
            log.debug("[VectorQualityGuard] stage={} dropped={} kept={} queryHash12={} queryLength={}",
                    safe(stage), dropped, out.size(), SafeRedactor.hash12(queryText),
    ...
    private static int safeInt(Object v, int def) {
        ...
        if (v instanceof Number n) {
            if (!Double.isFinite(n.doubleValue())) {
                log.debug("[VectorQualityGuard] fail-soft stage={} errorType={}", "safeInt.parse", "invalid_number");
                return def;
            }
            return n.intValue();
        }
    ...
    private static double safeDouble(Object v, double def) {
        ...
        if (v instanceof Number n) {
            ...
            if (!Double.isFinite(numeric)) {
                log.debug("[VectorQualityGuard] fail-soft stage={} errorType={}", "safeDouble.parse", "invalid_number");
                return def;
            }
            return numeric;
        }
