...
public class MemoryWriteInterceptor {
    ...
    public void save(String sessionKey, String query, String answer, double score) {
        try {
            ...
            log.debug("[AWX][rag][handler] memoryWrite reinforce failed failureReason={} errorType={} sessionHash={}",
                    "memory-write-reinforce-error",
                    SafeRedactor.traceLabelOrFallback(t.getClass().getSimpleName(), "unknown"),
                    SafeRedactor.hash12(String.valueOf(sessionKey)));
        }
    }
}
