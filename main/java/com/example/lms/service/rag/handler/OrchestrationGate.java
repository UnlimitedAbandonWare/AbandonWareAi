...
public final class OrchestrationGate {
    ...
    private boolean isCoolingDown(String canonicalSource) {
        try {
            ...
            TraceStore.put("retrieval.handler.suppressed.orchestration.cooldown", true);
            RetrievalHandlerTraceSuppressions.trace("orchestration.cooldown", e);
            return false;
        }
    }
    ...
    private static Map<String, Object> toMap(Object meta) {
        ...
        if (meta instanceof dev.langchain4j.rag.query.Metadata m) {
            ...
            try {
                ...
                TraceStore.put("retrieval.handler.suppressed.orchestration.metadata", true);
                RetrievalHandlerTraceSuppressions.trace("orchestration.metadata", e);
            }
            ...
            try {
                ...
                TraceStore.put("retrieval.handler.suppressed.orchestration.sessionId", true);
                RetrievalHandlerTraceSuppressions.trace("orchestration.sessionId", e);
                sid = null;
            }
            if (sid != null) {
