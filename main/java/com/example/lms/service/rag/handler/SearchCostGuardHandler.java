...
public class SearchCostGuardHandler extends AbstractRetrievalHandler {
    private static final Logger log = LoggerFactory.getLogger(SearchCostGuardHandler.class);
    ...
    protected boolean doHandle(Query q, List<Content> acc) {
        String text = q == null ? null : q.text();
        int est = 0;
        try {
            est = tokenEstimator.applyAsInt(text);
        } catch (Exception estimateEx) {
            TraceStore.put("retrieval.handler.suppressed.costGuard.estimate", true);
            RetrievalHandlerTraceSuppressions.trace("costGuard.estimate", estimateEx);
            traceCostGuardFailure("estimate", text, estimateEx);
            est = 0;
        }
        if (est >= thresholdTokens) {
            try {
                onRelief.accept("[SearchCostGuard] token ≥ threshold → use relief model for generation stage");
            } catch (Exception reliefEx) {
                TraceStore.put("retrieval.handler.suppressed.costGuard.relief", true);
                RetrievalHandlerTraceSuppressions.trace("costGuard.relief", reliefEx);
                traceCostGuardFailure("relief", text, reliefEx);
            }
        }
    ...
    private static void traceCostGuardFailure(String stage, String text, Throwable error) {
        String safeStage = SafeRedactor.traceLabelOrFallback(stage, "unknown");
        String safeReason = error == null
                ? "unknown"
                : "cost-guard-failure";
        TraceStore.inc("retrieval.costGuard." + safeStage + ".failed");
        TraceStore.putIfAbsent("retrieval.costGuard.failed", true);
        TraceStore.putIfAbsent("retrieval.costGuard.failureStage", safeStage);
        TraceStore.putIfAbsent("retrieval.costGuard.failureReason", safeReason);
        TraceStore.putIfAbsent("retrieval.costGuard.queryHash", SafeRedactor.hash12(text));
        TraceStore.putIfAbsent("retrieval.costGuard.queryLength", text == null ? 0 : text.length());
    }
}
