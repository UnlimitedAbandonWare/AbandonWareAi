package com.example.lms.service.rag.handler;

import com.example.lms.service.rag.WebSearchRetriever;
import com.example.lms.trace.SafeRedactor;
import dev.langchain4j.rag.content.Content;
import dev.langchain4j.rag.query.Query;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import java.util.List;

public class WebHandler extends AbstractRetrievalHandler {

    private static final Logger log = LoggerFactory.getLogger(WebHandler.class);

    private final WebSearchRetriever retriever;
    private final OrchestrationGate gate;

    public WebHandler(WebSearchRetriever retriever, OrchestrationGate gate) {
        this.retriever = retriever;
        this.gate = gate;
    }

    @Override
    protected boolean doHandle(Query q, List<Content> acc) {
        try {
            if (gate != null && !gate.allowWeb(q)) {
                log.debug("[Web] skipped by orchestration gate");
                return true;
            }

            acc.addAll(retriever.retrieve(q));
            return true;
        } catch (Exception e) {
            log.warn("[AWX][rag][handler] web failed failureReason={} errorType={} queryHash12={} queryLength={}",
                    "web-handler-error",
                    SafeRedactor.traceLabelOrFallback(e.getClass().getSimpleName(), "unknown"),
                    SafeRedactor.hash12(q == null ? null : q.text()),
                    q == null || q.text() == null ? 0 : q.text().length());
            return true;
        }
    }
}
