...
public class AmbiguityGuardHandler extends AbstractRetrievalHandler {
    ...
    protected boolean doHandle(Query query, List<Content> accumulator) {
        ...
        try {
            ...
            traceSuppressed("disambiguator", ex);
    ...
    private static void traceSuppressed(String stage, Exception ex) {
        ...
        TraceStore.put("rag.ambiguityGuard.suppressed." + safeStage, true);
        TraceStore.put("rag.ambiguityGuard." + safeStage + ".errorType", errorType);
        log.debug("[AmbiguityGuardHandler] fail-soft stage={} err={}", safeStage, errorType);
    }
}
