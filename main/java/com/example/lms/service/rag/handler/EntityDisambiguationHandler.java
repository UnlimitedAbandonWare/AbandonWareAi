...
public class EntityDisambiguationHandler {
    ...
    public Query disambiguate(Query query, List<String> history) {
        ...
        try {
            ...
            log.warn("[EntityDisambiguation] Query analysis failed, fallback to legacy. errorHash={} errorLength={}",
                    SafeRedactor.hashValue(messageOf(e)), messageLength(e));
        }

        // 2. 엔티티 쿼리로 확신이 있는 경우 → LLM 기반 재작성 우선 적용
        if (analysis != null && analysis.isEntityQuery()
                && analysis.riskAdjustedConfidence() >= entityConfidenceThreshold) {
            String expanded = buildExpandedQuery(analysis);
            log.info("[EntityDisambiguation] Entity query rewrite: originalHash12={} originalLength={} expandedHash12={} expandedLength={} (domainHash={}, contextHintCount={}, contextHintHash={}, noiseDomainCount={}, noiseDomainHash={}, rawConfidence={}, riskAdjustedConfidence={}, tier={})",
                    SafeRedactor.hash12(originalText),
                    originalText == null ? 0 : originalText.length(),
                    SafeRedactor.hash12(expanded),
                    expanded == null ? 0 : expanded.length(),
                    SafeRedactor.hash12(analysis.expectedDomain()),
                    analysis.contextHints().size(),
                    SafeRedactor.hash12(String.join("|", analysis.contextHints())),
                    analysis.noiseDomains().size(),
                    SafeRedactor.hash12(String.join("|", analysis.noiseDomains())),
                    analysis.confidenceScore(),
                    analysis.riskAdjustedConfidence(),
                    analysis.resourceTier());
            try {
                ...
                merged.put("originalQueryHash12", SafeRedactor.hash12(originalText));
                merged.put("originalQueryLength", originalText == null ? 0 : originalText.length());
                merged.put("isEntityQuery", Boolean.TRUE);
                merged.put("expectedDomainHash12", SafeRedactor.hash12(analysis.expectedDomain()));
                merged.put("noiseDomainCount", analysis.noiseDomains().size());
                merged.put("noiseDomainHash12", SafeRedactor.hash12(String.join("|", analysis.noiseDomains())));
                merged.put("contextHintCount", analysis.contextHints().size());
                merged.put("contextHintHash12", SafeRedactor.hash12(String.join("|", analysis.contextHints())));
                merged.put("entityConfidence", analysis.riskAdjustedConfidence());
                merged.put("entityConfidenceRaw", analysis.confidenceScore());
                merged.put("entityConfidenceRiskAdjusted", analysis.riskAdjustedConfidence());
                merged.put("resourceTier", analysis.resourceTier());

                // Values should be strings for LC4J Metadata to be safe across versions.
                return QueryUtils.buildQuery(expanded, MetadataSafe.asStrings(merged));
            } catch (Exception e) {
                log.debug("[EntityDisambiguation] rebuild failed, fallback to legacy disambiguation. errorHash={} errorLength={}",
                        SafeRedactor.hashValue(messageOf(e)), messageLength(e));
            }
        }
        ...
        try {
            ...
            if (confidence != null) {
                try {
                    ...
                    RetrievalHandlerTraceSuppressions.traceSuppressed(log, "EntityDisambiguationHandler", "confidence.parse", nfe);
            ...
            if (rewritten != null && !rewritten.isBlank() && goodConfidence) {
                return QueryUtils.rebuild(query, rewritten);
            }
        } catch (Exception e) {
            log.debug("[EntityDisambiguation] legacy disambiguate failed. errorHash={} errorLength={}",
                    SafeRedactor.hashValue(messageOf(e)), messageLength(e));
        }
    ...
    private static java.util.Map<String, Object> metaToMap(Object meta) {
        ...
        return QueryUtils.metadata(meta);
    }
