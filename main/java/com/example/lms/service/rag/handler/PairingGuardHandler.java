...
public class PairingGuardHandler extends AbstractRetrievalHandler {
    ...
    private final RoutingPlanService planner;
    ...
    protected boolean doHandle(Query q, List<Content> acc) {
        ...
        log.debug("[PairingGuard] intent=PAIRING, domainHash={} domainLength={} subjectHash={} subjectLength={} anchoredCount={}",
                SafeRedactor.hashValue(domain), lengthOf(domain),
                SafeRedactor.hashValue(subject), lengthOf(subject),
                anchored == null ? 0 : anchored.size());
