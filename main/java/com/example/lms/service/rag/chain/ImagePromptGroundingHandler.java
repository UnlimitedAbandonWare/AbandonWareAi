...
public class ImagePromptGroundingHandler implements ChainLink {
    ...
    public ImagePromptGroundingHandler(GroundedImagePromptBuilder groundedImagePromptBuilder) {
        this.groundedImagePromptBuilder = groundedImagePromptBuilder;
    }
    ...
    public ChainOutcome handle(ChainContext ctx, Chain next) {
        try {
            ...
            try {
                ...
                traceSuppressed("groundPrompt", e);
            ...
            try {
                ...
                // ignore
                traceSuppressed("groundedPromptHash", ignore);
            }
            return next.proceed(ctx);
        } catch (Exception e) {
            traceSuppressed("handler", e);
            log.warn("[AWX][rag][chain] imagePromptGrounding failed failureReason={} errorType={} sessionHash={}",
                    "image-prompt-grounding-handler-error",
    ...
    private static void traceSuppressed(String stage, Exception ex) {
        ...
        TraceStore.put("rag.imagePromptGrounding.suppressed." + safeStage, true);
        TraceStore.put("rag.imagePromptGrounding." + safeStage + ".errorType", errorType);
        if ("groundPrompt".equals(safeStage)) {
            log.debug("[ImagePromptGroundingHandler] fail-soft stage={}", "groundPrompt");
        } else if ("groundedPromptHash".equals(safeStage)) {
            log.debug("[ImagePromptGroundingHandler] fail-soft stage={}", "groundedPromptHash");
        } else {
            log.debug("[ImagePromptGroundingHandler] fail-soft stage={}", safeStage);
        }
        log.debug("[ImagePromptGroundingHandler] fail-soft errorType={}", errorType);
    }
}
