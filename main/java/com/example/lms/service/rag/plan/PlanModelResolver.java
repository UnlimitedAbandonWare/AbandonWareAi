    ...
    public String resolveRequestedModel(String planModel) {
        ...
        if (key.startsWith("llmrouter.")) {
            ...
            return switch (tier) {
                ...
                case "judge", "critic" -> firstNonBlank(
                        env.getProperty("llm.judge.model"),
                        env.getProperty("llm.high.model"),
                        env.getProperty("llm.chat-model"),
                        raw
                );
                case "coder", "code" -> firstNonBlank(
                        env.getProperty("llm.coder.model"),
                        raw
                );
                case "vision", "vl" -> firstNonBlank(
                        env.getProperty("llm.vision.model"),
                        env.getProperty("llm.fast.model"),
                        raw
                );
                default -> raw;
