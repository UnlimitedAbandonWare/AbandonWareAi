...
public class CfvmKallocLearningAspect {
    ...
    public Object aroundHandle(ProceedingJoinPoint pjp) throws Throwable {
        ...
        try {
            ...
            log.debug("[CFVM] sid lookup skipped stage=mdc_sid errorHash={} errorLength={}",
                    SafeRedactor.hashValue(messageOf(ignored)), messageLength(ignored));
        }
            ...
            if (keyObj != null && armObj != null) {
                ...
                if (failurePatterns != null) {
                    try {
                        ...
                        log.debug("[CFVM] fail-soft stage=failurePatterns.recent err=silent-failure"); failures = 0;
                    }
                }
                ...
                try {
                    ...
                    log.debug("[CFVM] feedback failed-soft. errorHash={} errorLength={}",
    ...
    private double computeReward(int deltaDocs, long elapsedMs, int failureCount) {
        ...
        double validationSampleScore = clamp01(readDouble("learning.validation.sampleScore", 0.0));
        double contaminationScore = clamp01(readDouble("learning.validation.contaminationScore", 0.0));
        boolean requeryRequired = readBoolean("learning.validation.requeryRequired", false);
        boolean requeryConfirmed = readBoolean("learning.validation.requeryConfirmed", !requeryRequired);
        double validationBonus = 0.10d * validationSampleScore;
        double contaminationPenalty = 0.12d * contaminationScore;
        double requeryPenalty = requeryRequired && !requeryConfirmed ? 0.08d : 0.0d;
        ...
        r += validationBonus;
        r -= contaminationPenalty;
        r -= requeryPenalty;
        TraceStore.put("cfvm.reward.validationBonus", validationBonus);
        TraceStore.put("cfvm.reward.contaminationPenalty", contaminationPenalty);
    ...
    private static double readDouble(String key, double def) {
        ...
        try {
            ...
            TraceStore.put("cfvm.kalloc.learning.suppressed.readDouble", true);
            TraceStore.put("cfvm.kalloc.learning.suppressed.readDouble.errorType", "invalid_number");
            return def;
        }
    }
