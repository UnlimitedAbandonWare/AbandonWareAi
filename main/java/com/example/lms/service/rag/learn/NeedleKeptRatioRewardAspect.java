    ...
    private static double safeDouble(Object v, double def) {
        ...
        if (v instanceof Number n) {
            ...
            if (!Double.isFinite(parsed)) {
                TraceStore.put("cfvm.reward.suppressed.safeDouble", true);
                TraceStore.put("cfvm.reward.suppressed.safeDouble.errorType", "invalid_number");
                return def;
            }
            return parsed;
        }
        if (v instanceof String s) {
            try { double parsed = Double.parseDouble(s); if (!Double.isFinite(parsed)) { throw new NumberFormatException("non-finite"); } return parsed;
