...
public class ContextConsistencyFilter {
    ...
    public List<Doc> filter(List<Doc> pool, String expectedDomain, List<String> noiseDomains) {
        ...
        final String expectedDomainHash = SafeRedactor.hashValue(expectedDomain);
        final String noiseDomainHash = SafeRedactor.hashValue(String.join("|", loweredNoise));
        final int noiseDomainCount = loweredNoise.size();
        ...
        return pool.stream()
                .filter(doc -> {
                    ...
                    if (isNoise) {
                        log.debug("[ConsistencyFilter] Dropping noisy doc (expectedDomainHash={}, noiseDomainCount={}, noiseDomainHash={}): titleHash={} titleLength={}",
                                expectedDomainHash, noiseDomainCount, noiseDomainHash, SafeRedactor.hashValue(title), title.length());
                    }
                    return !isNoise;
                })
                .collect(Collectors.toList());
