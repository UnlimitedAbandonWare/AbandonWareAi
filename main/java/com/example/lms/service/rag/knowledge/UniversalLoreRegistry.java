        ...
        try {
            ...
            for (Resource resource : resources) {
                try (InputStream inputStream = resource.getInputStream()) {
                    ...
                    log.warn("[LoreRegistry] lore catalog load failed catalogHash={} catalogNameLength={} errorHash={} errorLength={}",
                            SafeRedactor.hashValue(resource.getFilename()),
                            resource.getFilename() == null ? 0 : resource.getFilename().length(),
                            SafeRedactor.hashValue(messageOf(e)), messageLength(e));
                }
            }
            log.info("[LoreRegistry] Total lore keywords: {}", keywordMap.size());
        } catch (Exception e) {
            log.error("[LoreRegistry] lore catalog scan failed. errorHash={} errorLength={}",
                    SafeRedactor.hashValue(messageOf(e)), messageLength(e));
        }
    }
