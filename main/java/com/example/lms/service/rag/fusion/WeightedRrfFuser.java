...
import com.example.lms.service.rag.calib.RecencyBoostCalibrator;
...
public class WeightedRrfFuser<T extends WeightedRrfFuser.Result> {
    private static final Logger log = LoggerFactory.getLogger(WeightedRrfFuser.class);
    private static final RecencyBoostCalibrator RECENCY_CALIBRATOR = new RecencyBoostCalibrator();
    ...
    public static double weight(Locale locale, Result r) {
        ...
        // Apply additional time-aware calibration (RecencyBoostCalibrator)
        if (RECENCY_CALIBRATOR != null && r.publishedAt != null) {
            LocalDate docDate = LocalDate.ofInstant(r.publishedAt, ZoneId.of("Asia/Seoul"));
            recencyScore = RECENCY_CALIBRATOR.calibrate(recencyScore, docDate, r.snippet);
        }
        // [PATCH] Weights are now configurable via system properties:
        // rag.fusion.weight.locale, rag.fusion.weight.language,
        // rag.fusion.weight.authority, rag.fusion.weight.recency
        double localeWeight = getWeightOrDefault("rag.fusion.weight.locale", 0.40);
        double languageWeight = getWeightOrDefault("rag.fusion.weight.language", 0.25);
        double authorityWeight = getWeightOrDefault("rag.fusion.weight.authority", 0.20);
        double recencyWeight = getWeightOrDefault("rag.fusion.weight.recency", 0.15);
        return localeWeight * localeScore
    ...
    /**
     * System property 기반 weight 조회 헬퍼.
     * 지정된 키에 대한 double 값을 파싱하고, 실패 시 기본값을 반환한다.
     */
    private static double getWeightOrDefault(String key, double defaultValue) {
        try {
            ...
            if (!Double.isFinite(v)) {
                log.debug("[WeightedRrfFuser] fail-soft stage={} errorType={}",
                        "systemPropertyWeight", "invalid_number");
            }
            return defaultValue;
