...
public class ElementConstraintScorer {
    ...
    private String extractText(Content c) {
        ...
        try {
            ...
            traceTextSegmentMethodSuppressed(e);
            // 구버전 Content 구현에서는 textSegment() 가 없을 수 있음
        } catch (Exception e) {
            traceTextExtractSuppressed(e);
            // 방어적인 예외 처리
        }
        return c.toString();
    }

    private static void traceTextSegmentMethodSuppressed(Throwable e) {
        String errorType = SafeRedactor.traceLabelOrFallback(e.getClass().getSimpleName(), "unknown");
        TraceStore.put("rerank.elementConstraint.suppressed.stage", "textSegmentMethod");
        TraceStore.put("rerank.elementConstraint.suppressed.errorType", errorType);
        TraceStore.put("rerank.elementConstraint.suppressed.textSegmentMethod", true);
        TraceStore.put("rerank.elementConstraint.suppressed.textSegmentMethod.errorType", errorType);
    }

    private static void traceTextExtractSuppressed(Throwable e) {
        String errorType = SafeRedactor.traceLabelOrFallback(e.getClass().getSimpleName(), "unknown");
        TraceStore.put("rerank.elementConstraint.suppressed.stage", "textExtract");
        TraceStore.put("rerank.elementConstraint.suppressed.errorType", errorType);
        TraceStore.put("rerank.elementConstraint.suppressed.textExtract", true);
        TraceStore.put("rerank.elementConstraint.suppressed.textExtract.errorType", errorType);
    }
