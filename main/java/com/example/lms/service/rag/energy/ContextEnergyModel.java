...
public class ContextEnergyModel {
    ...
    private static final System.Logger LOG = System.getLogger(ContextEnergyModel.class.getName());
    private static final Logger log = LoggerFactory.getLogger(ContextEnergyModel.class);
    ...
    private static double recencyBonus(String t) {
        ...
        while (m.find()) {
            try {
                ...
                log.debug("[ContextEnergyModel] fail-soft stage={}", "recencyYear.parse");
                traceEnergySkipped("recency_year_parse", ex);
            }
        }
        if (max < 0) return 0.0;
    ...
    private static void traceEnergySkipped(String stage, Throwable error) {
        LOG.log(System.Logger.Level.DEBUG,
                "Context energy model skipped stage=" + stage + " errorType=" + errorType(error));
    }
