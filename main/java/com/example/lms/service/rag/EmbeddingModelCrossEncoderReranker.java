...
public class EmbeddingModelCrossEncoderReranker implements CrossEncoderReranker {
        ...
        try {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (log.isTraceEnabled()) {
                    log.trace("[Rerank] urlHash={} cred={} decay={}", SafeRedactor.hashValue(url), cred, authorityDecayMultiplier);
                }
            ...
            traceSuppressed("rerank", e);
            log.warn("[AWX][rag][rerank] embedding fallback failureReason={} errorType={} queryHash12={} queryLength={}",
                    "embedding-rerank-error", SafeRedactor.traceLabelOrFallback(e.getClass().getSimpleName(), "unknown"), SafeRedactor.hash12(query), query == null ? 0 : query.length());
    ...
    public List<Content> rerank(String query,
        ...
        try {
            ...
            traceSuppressed("rules.rerank", e);
            int k = Math.max(1, Math.min(topN, candidates.size()));
            return new ArrayList<>(candidates.subList(0, k));
        }
    }
    ...
    private static void traceSuppressed(String stage, Exception ex) {
        ...
        TraceStore.put("rag.embeddingCrossEncoderReranker.suppressed." + safeStage, true);
        TraceStore.put("rag.embeddingCrossEncoderReranker." + safeStage + ".errorType", errorType);
        log.debug("[Rerank] fail-soft stage={} errorType={}", safeStage, errorType);
    }
    ...
    private static String safeUrl(Content c, TextSegment seg) {
        try {
            ...
            log.debug("[Rerank] fail-soft stage={}", "safeUrl");
            return null;
        }
    }
