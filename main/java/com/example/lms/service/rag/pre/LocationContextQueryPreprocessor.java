...
public class LocationContextQueryPreprocessor implements MetaAwareQueryContextPreprocessor {
    ...
    public String enrich(String q, Map<String, Object> meta) {
        ...
        if (boundaryOn && isWebPurpose) {
            traceRewriteSkipped(s, true, true);
            return s;
        }
    ...
    private static void traceRewriteSkipped(String query, boolean webPurpose, boolean privacyBoundary) {
        try {
            TraceStore.put("query.location.rewriteSkipped", true);
            TraceStore.put("query.location.rewriteSkipped.reason", "privacy_boundary_web_query");
            TraceStore.put("query.location.rewriteSkipped.webPurpose", webPurpose);
            TraceStore.put("query.location.rewriteSkipped.privacyBoundary", privacyBoundary);
