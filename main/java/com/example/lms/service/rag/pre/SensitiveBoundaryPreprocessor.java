...
public class SensitiveBoundaryPreprocessor implements MetaAwareQueryContextPreprocessor {
    ...
    public String enrich(String q, Map<String, Object> meta) {
        ...
        GuardContext g = GuardContextHolder.getOrDefault();
        boolean on = g.isSensitiveTopic() || g.planBool("privacy.boundary.mask-web-query", true);
        if (!on) return q;
        ...
        try {
            ...
            TraceStore.put("privacy.boundary.suppressed.stage", "metaWrite");
            TraceStore.put("privacy.boundary.suppressed.errorType", errorType);
            TraceStore.put("privacy.boundary.suppressed.metaWrite", true);
            TraceStore.put("privacy.boundary.suppressed.metaWrite.errorType", errorType);
            TraceStore.put("privacy.boundary.suppressed.metaWrite.queryHash", SafeRedactor.hash12(q));
            TraceStore.put("privacy.boundary.suppressed.metaWrite.queryLength", q == null ? 0 : q.length());
        }
