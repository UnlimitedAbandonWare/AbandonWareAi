...
public interface SseEventPublisher {
    ...
    /**
     * Emit an SSE scoped to a logical session. Implementations that cannot
     * route by session may fall back to the global event stream.
     */
    default void emit(String type, Object payload, String sessionId) {
        emit(type, payload);
    }
    ...
    // Implementations with replay support override this. The primary
    // LoggingSseEventPublisher does, while no-op fallbacks intentionally stay silent.
    default reactor.core.publisher.Flux<java.util.Map<String,Object>> asStream() {
        return reactor.core.publisher.Flux.empty();
    }

    default reactor.core.publisher.Flux<java.util.Map<String,Object>> asStream(String sessionId) {
        return asStream();
    }
}
