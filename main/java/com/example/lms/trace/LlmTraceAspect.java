...
public class LlmTraceAspect {
        ...
        return mono
                .doOnSuccess(resp -> {
                    ...
                    TraceLogger.emit("llm_resp", "llm", Map.of(
                            "finish", "stop",
                            "otok", otok,
                            "resp_hash", hashOrEmpty(resp),
                            "resp_len", lengthOf(resp),
                            "lat_ms", dt,
                            "tool_calls", 0
                    ));
                ...
                .doOnError(ex -> {
                    ...
                    TraceLogger.emit("llm_resp", "llm", Map.of(
                            "finish", "error",
                            "otok", 0,
                            "resp_hash", hashOrEmpty(ex.getMessage()),
                            "resp_len", lengthOf(ex.getMessage()),
                            "lat_ms", dt,
                            "tool_calls", 0
                    ));
                });
    ...
    public Object aroundStream(ProceedingJoinPoint pjp, Prompt prompt, GenerationParams params) throws Throwable {
        ...
        return original
                .doOnNext(tc -> {
                    String text = tc == null || tc.text() == null ? "" : tc.text();
                    ...
                    tokenCount.addAndGet(text.isEmpty() ? 0 : Math.max(1, text.length() / 4));
                })
                .doFinally(signal -> {
                    long dt = System.currentTimeMillis() - t0;
                    ...
                    TraceLogger.emit("llm_resp", "llm", Map.of(
                            "finish", finish,
                            "otok", tokenCount.get(),
                            "resp_hash", hashOrEmpty(preview.toString()),
                            "resp_len", preview.length(),
                            "lat_ms", dt,
                            "tool_calls", 0
                    ));
