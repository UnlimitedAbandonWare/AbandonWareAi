...
public class SoakProbeController {
            ...
            try {
                ...
                for (int c = 1; c <= cycles; c++) {
                    ...
                    try {
                        ...
                        log.debug("[AWX][probe][soak] sleep interrupted stage=open_wait errorType={}",
    ...
    static String blockedProbeNote(String message, Duration remaining) {
        long remainingMs = remaining == null ? 0L : Math.max(0L, remaining.toMillis());
        return "blocked: error=" + SafeRedactor.traceLabelOrFallback(message, "open")
