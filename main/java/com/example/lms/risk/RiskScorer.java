...
public class RiskScorer {
    ...
    public int shrinkTopK(int baseK, RiskDecisionIndex idx, int minK) {
        int safeBaseK = Math.max(0, baseK);
        int safeMinK = Math.max(0, minK);
        double rdi = idx == null ? 0.0 : idx.rdi;
        double g = 1.0 - 0.6*rdi; // shrink up to 60%
        int k = (int)Math.round(safeBaseK * g);
        return Math.max(safeMinK, k);
    }
}
