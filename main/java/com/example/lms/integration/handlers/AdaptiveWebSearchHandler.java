...
public class AdaptiveWebSearchHandler extends AbstractRetrievalHandler {
    ...
    protected boolean doHandle(Query q, List<Content> acc) {
        ...
        try {
            ...
            if (md != null) {
                ...
                if (v2 != null) {
                    modeStr = String.valueOf(v2).trim();
                }
            }
            ...
            traceSuppressed("metadataGate", ignore);
        }
        ...
        try {
            ...
            if (m != null) {
                String rawMode = String.valueOf(m).trim();
                if (!rawMode.isEmpty()) {
                    mode = SearchMode.valueOf(rawMode.toUpperCase(Locale.ROOT));
                }
            }
        } catch (Exception ignore) {
        ...
        try {
            ...
            for (WebSearchProvider p : resolveProviders(provs)) {
                ...
                try {
                    ...
                    log.debug("[AdaptiveWeb] provider failed provider={} errorHash={} errorLength={}",
                            safeProviderId(p), SafeRedactor.hashValue(messageOf(e)), messageLength(e));
                }
            }
            // Meta hints (안전 파서)
            ...
            try {
                ...
                traceSuppressed("domainProfile", ignore);
            }
            ...
            try {
                ...
                traceSuppressed("financeFilter", ignore);
            }
                    ...
                    try {
                        ...
                        if (minRel > 0.0) {
                            ...
                            try {
                                ...
                                log.debug("[AdaptiveWeb] relatedness failed queryHash={} queryLength={} errorHash={} errorLength={}",
                                        SafeRedactor.hashValue(queryText), queryText == null ? 0 : queryText.length(),
                                        SafeRedactor.hashValue(messageOf(ignore2)), messageLength(ignore2));
                            }
                        ...
                        traceSuppressed("precisionFetch", ex);
                    }
                }
                if (big.length() > 0) {
            ...
            for (int i = 0; i < docs.size(); i++) {
                ...
                if (_snip == null || _snip.isBlank()) {
                    try {
                        ...
                        /* fail-soft */
                        traceSuppressed("snippetFetch", ignore);
                    }
                    if (_snip == null) _snip = "";
                }
            ...
            log.warn("[AdaptiveWeb] search failed. errorHash={} errorLength={}",
    ...
    private List<WebSearchProvider> resolveProviders(List<ProviderId> desired) {
        ...
        for (WebSearchProvider p : providers) {
            ...
            try {
                ...
                traceSuppressed("providerProbe", ignore);
            }
        }
        if (desired == null || desired.isEmpty()) {
            traceProviderSelection(java.util.Collections.emptyList(), registered, registered, java.util.Collections.emptyList());
            return providers;
        }
        ...
        for (ProviderId id : desired) {
            ...
            resolved.add(id.name());
            out.add(p);
        }
        traceProviderSelection(requested, registered, resolved, missing);
        return out;
    }

    private static void traceProviderSelection(List<String> requested,
                                               List<String> registered,
                                               List<String> resolved,
                                               List<String> missing) {
        try {
            TraceStore.put("web.adaptive.requestedProviders", List.copyOf(requested));
            TraceStore.put("web.adaptive.registeredProviders", List.copyOf(registered));
            TraceStore.put("web.adaptive.resolvedProviders", List.copyOf(resolved));
            TraceStore.put("web.adaptive.missingProviders", List.copyOf(missing));
            TraceStore.put("web.adaptive.requestedProviderCount", requested.size());
            TraceStore.put("web.adaptive.registeredProviderCount", registered.size());
            TraceStore.put("web.adaptive.resolvedProviderCount", resolved.size());
            TraceStore.put("web.adaptive.missingProviderCount", missing.size());
        } catch (Exception ignore) {
            // tracing is best-effort
            traceSuppressed("providerTrace", ignore);
        }
    }
    ...
    private static Map<String, Object> toMetaMap(Query q) {
        if (q == null) return java.util.Collections.emptyMap();
        return QueryUtils.metadata(q);
    }

    private static String safeProviderId(WebSearchProvider provider) {
        ProviderId providerId = provider == null ? null : provider.id();
        return providerId == null
                ? "unknown"
                : SafeRedactor.traceLabelOrFallback(providerId.name(), "unknown");
    }
    ...
    private static void traceSuppressed(String stage, Throwable failure) {
        ...
        String errorType = "metaInt".equals(safeStage)
                ? "invalid_number"
                : failure == null
                ? "unknown"
                : SafeRedactor.traceLabelOrFallback(failure.getClass().getSimpleName(), "unknown");
        TraceStore.put("web.adaptive.suppressed.stage", safeStage);
        TraceStore.put("web.adaptive.suppressed.errorType", errorType);
        TraceStore.put("web.adaptive.suppressed." + safeStage, true);
        TraceStore.put("web.adaptive.suppressed." + safeStage + ".errorType", errorType);
    }
}
