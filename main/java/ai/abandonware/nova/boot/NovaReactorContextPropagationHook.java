...
public class NovaReactorContextPropagationHook implements InitializingBean, DisposableBean {
    ...
    public void afterPropertiesSet() {
        ...
        Schedulers.onScheduleHook(HOOK_KEY, runnable -> {
            try {
                ...
                traceSuppressed("scheduleHook.contextProbe", contextError);
        ...
        try {
            debugEventStore.emit(DebugProbeType.CONTEXT_PROPAGATION, DebugEventLevel.INFO,
                    ...
                    null);
        } catch (Throwable debugEventError) {
            traceSuppressed("scheduleHook.debugEvent", debugEventError);
        }
    }
    ...
    public void destroy() {
        try {
            ...
            log.debug("[Nova] Reactor schedule hook reset failed (key={}) errorHash={} errorLength={}",
                    HOOK_KEY, SafeRedactor.hashValue(messageOf(t)), messageLength(t));
        }
    }
    ...
    private static void traceSuppressed(String stage, Throwable error) {
        try {
            TraceStore.inc("reactor.contextPropagation.suppressed.count");
            TraceStore.put("reactor.contextPropagation.suppressed.stage",
                    SafeRedactor.traceLabelOrFallback(stage, "unknown"));
            TraceStore.put("reactor.contextPropagation.suppressed.errorType", errorType(error));
        } catch (RuntimeException traceError) {
            log.debug("[Nova] Reactor context suppression trace failed: errorHash={} errorLength={}",
                    SafeRedactor.hashValue(messageOf(traceError)), messageLength(traceError));
        }
    }
