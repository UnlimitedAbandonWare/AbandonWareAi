...
public class UawIdleAutoTrainingPipelineAspect {
    ...
    public Object aroundAsk(ProceedingJoinPoint pjp) throws Throwable {
        ...
        try {
            ...
            traceSuppressed("guardContext.previous");
        }
        ...
        try {
            ...
            traceSuppressed("guardContext.seed");
        }
        ...
        try {
            ...
            try {
                ...
                traceSuppressed("augmenter.canonicalQuery");
            }
            ...
            try {
                TraceStore.put("uaw.pipeline.domainProfile", SafeRedactor.hashValue(domainProfile));
            } catch (Throwable ignore) {
                traceSuppressed("trace.domainProfile");
            }
            ...
            if (nightmareBreaker != null) {
                ...
                try {
                    ...
                    traceSuppressed("breaker.websearch");
                }
            }
            ...
            try {
                ...
                traceSuppressed("trace.webProviders");
            }
            ...
            try {
                ...
                traceSuppressed("breaker.retrieval");
            }
            ...
            try {
                ...
                traceSuppressed("guardContext.degrade");
            }

            try {
                TraceStore.put("uaw.pipeline.degradeRag", degradeRag);
            } catch (Throwable ignore) {
                traceSuppressed("trace.degradeRag");
            }
            ...
            try {
                ...
                TraceStore.put("uaw.pipeline.plan.searchQueryCount", searchQueries);
                TraceStore.put("uaw.pipeline.plan.maxSources", maxSources);
            } catch (Throwable ignore) {
                traceSuppressed("trace.plan");
            }

            // Optional overrides
            final String forcedModel = autolearnStrictModel();
            ...
            try {
                ...
                steps.add("domainProfileHash=" + SafeRedactor.hashValue(domainProfile));
                steps.add("domainProfileLength=" + (domainProfile == null ? 0 : domainProfile.length()));
                steps.add("providers=" + providers);
                steps.add("searchMode=" + searchMode.name());
                steps.add("searchQueryCount=" + searchQueries);
                ...
                traceSuppressed("trace.planSteps");
            }
    ...
    private boolean isOpenPrefix(String keyPrefix) {
        ...
        try {
            ...
            traceSuppressed("breaker.prefix");
            return false;
        }
    }

    private String autolearnStrictModel() {
        String configured = trimToNull(env.getProperty("uaw.autolearn.strict.model"));
    ...
    private static SearchMode resolveSearchMode(String raw, int searchQueries) {
        if (raw != null && !raw.isBlank()) {
            try {
                ...
                traceSuppressed("searchMode.resolve");
            }
        }
    ...
    private static Double resolveTemperature(String raw) {
        ...
        try {
            ...
            traceSuppressed("temperature.resolve");
            return null;
        }
    }
    ...
    private static String summarize(Throwable t) {
        ...
        msg = SafeRedactor.safeMessage(msg, 180);
        msg = msg == null ? "" : msg.replaceAll("\\s+", " ").trim();
        String cn = t.getClass().getSimpleName();
        return msg.isBlank() ? cn : (cn + ": " + msg);
    }

    private static void traceSuppressed(String stage) {
        log.debug("[UAWPipeline] suppressed stage={}", SafeRedactor.traceLabelOrFallback(stage, "unknown"));
    }
