...
public class ProviderRateLimitBackoffAspect {
        ...
        try {
            ...
            if (isAwaitTimeout(t)) {
                ...
                backoff.recordFailure(RateLimitBackoffCoordinator.PROVIDER_NAVER,
                        ...
                        "naver_exception_await_timeout", "await-timeout");
                try {
                    TraceStore.inc("web.naver.await_timeout.exception.count");
                } catch (Throwable ignore) {
                    traceSuppressed("naver.awaitTimeoutException.count", ignore);
                }
            ...
            if (isTimeout(t)) {
                backoff.recordFailure(RateLimitBackoffCoordinator.PROVIDER_NAVER,
                        ...
                        "naver_exception_timeout", "timeout");
                RateLimitBackoffCoordinator.Decision d2 = backoff.shouldSkip(RateLimitBackoffCoordinator.PROVIDER_NAVER);
                markSkipped("naver", d2);
                return List.of();
            }
            if (isCancelled(t)) {
                markCancelledNoBreaker("naver", t);
                return List.of();
            }
            throw t;
        }
        ...
        try {
            ...
            if (isAwaitTimeout(t)) {
                ...
                try {
                    ...
                    traceSuppressed("naver.awaitTimeoutException.traceCount", ignore);
                }
                RateLimitBackoffCoordinator.Decision d2 = backoff.shouldSkip(RateLimitBackoffCoordinator.PROVIDER_NAVER);
                markSkipped("naver", d2);
                return new NaverSearchService.SearchResult(List.of(), new NaverSearchService.SearchTrace());
        ...
        try {
            ...
            if (out instanceof BraveSearchResult r) {
                ...
                if (st == BraveSearchResult.Status.HTTP_429
                    ...
                    String safeReason = SafeRedactor.traceLabelOrFallback(dr, "disabled");
                    markWebPartialDown("brave", dr);
                    ...
                    TraceStore.putIfAbsent("web.brave.skipped.reason", safeReason);
            ...
            if (isAwaitTimeout(t)) {
                backoff.recordFailure(RateLimitBackoffCoordinator.PROVIDER_BRAVE,
                        RateLimitBackoffCoordinator.FailureKind.AWAIT_TIMEOUT,
                        "brave_exception_await_timeout", "await-timeout");
            ...
            if (isTimeout(t)) {
                backoff.recordFailure(RateLimitBackoffCoordinator.PROVIDER_BRAVE,
                        RateLimitBackoffCoordinator.FailureKind.TIMEOUT,
                        "brave_exception_timeout", "timeout");
                RateLimitBackoffCoordinator.Decision d2 = backoff.shouldSkip(RateLimitBackoffCoordinator.PROVIDER_BRAVE);
                markSkipped("brave", d2);
            ...
            if (isCancelled(t)) {
                markCancelledNoBreaker("brave", t);
                return new BraveSearchResult(List.of(), BraveSearchResult.Status.EXCEPTION,
                        null, 0L, "brave cancelled", 0L);
            }
            throw t;
        }
    }
    ...
    private static boolean isAwaitTimeout(Throwable t) {
        ...
        try {
            ...
            traceSuppressed("awaitTimeout.message", ignore);
            msg = null;
        }
    ...
    private static void markSkipped(String provider, RateLimitBackoffCoordinator.Decision d) {
        try {
            ...
            traceSuppressed("markSkipped.trace", ignore);
        }
    }
    ...
    private static void markRateLimited(String provider, Long retryAfterMs, Throwable t) {
        try {
            ...
            if (t != null) {
                TraceStore.put("web." + provider + ".skipped.err", "rate-limit");
            }
            TraceStore.put("web." + provider + ".skipped.tsMs", System.currentTimeMillis());
            TraceStore.inc("web." + provider + ".skipped.count");
            ...
            if (TraceStore.putIfAbsent(onceKey, true) == null) {
                log.warn("[Nova] {} rate-limit(429) intercepted; applying backoff (retryAfterMs={}; type={}){}",
                        provider, retryAfterMs, "rate-limit", LogCorrelation.suffix());
            }
        } catch (Throwable ignore) {
            traceSuppressed("markRateLimited.trace", ignore);
        }
    }
    ...
    private static void markRateLimitedMeta(String provider, Long retryAfterMs, BraveSearchResult.Status st) {
        try {
            ...
            TraceStore.put("web.failsoft.rateLimitBackoff." + provider + ".reason", safeStr(reason));
            TraceStore.putIfAbsent("web." + provider + ".skipped.reason", "cooldown");
            TraceStore.putIfAbsent("web." + provider + ".skipped.stage", "aop_backoff");
            if (retryAfterMs != null && retryAfterMs > 0L) {
                TraceStore.put("web." + provider + ".skipped.extraMs", retryAfterMs);
            ...
            traceSuppressed("markRateLimitedMeta.trace", ignore);
        }
    }
    ...
    private static void markWebPartialDown(String provider, String reason) {
        try {
            ...
            String safeReason = safeStr(reason);
            if (hasText(safeReason)) {
                TraceStore.putIfAbsent("orch.webPartialDown.reason." + provider, safeReason);
                TraceStore.putIfAbsent("orch.webPartialDown.firstReason", safeReason);
            }
        } catch (Throwable ignore) {
            traceSuppressed("markWebPartialDown.trace", ignore);
        }
    }
    ...
    private static void markBraveDisabledReasonIfAbsent(String reason) {
        try {
            String safeReason = safeStr(reason);
            if (hasText(safeReason)) {
                TraceStore.putIfAbsent("web.await.brave.disabledReason", safeReason);
            }
        } catch (Throwable ignore) {
            traceSuppressed("markBraveDisabledReasonIfAbsent.trace", ignore);
        }
    }
    ...
    private static void markBraveDisabledReasonOverwrite(String reason) {
        try {
            String safeReason = safeStr(reason);
            if (hasText(safeReason)) {
                TraceStore.put("web.await.brave.disabledReason", safeReason);
            }
        } catch (Throwable ignore) {
            traceSuppressed("markBraveDisabledReasonOverwrite.trace", ignore);
        }
    }

    private static void markCancelledNoBreaker(String provider, Throwable t) {
        String p = provider == null || provider.isBlank()
                ? "unknown"
        ...
        try {
            ...
            traceSuppressed("markCancelledNoBreaker.trace", ignore);
        }
        if (log.isDebugEnabled()) {
            log.debug("[Nova] {} cancelled; returning fail-soft empty result without breaker backoff (type={}){}",
                    p, type, LogCorrelation.suffix());
        }
    }
        ...
        try {
            ...
            if (tgt instanceof BraveSearchService svc) {
                ...
                if (until > 0L && now < until) {
                    ...
                    TraceStore.put("web.brave.disabled.remainingMs", rem);
            ...
            traceSuppressed("resolveBraveDisabledReason.trace", ignore);
        }
    ...
    private static String extractRetryAfterHeader(Throwable t) {
        try {
            ...
            traceSuppressed("retryAfter.header", ignore);
        }
        return null;
    }
    ...
    private static String safeStr(Object o) {
        ...
        String v = SafeRedactor.traceLabel(o);
        return v == null ? "" : v;
    }
    ...
    private static void traceSuppressed(String stage, Throwable ignored) {
        ...
        TraceStore.put("web.failsoft.rateLimitBackoff.suppressed.stage", safeStage);
        TraceStore.put("web.failsoft.rateLimitBackoff.suppressed.errorType", errorType);
        TraceStore.put("web.failsoft.rateLimitBackoff.suppressed." + safeStage, true);
        TraceStore.put("web.failsoft.rateLimitBackoff.suppressed." + safeStage + ".errorType", errorType);
    }
}
