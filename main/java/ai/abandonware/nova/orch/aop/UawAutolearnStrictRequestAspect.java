...
public class UawAutolearnStrictRequestAspect {
    ...
    public Object aroundAsk(ProceedingJoinPoint pjp) throws Throwable {
        ...
        try {
            ...
            try {
                ...
                traceSuppressed("trace.autolearn");
            }
            ...
            try {
                if (nightmareBreaker != null) {
                    if (nightmareBreaker.isAnyOpenPrefix(NightmareKeys.CHAT_DRAFT)) {
                        String blockedReason = "breaker-open:" + NightmareKeys.CHAT_DRAFT;
                        TraceStore.put("uaw.strict.failClosed", true);
                        TraceStore.put("uaw.strict.failClosed.reason", blockedReason);
                        traceBlocked(blockedReason);
            ...
            try {
                ...
                traceSuppressed("guardContext.degradeFlags");
            }
            ...
            try {
                ...
                if (isHardLlmFailure(t)) {
                    ...
                    log.error("[UAWStrict] hard LLM failure in strict path; returning empty result (skip). rootHash={} rootLength={}",
    ...
    private static Double resolveTemperature(String raw) {
        ...
        try {
            ...
            traceSuppressed("temperature.parse");
            return null;
        }
    }
    ...
    private static void traceBlocked(String reason) {
        try {
            TraceStore.put("uaw.strictRequest.blocked", SafeRedactor.traceLabelOrFallback(reason, "blocked"));
        } catch (RuntimeException ignore) {
            traceSuppressed("strictRequest.blocked");
        }
    }

    private static void traceSuppressed(String stage) {
        log.debug("[UAWStrict] suppressed stage={}",
