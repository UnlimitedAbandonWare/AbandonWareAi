...
public class WebFailSoftDomainStageReportService {
            ...
            if (includeCanonicalQueries) {
                if (!hs.byCanonicalQueryDevCommunityUnverifiedSelected.isEmpty()) {
                    row.put("topCanonicalQueries",
                            topDiagnosticCounts(hs.byCanonicalQueryDevCommunityUnverifiedSelected, "canonicalQuery",
                                    "web.failsoft.domainStageReport.canonicalQuery", 3));
                }
    ...
    private Map<String, Object> suggestDevCommunityUnverified(HostStats hs) {
        ...
        if (topReason != null) {
            s.put("topReason", diagnosticReason(topReason));
        ...
        if (includeCanonicalQueries) {
            ...
            if (tq != null) {
                s.put("topCanonicalQuery",
                        SafeRedactor.diagnosticValue("web.failsoft.domainStageReport.canonicalQuery", tq.getKey(),
                                240));
    ...
    private Map<String, Object> configConflicts(int limit) {
        ...
        try {
            ...
            log.debug("[AWX][search][zero-results] domain stage report skipped stage=dev_community_deny_domains errorType={}",
    ...
    private Map<String, Object> officialDocsRescueQueries(int topN, int minCount) {
        ...
        for (QueryStats qs : topStarvation) {
            ...
            row.put("canonicalQuery", SafeRedactor.diagnosticValue(
                    "web.failsoft.domainStageReport.canonicalQuery", qs.canonicalQuery, 240));
            ...
            row.put("suggestedRescueQueryTemplates", SafeRedactor.diagnosticValue(
                    "web.failsoft.domainStageReport.suggestedRescueQuery", templates, 240));
            row.put("suggestedRescueQueryExamples", SafeRedactor.diagnosticValue(
                    "web.failsoft.domainStageReport.suggestedRescueQuery",
        ...
        out.put("topCanonicalTokens", topHashedCounts(tokenCounts, "tokenHash", 12));
    ...
    private static List<Map<String, Object>> topDiagnosticCounts(Map<String, LongAdder> m, String keyField,
            String diagnosticKey, int topN) {
        ArrayList<Map.Entry<String, LongAdder>> entries = new ArrayList<>(m.entrySet());
        entries.sort((a, b) -> Long.compare(b.getValue().sum(), a.getValue().sum()));
        if (entries.size() > topN) {
            entries.subList(topN, entries.size()).clear();
        ...
        for (Map.Entry<String, LongAdder> e : entries) {
            Map<String, Object> row = new LinkedHashMap<>();
            row.put(keyField, SafeRedactor.diagnosticValue(diagnosticKey, e.getKey(), 240));
            row.put("count", e.getValue().sum());
            out.add(row);
        }
        return out;
    ...
    private static List<Map<String, Object>> topHashedCounts(Map<String, LongAdder> m, String keyField, int topN) {
        ArrayList<Map.Entry<String, LongAdder>> entries = new ArrayList<>(m.entrySet());
        entries.sort((a, b) -> Long.compare(b.getValue().sum(), a.getValue().sum()));
        if (entries.size() > topN) {
            entries.subList(topN, entries.size()).clear();
        ...
        for (Map.Entry<String, LongAdder> e : entries) {
            Map<String, Object> row = new LinkedHashMap<>();
            row.put(keyField, SafeRedactor.hashValue(e.getKey()));
            row.put("count", e.getValue().sum());
            out.add(row);
        }
        return out;
    ...
    private static List<Map<String, Object>> topReasons(Map<String, LongAdder> m, int topN) {
        ...
        for (Map.Entry<String, LongAdder> e : entries) {
            Map<String, Object> row = new LinkedHashMap<>();
            row.put("reason", diagnosticReason(e.getKey()));
            row.put("count", e.getValue().sum());
            out.add(row);
        }
        return out;
    ...
    private static Object diagnosticReason(String reason) {
        String safe = SafeRedactor.traceLabelOrFallback(reason, "unknown");
        if (safe == null || safe.isBlank()) {
            return "";
        }
        if (safe.matches("[A-Za-z0-9_.:-]{1,120}")) {
            return safe;
        }
        return SafeRedactor.diagnosticValue("web.failsoft.domainStageReport.rawText", safe, 120);
    }
    ...
    private static boolean sysBool(String key, boolean def) {
        try {
            ...
            traceSuppressed("sysBool", e);
            return def;
        }
    }

    private static void traceSuppressed(String stage, Throwable e) {
        log.debug("[AWX][search][zero-results] domain stage report suppressed stage={} errorType={}",
                stage,
                errorType(stage, e));
    }

    private static String errorType(String stage, Throwable e) {
        if (("sysInt".equals(stage) || "sysLong".equals(stage)) && e instanceof NumberFormatException) {
