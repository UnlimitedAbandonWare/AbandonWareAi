...
public class BraveAdaptiveQpsRestTemplateInterceptor implements ClientHttpRequestInterceptor {
    ...
    private void onResponse(ClientHttpResponse resp) throws IOException {
        ...
        try {
            ...
            traceSuppressed("response.status", e);
            status = null;
        }
        ...
        try {
            ...
            traceSuppressed("response.headers", e);
            return;
        }
        if (h == null) {
            return;
        ...
        if (props.isTraceHeaders()) {
            try {
                ...
                traceSuppressed("ratelimit.headers.trace");
            }
        }
        ...
        try {
            ...
            traceSuppressed("ratelimit.reset.trace", e);
            // fail-soft
        }
        ...
        if (is429) {
            ...
            if (baseMs > 0L) {
                ...
                try {
                    ...
                    traceSuppressed("cooldown.http429.trace");
                }
            }
        ...
        if (!monthlyUnlimited && monthLimit > 0L && monthRemainingVal == 0L) {
            ...
            try {
                ...
                traceSuppressed("quota.resetAt", e);
                // fail-soft
            }
        ...
        if (rateLimiter != null && !monthlyUnlimited && monthLimit > 0L && localMonthRemainingVal > 0L
                && monthResetSec > 0L) {
            long threshold = Math.max(0L, props.getMonthlyRemainingThreshold());
            if (localMonthRemainingVal <= threshold) {
                double spread = (localMonthRemainingVal / (double) monthResetSec);
            ...
            if (Math.abs(target - prev) >= 0.005d) {
                ...
                if (props.isTracePenalty() || props.isTraceHeaders()) {
                    try {
                        ...
                        traceSuppressed("effectiveQps.trace");
                    }
                }
            }
        }
    ...
    private void trySetWebRateLimited(String reason) {
        try {
            ...
            if (reason != null && !reason.isBlank()) {
                TraceStore.putIfAbsent("orch.webPartialDown.reason", safeReason(reason));
            }
        } catch (Exception ignore) {
            traceSuppressed("partialDown.trace");
        }
    }
        ...
        if (untilEpochMs > prev) {
            ...
            if (props.isTraceHeaders() || props.isTracePenalty()) {
                try {
                    ...
                    if (reason != null) {
                        TraceStore.put("brave.ratelimit.cooldownReason", safeReason(reason));
                    }
                } catch (Exception ignore) {
                    traceSuppressed("cooldown.install.trace");
                }
            }
        }
    }
    ...
    private void tryClearQuotaLatchIfAny(String why) {
        try {
            ...
            traceSuppressed("quotaLatch.clear");
        }
        ...
        try {
            ...
            if (why != null) {
                TraceStore.put("brave.ratelimit.quotaLatch.cleared.reason", safeReason(why));
            }
        } catch (Exception ignore) {
            traceSuppressed("quotaLatch.clear.trace");
        }
    }
    ...
    private void setOperationallyDisabledUntil(long untilEpochMs, String kind) {
        ...
        String disabledReason = (kind == null || kind.isBlank())
                ? "quota_exhausted"
                : safeReason(kind);

        // Request-scoped visibility (EvidenceListTraceInjectionAspect reads this key).
        try {
            TraceStore.put("web.await.brave.disabledReason", disabledReason);
        } catch (Exception ignore) {
            traceSuppressed("disabled.reason.trace");
        }

        // Also mark canonical skip keys so downstream policy can reason about it.
        try {
            TraceStore.put("web.brave.skipped", true);
            TraceStore.putIfAbsent("web.brave.skipped.reason", disabledReason);
            ...
            traceSuppressed("disabled.skip.trace");
        }

        brave.setOperationallyDisabled(disabledReason);
    }
...
private static long parseRetryAfterMs(String raw) {
    ...
    private static void traceSuppressed(String stage) {
        log.debug("[nova][brave-adaptive-qps] suppressed stage={}", safeReason(stage));
    }

    private static void traceSuppressed(String stage, Throwable e) {
        log.debug("[nova][brave-adaptive-qps] suppressed stage={} errorType={}",
                safeReason(stage),
                suppressedErrorType(stage, e));
    }

    private static String suppressedErrorType(String stage, Throwable e) {
        if (e == null) {
            return "unknown";
        }
        if (e instanceof NumberFormatException && isNumericParserStage(stage)) {
            return "invalid_number";
        }
        if (isDateParserStage(stage)) {
            return "invalid_date";
        }
        return e.getClass().getSimpleName();
    }

    private static boolean isNumericParserStage(String stage) {
        return "RatePair.toLong".equals(stage)
                || "parseRetryAfterMs.delta".equals(stage);
    }

    private static boolean isDateParserStage(String stage) {
    ...
    static final class RatePair {
        ...
        private static long toLong(String s) {
            ...
            try {
                ...
                traceSuppressed("RatePair.toLong", e);
                return -1L;
            }
        }
