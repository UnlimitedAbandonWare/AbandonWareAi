    ...
    private static long asLong(@Nullable Object v, long def) {
        ...
        if (v instanceof Number n) {
            ...
            if (!Double.isFinite(numeric)) {
                traceSuppressed("asLong", new NumberFormatException("non-finite number"));
                return def;
            }
            return n.longValue();
        }
        try {
            ...
            traceSuppressed("asLong", e);
            return def;
        }
    }

    private static void traceSuppressed(String stage, Throwable e) {
        MDC.put("trace.canonicalizer.suppressed.stage", stage);
        MDC.put("trace.canonicalizer.suppressed.errorType",
                "asLong".equals(stage) ? "invalid_number" : e == null ? "unknown" : e.getClass().getSimpleName());
    }
}
