...
public class DegradedStorageDrainer {
        ...
        try {
            ...
            if (storage instanceof DegradedStorageWithAck ackable) {
                ...
                try {
                    ...
                    logSuppressed("ack.sweep", ignore);
                }
                ...
                if (!memoryEnabled) {
                    ...
                    for (DegradedStorageWithAck.ClaimedPending c : claimed) {
                        ...
                        try {
                            ...
                            logSuppressed("memoryDisabled.release", ignore);
                        }
                    }
                ...
                for (DegradedStorageWithAck.ClaimedPending c : claimed) {
                    ...
                    if (!shouldPromote(ds.getDrain(), snippet)) {
                        try {
                            ...
                            logSuppressed("notPromotable.release", ignore);
                        }
                        released++;
                        continue;
                    }
                    ...
                    try {
                        ...
                        memory.reinforceWithSnippet(e.sessionKey(), query, snippet, safeDegradedSourceTag(e.reason()), score);
                        try {
                            ackable.ack(c.token());
                        } catch (Exception ignore) {
                            logSuppressed("promoted.ack", ignore);
                        }
                        promoted++;
                    } catch (Exception ex) {
                        logSuppressed("promoted.failure", ex);
                        try {
                            ...
                            logSuppressed("promoted.nack", ignore);
                        }
                        failed++;
                        log.warn("[degraded-drain] promote failed; kept in outbox. reason={} errorHash={} errorLength={}",
                                safeReason(e.reason()), com.example.lms.trace.SafeRedactor.hashValue(messageOf(ex)),
                                messageLength(ex));
                    }
                }
    ...
    private boolean getBool(String key, boolean def) {
        try {
            ...
            logSuppressed("config.getBool", ignore);
            return def;
        }
    }
    ...
    private static void logSuppressed(String stage, Exception ignored) {
        if (log.isDebugEnabled()) {
            log.debug("[degraded-drain] suppressed stage={}",
                    com.example.lms.trace.SafeRedactor.traceLabelOrFallback(stage, "unknown"));
        }
    }

    private static String safeDegradedSourceTag(String reason) {
        return "degraded:" + com.example.lms.trace.SafeRedactor.traceLabelOrFallback(reason, "unknown");
    }
    ...
    private static String sanitizeSnippet(String raw) {
        ...
        return com.example.lms.trace.SafeRedactor.safeMessage(s, 1200).trim();
    }
}
