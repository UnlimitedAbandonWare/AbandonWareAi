...
import jakarta.persistence.PersistenceException;   // 🚑 품질-필터 예외
import lombok.*;
...
public class TranslationMemory {
    ...
    private void prePersist() {
        ...
        if (this.sourceHash == null && content != null) {
             ...
             *   🚫  환각·저품질 필터  (뚱딴지 차단)
             * ───────────────────────────────────────────── */
            final double MIN_SCORE = 0.20;      // 강화 점수 하한
            final double MIN_COS   = 0.15;      // 코사인 유사도 하한

            // 1) 내용 자체가 무가치하거나 “정보 없음” 패턴
            if (content == null
                    || content.length() < 8
                    || content.matches("(?i)^정보\\s*없음\\.?$")) {
                throw new PersistenceException("[TM] low-value content filtered");
            }

            // 2) 신뢰도 부족 → 저장은 하되 검색 제외(STALE)
            boolean lowScore = (score != null && score < MIN_SCORE);
            boolean lowCos   = (cosineSimilarity != null && cosineSimilarity < MIN_COS);
            if (lowScore || lowCos) {
                this.status = MemoryStatus.STALE;
            }
        }
    }
