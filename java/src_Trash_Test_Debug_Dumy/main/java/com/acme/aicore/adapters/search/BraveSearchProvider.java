...
public class BraveSearchProvider implements WebSearchProvider {
    ...
    // Prefer the property key `search.brave.api-key` when defined; otherwise fall back to the
    // environment variables SEARCH_BRAVE_API_KEY or BRAVE_API_KEY.  Using SpEL here
    // avoids the issue where a blank property value would mask environment variables.
    // property → env(BRAVE_API_KEY → SEARCH_BRAVE_API_KEY) → 빈값
    // 현장에 따라 자주 쓰는 키 이름까지 모두 폴백
    @Value("${search.brave.api-key:${brave.api.key:${BRAVE_API_KEY:${SEARCH_BRAVE_API_KEY:${BRAVE_SEARCH_API_KEY:}}}}}")
    ...
    // Internal flag used to mark the provider as offline when no API key is present.
    private boolean offline;

    // Post construction hook to log the API key status.  Logs a warning when
    // the key is missing and marks the provider as offline; otherwise logs
    // the key length and redacted suffix for diagnostics.
    @jakarta.annotation.PostConstruct
    void init() {
        if (apiKey == null || apiKey.isBlank()) {
            offline = true;
            org.slf4j.LoggerFactory.getLogger(getClass())
                .warn("[BRAVE] offline=true reason=missing_api_key");
        } else {
            org.slf4j.LoggerFactory.getLogger(getClass())
                .info("[BRAVE] key=***{} (len={})", apiKey.substring(Math.max(0, apiKey.length() - 4)), apiKey.length());
        }
    }
    /**
