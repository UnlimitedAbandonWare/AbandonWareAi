...
public final class OfflineReplayRunner {
    ...
    /** Logger for console output replacement. */
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(OfflineReplayRunner.class);

    public static void main(String[] args) throws IOException {
        if (args.length == 0) {
            log.error("Usage: OfflineReplayRunner <replay.jsonl> [--quiet]");
            System.exit(1);
        }
        File logFile = new File(args[0]);
        if (!logFile.exists() || !logFile.isFile()) {
            log.error("Replay log does not exist: {}", logFile);
            System.exit(2);
        }
        ...
        if (!quiet) {
            log.info("Offline replay evaluation completed");
            log.info("{}", metrics);
        }
        ...
        if (metrics.getNdcgAt10() < ndcgThreshold) {
            log.error(String.format("NDCG@10 %.4f fell below threshold %.4f", metrics.getNdcgAt10(), ndcgThreshold));
            System.exit(3);
        }
        if (metrics.getMrrAt10() < mrrThreshold) {
            log.error(String.format("MRR@10 %.4f fell below threshold %.4f", metrics.getMrrAt10(), mrrThreshold));
            System.exit(4);
        }
    }
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(logFile), StandardCharsets.UTF_8))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    // When a line fails to parse we skip it but log a warning to aid debugging.
                    log.warn("Failed to parse replay line: {}", line);
