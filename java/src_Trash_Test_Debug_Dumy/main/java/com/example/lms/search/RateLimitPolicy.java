...
public class RateLimitPolicy {
    ...
    public void updateFromHeaders(HttpHeaders h) {
        ...
        long rem = firstLong(h,
                new String[]{"x-rate-limit-remaining", "x-ratelimit-remaining", "ratelimit-remaining"});
        if (rem >= 0) remaining.set(rem);
        long resetSec = firstLong(h,
                new String[]{"x-rate-limit-reset", "x-ratelimit-reset", "ratelimit-reset"});
        if (resetSec > 0) resetEpochMs.set(System.currentTimeMillis() + resetSec * 1000L);
        long retryAfterSec = firstLong(h, new String[]{"retry-after"});
