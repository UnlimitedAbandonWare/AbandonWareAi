...
public class WebSearchRetriever implements ContentRetriever {
    ...
    public List<Content> retrieve(Query query) {
        ...
        // Limit the number of returned snippets to topK and ensure a non-empty
        // result list.  When the downstream providers return no results
        // the ensureNonEmpty() helper will create a fallback segment prefixed
        // with the query.  This guarantees that the calling chain always
        // receives at least one Content item.
        java.util.List<dev.langchain4j.rag.content.Content> result = out.stream().limit(topK).toList();
        String qText = (query != null ? query.text() : normalized);
        return ensureNonEmpty(result, qText);
    }
    ...
    // --- Minimal fallback to ensure non-empty search when providers return nothing ---
    private static final String FALLBACK_SNIPPET_PREFIX = "🔎 Fallback:";
    private java.util.List<dev.langchain4j.rag.content.Content> ensureNonEmpty(java.util.List<dev.langchain4j.rag.content.Content> in, String query) {
        if (in != null && !in.isEmpty()) return in;
        dev.langchain4j.data.segment.TextSegment seg = dev.langchain4j.data.segment.TextSegment.from(
                (FALLBACK_SNIPPET_PREFIX + " " + (query == null ? "" : query)).trim());
        dev.langchain4j.rag.content.Content c = dev.langchain4j.rag.content.Content.from(seg);
