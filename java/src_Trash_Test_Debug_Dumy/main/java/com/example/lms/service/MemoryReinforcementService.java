...
// 에너지는 "낮을수록 좋음": 혜택항은 음수, 탐험항은 양수
    ...
    public void applyFeedback(String sessionId,
        ...
        try {
            // (기존 긍/부정 점수 업데이트 로직)
            // Existing logic for upserting via repository and incrementing hit count by source hash is preserved.
    ...
    public void reinforceWithSnippet(TranslationMemory t) {
        try {
            // 0) 안전 추출
            String content = tryGetString(t, new String[]{"getContent", "getText", "getBody"});
            if (!StringUtils.hasText(content)) return;
            String text = content.trim();
            if (text.length() < minContentLength) return;
            if (text.length() > maxContentLength) text = text.substring(0, maxContentLength);

            String sid    = normalizeSessionId(tryGetString(t, new String[]{"getSid", "getSessionId"}));
            String query  = tryGetString(t, new String[]{"getQuery"});
            String source = tryGetString(t, new String[]{"getSourceType", "getSource"});
            Double rawSc  = tryGetDouble(t, new String[]{"getScore"});
