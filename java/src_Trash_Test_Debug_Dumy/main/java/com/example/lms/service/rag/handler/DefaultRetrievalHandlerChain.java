...
public class DefaultRetrievalHandlerChain implements RetrievalHandler {
    ...
    // Optional deep research stage inserted after Analyze; may be null (fail‑soft).
    @org.springframework.lang.Nullable
    private final com.example.lms.service.rag.DeepResearchRetriever deepResearch;
    ...
    public void handle(Query query, List<Content> accumulator) {
        ...
        // 3.5 Optional DeepResearch: perform deeper analysis after the basic analyze stage
        boolean needDeep = false;
        try {
            needDeep = gate != null && gate.needsSelfAsk(q);
        } catch (Exception ignore) {}
        if (needDeep && deepResearch != null) {
            try {
                add(accumulator, deepResearch.retrieve(query));
            } catch (Exception ignore) {
                // fail-soft: ignore errors from deep research
            }
        }
