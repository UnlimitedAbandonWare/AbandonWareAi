...
import com.example.lms.service.rag.safety.SafeRetrieverRunner;
...
public class DynamicRetrievalHandlerChain implements RetrievalHandler {
    ...
    // Safe retriever runner to enforce hard timeouts on individual retrievals
    private final SafeRetrieverRunner safeRunner;

    @Value("${retrieval.timeout.ms:2400}")
    ...
    public void handle(Query query, List<Content> accumulator) {
        ...
        for (Source src : plan) {
            ...
            try {
                switch (src) {
                    case WEB -> {
                        // Execute the web retriever with a hard timeout and fail‑soft fallback
                        java.util.List<Content> items = safeRunner.run(() -> web.retrieve(query), java.time.Duration.ofMillis(retrievalTimeoutMs), java.util.Collections.emptyList());
                        add(accumulator, items);
                        try {
                            log.debug("[RetrieverTimeout] WEB returned {} items", items != null ? items.size() : 0);
                        } catch (Exception ignore) {}
                    }
                    case VECTOR -> {
                        // Execute the vector retriever via the RAG service with a timeout
                        ContentRetriever vector = rag.asContentRetriever(pineconeIndexName);
                        java.util.List<Content> items = safeRunner.run(() -> vector.retrieve(query), java.time.Duration.ofMillis(retrievalTimeoutMs), java.util.Collections.emptyList());
                        add(accumulator, items);
                        try {
                            log.debug("[RetrieverTimeout] VECTOR returned {} items", items != null ? items.size() : 0);
                        } catch (Exception ignore) {}
                    }
                    case KG -> {
                        if (kg != null) {
                            java.util.List<Content> items = safeRunner.run(() -> kg.retrieve(query), java.time.Duration.ofMillis(retrievalTimeoutMs), java.util.Collections.emptyList());
                            add(accumulator, items);
                            try {
                                log.debug("[RetrieverTimeout] KG returned {} items", items != null ? items.size() : 0);
                            } catch (Exception ignore) {}
                        }
                    }
                }
        ...
        try {
            if (repair != null) {
                java.util.List<Content> items = safeRunner.run(() -> repair.retrieve(query), java.time.Duration.ofMillis(retrievalTimeoutMs), java.util.Collections.emptyList());
                add(accumulator, items);
                try {
                    log.debug("[RetrieverTimeout] REPAIR returned {} items", items != null ? items.size() : 0);
                } catch (Exception ignore) {}
            }
        } catch (Exception e) {
