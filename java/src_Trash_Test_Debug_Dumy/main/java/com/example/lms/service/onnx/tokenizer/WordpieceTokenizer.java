...
public class WordpieceTokenizer implements CrossEncoderTokenizer {
    // Special token IDs for BERT-style models.  These values correspond
    // to the positions of [CLS], [SEP] and [PAD] in most pretrained vocabularies.
    // They can be overridden via a constructor or configuration when needed.
    private int clsId = 101;
    private int sepId = 102;
    private int padId = 0;
    // Default maximum sequence length.  When encoding a pair longer than this
    // length the sequence will be truncated; shorter sequences are padded.
    private int maxSeqLen = 256;

    /**
     * Pack two token ID sequences into a single array using [CLS], [SEP] and
     * [PAD] markers.  The result has fixed length {@link #maxSeqLen}.  When
     * the concatenated sequence exceeds the maximum length it is truncated.
     * When shorter, it is padded with {@link #padId}.
     *
     * @param a token IDs for the first sequence (e.g., query)
     * @param b token IDs for the second sequence (e.g., document)
     * @return packed and padded token ID array
     */
    int[] packPair(java.util.List<Integer> a, java.util.List<Integer> b) {
        java.util.ArrayList<Integer> ids = new java.util.ArrayList<>(maxSeqLen);
        ids.add(clsId);
        ...
        if (ids.size() > maxSeqLen) {
            ids.subList(maxSeqLen, ids.size()).clear();
        }
        // pad to maxSeqLen
        while (ids.size() < maxSeqLen) {
            ids.add(padId);
        }
