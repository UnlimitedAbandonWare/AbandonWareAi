...
public class OnnxRuntimeService {
     ...
     * nested placeholder resolves the abandonware key first and falls back to
    ...
    /**
     * Compute similarity scores for a batch of query/document pairs.  This is a
     * convenience wrapper around {@link #scorePair(String, String)} that
     * iterates over each pair and returns an array of doubles.  When the ONNX
     * runtime is unavailable the fallback lexical similarity is used.  The
     * returned array has the same length as the input list.
     *
     * @param pairs list of query/document pairs
     * @return array of similarity scores in the range [0,1]
     */
    public double[] scoreBatch(java.util.List<org.apache.commons.lang3.tuple.Pair<String, String>> pairs) {
        ...
        for (int i = 0; i < pairs.size(); i++) {
            ...
            if (p == null) {
                ...
                scores[i] = scorePair(p.getLeft(), p.getRight());
            }
        }
        return scores;
    }
