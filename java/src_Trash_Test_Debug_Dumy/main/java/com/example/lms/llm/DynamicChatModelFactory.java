...
public class DynamicChatModelFactory {
    ...
    // Inject the OpenAI API key from configuration only. No environment fallback is permitted.
    ...
    public ChatModel lc(String modelId, double temperature, double topP, Integer maxTokens) {
        ...
        OpenAiChatModel.OpenAiChatModelBuilder builder = OpenAiChatModel.builder()
                .modelName(modelGuard.requireAllowedOrFallback(modelId))
        ...
        // Apply the maximum token limit if provided.  LangChain4j 1.x no longer accepts
        // explicit maxTokens settings on the builder.  The token limit is
        // instead enforced server‑side or via the API provider defaults.
        // When a limit is provided we intentionally ignore it to avoid API errors.
