...
public class SettingsController {
    ...
    /**
     * Injected guard to validate model identifiers in settings.  When a
     * 'defaultModel' entry is provided in the incoming payload this guard
     * ensures the value is present in the allowed model list.  Invalid
     * models cause the controller to return a 400 error without
    ...
    public ResponseEntity<Map<String, String>> saveAllSettings(@RequestBody Map<String, String> settings) {
        ...
        if (settings != null && settings.containsKey("defaultModel")) {
            String dm = String.valueOf(settings.get("defaultModel")).trim();
            if (!dm.isBlank() && !modelGuard.allowed(dm)) {
                return new ResponseEntity<>(
                        Map.of(
                                "error", "unsupported_model",
                                "message", "Model not allowed: " + dm,
                                "allowed", String.join(", ", modelGuard.allowedList())
                        ),
