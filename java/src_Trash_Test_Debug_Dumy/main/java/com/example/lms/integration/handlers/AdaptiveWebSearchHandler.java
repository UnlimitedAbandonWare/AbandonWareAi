...
public class AdaptiveWebSearchHandler extends AbstractRetrievalHandler {
    ...
    /**
     * Fanout search service that coordinates across all registered web search
     * providers.  This service attempts an Azure-first search and, if a
     * quorum of results is not achieved or Azure is unavailable, falls back
     * to the remaining providers in parallel.  It also tracks failures and
     * trips a circuit breaker when a configured threshold is exceeded.
     */
    private final com.example.lms.gptsearch.web.FanoutQuorumSearchService searchFanout;
    ...
    protected boolean doHandle(Query q, List<Content> acc) {
        ...
        try {
            ...
            // If RAG (vector retrieval) is enabled, automatically enable web search as well.
            boolean ragOn = false;
            try {
                Object rf;
                if (md != null) {
                    // Prefer explicit rag.on, then rag, then useRag keys
                    if (md.containsKey("rag.on")) {
                        rf = md.get("rag.on");
                    } else if (md.containsKey("rag")) {
                        rf = md.get("rag");
                    } else {
                        rf = md.get("useRag");
                    }
                ...
                if (rf instanceof Boolean b) {
                    ragOn = b;
                } else if (rf != null) {
                    ragOn = Boolean.parseBoolean(String.valueOf(rf));
                }
            } catch (Exception ignore) {
                // ignore RAG flag parsing errors
            }
            // widen search gate when decision indicates knowledge query or the query text looks like a question
            try {
                String _qt = (q.text() == null ? "" : q.text().trim());
                SearchDecision _dec = decisionService.decide(_qt, SearchMode.AUTO, null, null);
                boolean _looks = _qt.endsWith("?")
                        || _qt.contains("뭐냐") || _qt.contains("무엇") || _qt.contains("누구")
                        || _qt.contains("어디") || _qt.contains("언제") || _qt.contains("왜")
                        || _qt.contains("정의") || _qt.contains("뜻") || _qt.contains("정보");
                if (!"OFF".equalsIgnoreCase(modeStr) && ((_dec != null && _dec.shouldSearch()) || ragOn || _looks)) {
                    useWeb = true;
                }
            } catch (Exception ignore) {
        ...
        // When vector retrieval (RAG) is enabled, override the search decision gate.
        boolean __ragOnFlag = false;
        try {
            Object rf;
            if (metaMap != null) {
                if (metaMap.containsKey("rag.on")) {
                    rf = metaMap.get("rag.on");
                } else if (metaMap.containsKey("rag")) {
                    rf = metaMap.get("rag");
                } else {
                    rf = metaMap.get("useRag");
                }
            } else {
                rf = null;
            }
            if (rf instanceof Boolean b) {
                ...
                __ragOnFlag = Boolean.parseBoolean(String.valueOf(rf));
            }
        } catch (Exception ignore) {
            __ragOnFlag = false;
        }
        if (!decision.shouldSearch() && !__ragOnFlag) {
            return true;
        }
        // Guard against missing providers or invalid quorum configuration.  Use reflection to access provider list and snapshot to read quorum.
        try {
            int __providerCount = 0;
            try {
                java.lang.reflect.Field pf = searchFanout.getClass().getDeclaredField("providers");
                pf.setAccessible(true);
                Object obj = pf.get(searchFanout);
                if (obj instanceof java.util.List) {
                    __providerCount = ((java.util.List<?>) obj).size();
                }
            } catch (Exception ignore) {
                __providerCount = 0;
            }
            int __quorumMinVal = 0;
            try {
                java.util.Map<String, Object> snap = searchFanout.snapshot();
                Object qmObj = snap.get("quorum.min");
                if (qmObj instanceof Number) {
                    __quorumMinVal = ((Number) qmObj).intValue();
                }
            } catch (Exception ignore) {
                __quorumMinVal = 0;
            }
            if (__providerCount <= 0 || __quorumMinVal < 1) {
                log.info("[WEB] skip: no-provider-config q='{}'", queryText);
                return true;
            }
        } catch (Exception ignore) {
            // If reflection or snapshot checks fail, continue execution.
        }
        try {
            ...
            // Accumulate documents via fanout/quorum service.  The fanout service
            // returns a unified WebSearchResult containing deduplicated documents.
            List<WebDocument> docs = new ArrayList<>();
            List<String> providerNames = new ArrayList<>();
            try {
                com.example.lms.gptsearch.web.dto.WebSearchResult r = searchFanout.search(wq);
                if (r != null && r.getDocuments() != null) {
                    for (WebDocument d : r.getDocuments()) {
                        docs.add(d);
                        // Use the provider id from the fanout result (e.g. "fanout") for each document.
                        providerNames.add(r.getProviderId());
                    }
                }
            } catch (Exception e) {
                // Fanout failures should not propagate; log and proceed with empty docs.
                log.debug("Fanout search failed: {}", e.toString());
            }
            // Meta hints (안전 파서)
