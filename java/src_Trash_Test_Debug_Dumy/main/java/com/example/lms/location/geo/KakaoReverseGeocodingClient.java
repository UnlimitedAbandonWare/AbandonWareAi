...
public class KakaoReverseGeocodingClient implements ReverseGeocodingClient {
    ...
    public Optional<Address> reverse(double lat, double lng) {
        ...
        try {
            ...
            String json;
            try {
                // Convert the reactive Mono into a CompletableFuture and wait for
                // completion.  Using get() avoids blocking the reactive event
                // loop because the subscription is scheduled on a bounded
                // elastic scheduler above.  If the future completes
                // exceptionally we log and return empty.
                json = call.toFuture().get();
            } catch (InterruptedException | java.util.concurrent.ExecutionException ex) {
                Thread.currentThread().interrupt();
                log.debug("Reverse geocoding call interrupted", ex);
                return Optional.empty();
            }
