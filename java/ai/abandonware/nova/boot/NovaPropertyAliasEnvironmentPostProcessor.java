 ...
 * - Some libraries or merged patches use other spellings (openai.api.key, spring.ai.openai.api-key, ...)
 *
 * <p>This post-processor primarily provides safe aliases when the target key is blank/missing.
 * Additionally, it can reconcile a small set of known-bad conflicts (e.g., duplicate OpenAI API key keys)
...
public class NovaPropertyAliasEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {
    ...
    public void postProcessEnvironment(ConfigurableEnvironment env, SpringApplication application) {
        ...
        // - Earlier patches populated multiple keys for convenience/compatibility, which
        ...
        if (envKey != null) {
            ...
            // No strict key is set; if we have a compat key, bind it to our documented flat key.
            winnerKeyName = "llm.api-key-openai";
        }
        ...
        int present = 0;
        if (envKey != null) present++;
        if (flatKey != null) present++;
        if (nestedKey != null) present++;

        if (present > 1) {
            boolean different = false;
            String first = (envKey != null) ? envKey : (flatKey != null ? flatKey : nestedKey);
            if (envKey != null && !envKey.equals(first)) different = true;
            if (flatKey != null && !flatKey.equals(first)) different = true;
            if (nestedKey != null && !nestedKey.equals(first)) different = true;
        ...
        if (winnerValue != null) {
            ...
            // Populate non-strict compatibility keys (safe, KeyResolver does not check these).
        ...
        // ---- LLM model id aliases (prevents empty env override -> downstream "model is required") ----
        // NOTE: Spring placeholder defaults (${ENV:${fallback}}) do NOT apply when ENV is present-but-empty.
        // That pattern frequently results in resolved "" (blank) model ids. OpenAI-compatible servers then respond
        ...
        // Run after config data load so we can see what actually resolved, but insert the
        // alias property source with HIGH precedence so it can override blank placeholder
    ...
    @Override
    public int getOrder() {
        // Run late (after config data load) so aliases are computed from the final environment.
        return Ordered.LOWEST_PRECEDENCE;
    }

    private static void putIfBlank(ConfigurableEnvironment env, Map<String, Object> out, String targetKey, String value) {
     ...
     *
     * <p>Used to satisfy strict downstream validators (e.g., KeyResolver) that
     * require only ONE key among a known set to be visible as non-blank.</p>
     */
    ...
    private static void warnIfConflicting(ConfigurableEnvironment env, String keyA, String keyB) {
        ...
        try {
            ...
            if (!a.trim().equals(b.trim())) {
                log.warn("[NovaAlias] Conflicting values detected for '{}' and '{}'. Set only one (values are redacted).", keyA, keyB);
            }
        } catch (Exception ignore) {
            // non-fatal
        }
    ...
    private static String reconcileOpenAiKeyConflict(
        ...
        // NOTE: Some operators set env vars with surrounding quotes ("sk-..."), which should be treated
        ...
        // If normalized secrets match but raw values differ (quotes, whitespace), we still override
        // both keys to the normalized winner to prevent strict downstream validators from failing.
        ...
        // Force both keys to the winner value so downstream strict validation won't hard-fail.
    ...
    private enum ProviderHint { OPENAI, GROQ, AZURE, UNKNOWN }
    ...
    private static int scoreApiKeyForHint(String rawKey, ProviderHint hint) {
        ...
        switch (hint) {
            case OPENAI -> {
                if (looksLikeOpenAiKey(k)) score += 20;
                if (looksLikeGroqKey(k)) score -= 20;
            }
            case GROQ -> {
                if (looksLikeGroqKey(k)) score += 20;
                if (looksLikeOpenAiKey(k)) score -= 5; // still acceptable for some gateways, but deprioritize
            }
            case AZURE, UNKNOWN -> {
                // No strong prefix expectations.
                if (looksLikeOpenAiKey(k)) score += 5;
                if (looksLikeGroqKey(k)) score += 5;
            }
        }
    ...
    private static boolean looksLikePlaceholder(String v) {
        ...
        // If config placeholder leaked through (e.g., "${OPENAI_API_KEY:}") treat as placeholder.
