...
public class FailSoftQueryAugmentAspect {
    ...
    public Object aroundRetrieveAll(ProceedingJoinPoint pjp) throws Throwable {
        ...
        if (ctx != null) {
            shouldActivate = ctx.isAuxDown() || ctx.isCompressionMode() || ctx.isStrikeMode() || ctx.isBypassMode();
        }
        if (!shouldActivate) {
            return pjp.proceed();
        }
        ...
        try {
            ...
            TraceStore.put("orch.failsoft.queryAugment.reason", ctx == null ? "unknown" :
                    (ctx.isAuxHardDown() ? "auxHardDown" : (ctx.isAuxDegraded() ? "auxDegraded" :
                            (ctx.isCompressionMode() ? "compression" : (ctx.isStrikeMode() ? "strike" : "bypass")))));
