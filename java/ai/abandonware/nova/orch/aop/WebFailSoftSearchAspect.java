...
public class WebFailSoftSearchAspect {
    ...
    public Object aroundSearch(ProceedingJoinPoint pjp) throws Throwable {
        ...
        // Pre-flight: if Brave is already in coordinator backoff, mark partial-down early
        ...
        try {
            ...
            if (backoff != null) {
                RateLimitBackoffCoordinator.Decision bd = backoff.shouldSkip(RateLimitBackoffCoordinator.PROVIDER_BRAVE);
        ...
        try {
            ...
            webHardDown = (ctx != null && ctx.isWebRateLimited())
                    ...
                    // If we still have some non-empty raw results, do NOT treat "skipped >= 2" as a hard-down:
                    // a single provider (e.g., Brave) may be cooling down while Naver still returns usable evidence.
        ...
        if ((staged == null || staged.isEmpty())
            ...
            // Apply provider backoff (await-timeout/cancel) from the *first* attempt before we start
            // extra calls, so we don't loop on the same timebox failures inside one request.
            ...
            for (String q2 : aug.queries()) {
                ...
                try {
                    ...
                    // Apply await-timeout backoff even when the staged output is empty, so subsequent extra calls
                    // can skip the repeatedly timing-out provider instead of looping on timebox failures.
                    summarizeAwaitEventsForTrace();
                    maybeApplyProviderBackoffFromAwaitSummary();
    ...
    private void maybeApplyProviderBackoffFromAwaitSummary() {
        ...
        try {
            ...
            boolean naverAwaitTimeoutApplied = false;
            try {
                naverAwaitTimeoutApplied = Boolean.TRUE.equals(
                        TraceStore.get("web.failsoft.rateLimitBackoff.naver.awaitTimeoutApplied"));
            } catch (Throwable ignore) {
                // best-effort
            }

            if (naverAwaitTimeout > 0L && !naverAwaitTimeoutApplied) {
            ...
            boolean braveAwaitTimeoutApplied = false;
            try {
                braveAwaitTimeoutApplied = Boolean.TRUE.equals(
                        TraceStore.get("web.failsoft.rateLimitBackoff.brave.awaitTimeoutApplied"));
            } catch (Throwable ignore) {
                // best-effort
            }

            if (braveAwaitTimeout > 0L && !braveAwaitTimeoutApplied) {
                backoff.recordFailure(
                        RateLimitBackoffCoordinator.PROVIDER_BRAVE,
                        RateLimitBackoffCoordinator.FailureKind.AWAIT_TIMEOUT,
                        "await_timeout",
                        "count=" + braveAwaitTimeout);
                try {
    ...
    private void recordSoakKpiMinuteSummary(long runId, @Nullable List<String> out) {
        ...
        try {
            ...
            boolean braveCooldownFlag = isTruthy(TraceStore.get("web.brave.cooldown.hit")) || drt.startsWith("cooldown");
    ...
    private void emitSoakKpiJson(long runId, Map<String, Integer> stageCountsSelectedFromOut, List<String> out) {
        ...
        try {
            ...
            try {
                ...
                // Await-timeout local cooldown observability (applied by WebFailSoftSearchAspect)
            ...
            try {
                ...
                j.put("web.brave.rate_limit_local.cooldownMs", toLong(TraceStore.get("web.brave.rate_limit_local.cooldownMs")));
                // Local backoff observability (streak + jitter + cap) for RATE_LIMIT_LOCAL loops
                ...
                j.put("web.brave.rate_limit_local.jitterMs", toLong(TraceStore.get("web.brave.rate_limit_local.jitterMs")));
                j.put("web.brave.rate_limit_local.capMs", toLong(TraceStore.get("web.brave.rate_limit_local.capMs")));
                j.put("web.brave.cooldown.reason", safeTrim(stringOrNull(TraceStore.get("web.brave.cooldown.reason")), 64));

                j.put("web.await.minLiveBudget.officialOnly", isTruthy(TraceStore.get("web.await.minLiveBudget.officialOnly")));
                j.put("web.await.minLiveBudget.officialOnly.floorMs", toLong(TraceStore.get("web.await.minLiveBudget.officialOnly.floorMs")));
                ...
                j.put("aux.queryTransformer.hintTimeout.count", toLong(TraceStore.get("aux.queryTransformer.hintTimeout.count")));
                ...
                // QueryTransformer degraded counters (breaker-open / prompt-len-guard / empty-json etc.)
                ...
                // Fallback-seed specificity scores (when guardContext.userQuery overrides a generic tail)
            ...
            try {
                ...
                j.put("web.await.events.summary.engine.Naver.cause.await_timeout.count",
                        toLong(firstNonNull(TraceStore.get("web.await.events.summary.engine.Naver.cause.await_timeout.count"),
                ...
                j.put("web.await.events.summary.engine.Brave.cause.await_timeout.count",
                        toLong(firstNonNull(TraceStore.get("web.await.events.summary.engine.Brave.cause.await_timeout.count"),
