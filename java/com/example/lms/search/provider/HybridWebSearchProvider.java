...
public class HybridWebSearchProvider implements WebSearchProvider {
    ...
    private static void recordBraveSkipped(String reason, String stage, long extraMs, Throwable err) {
        try {
            ...
            if (reason != null && !reason.isBlank()) TraceStore.put("web.brave.skipped.reason", reason);
            if (stage != null && !stage.isBlank()) TraceStore.put("web.brave.skipped.stage", stage);
            if (extraMs > 0) TraceStore.put("web.brave.skipped.extraMs", extraMs);
            if (err != null) {
    ...
    private static void recordAwaitEvent(
        ...
        try {
            ...
            // - soft timeout: awaitSoft (opportunistic wait) + orchestration-side budget_exhausted
            // - hard timeout: real awaitWithDeadline get(timeout) expiration
            boolean stageSoft = "soft".equalsIgnoreCase(String.valueOf(stage));
            boolean softTimeout = timeoutFlag && (stageSoft || c.equals("budget_exhausted") || c.equals("timeout_soft"));
            ...
            // Even in normal mode, keep at least one representative "missing_future"/"skip_*"
            ...
            if (timeoutFlag) {
                ...
                // Split counters so dashboards can compute percentages without soft-timeout noise.
                if (softTimeout) {
    ...
    private <T> T awaitWithDeadline(
        ...
        if (timeoutMs <= 0) {
            ...
            if (isTraceTag(tag)) {
                log.debug("[{}] Hard Timeout (budget exhausted) - cancel(false){}", tag, LogCorrelation.suffix());
            } else {
                log.warn("[{}] Hard Timeout (budget exhausted) - cancel(false){}", tag, LogCorrelation.suffix());
            }
        ...
        try {
            ...
            long waitedMs = Math.max(0L, (System.nanoTime() - startNs) / 1_000_000L);
            if (isTraceTag(tag)) {
                log.debug("[{}] Hard Timeout ({}ms) - cancel(false){}", tag, timeoutMs, LogCorrelation.suffix());
            } else {
                log.warn("[{}] Hard Timeout ({}ms) - cancel(false){}", tag, timeoutMs, LogCorrelation.suffix());
            }
    ...
    private List<String> searchKoreanBraveAndNaver(String query, int topK) {
        ...
        if (braveFuture == null && braveSkipReason != null && !"submit_failed".equals(braveSkipReason)) {
        ...
        if (skipNaver) {
            ...
            final java.time.Duration naverBlockTimeout = java.time.Duration.ofMillis(
                    Math.max(250L, Math.max(0L, remainingMs(deadlineNs) - 50L)));
        ...
        mergeMeta.put("calledBrave", braveFuture != null);
    ...
    private void emitMergeBoundaryEvent(
        ...
        try {
            ...
            // Digests (order-sensitive + order-insensitive) help detect merge ordering issues.
