...
public class HybridWebSearchProvider implements WebSearchProvider {
    ...
    private String extractKeywords(String query) {
        ...
        // 조사/어미/불용어 제거 (간단 버전)
        String s = query;
        s = s.replaceAll("(누구야|뭐야|무엇이야|무슨뜻|무슨의미|알려줘(?:봐|요)?|말해줘(?:봐|요)?|검색해(?:줘|봐|요)?|찾아줘(?:봐|요)?|설명해줘(?:봐|요)?|해줘(?:봐|요)?|어떤|사람|캐릭터|이야|인가요\\??|좀)", "");
        // 존칭 완화
    ...
    private <T> T safeGetNow(Future<T> future, T fallback, String tag, String stage) {
        ...
        if (future == null) {
            recordAwaitEvent(stage, tag, step, "missing_future", 0L, 0L, null);
            return fallback;
        }
    ...
    private static void recordAwaitEvent(
        ...
        try {
            ...
            boolean okish = c.equals("ok") || c.equals("done") || c.equals("done_null");
    ...
    private <T> T awaitWithDeadline(
        ...
        if (future == null) {
            recordAwaitEvent("hard", tag, step, "missing_future", 0L, 0L, null);
            return fallback;
        }
