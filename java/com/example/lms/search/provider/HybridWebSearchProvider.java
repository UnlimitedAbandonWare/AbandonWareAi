...
public class HybridWebSearchProvider implements WebSearchProvider {
    ...
    private <T> T awaitWithDeadline(
        ...
        final long floorMs = Math.max(0L, awaitMinLiveBudgetMs);
        ...
        final boolean floorApplied = floorMs > 0L
                && (budgetExhausted || nearExhausted || (awaitFloorTinyBudget && tinyBudget));
        ...
        try {
            ...
            recordAwaitEvent("hard", tag, stepLabel, floorApplied ? floorCause : "timeout", timeoutMs, waitedMs, te);
            return fallback;
        } catch (InterruptedException ie) {
            // Avoid poisoning pooled request threads.
            Thread.interrupted();
    ...
    private List<String> searchKoreanNaverAndBrave(String query, int topK) {
        ...
        BraveSearchResult braveMeta = braveSoftJoin
    ...
    private List<String> maybeRemergeOnceCacheOnly(String originalQuery, int topK) {
        ...
        for (int i = 0; i < polls; i++) {
            ...
            merged = mergeAndLimit(braveCached.snippets(), naverCached, topK);

            TraceStore.put("web.failsoft.remergeOnce.braveCached", braveCached.snippets().size());
            TraceStore.put("web.failsoft.remergeOnce.naverCached", naverCached.size());
