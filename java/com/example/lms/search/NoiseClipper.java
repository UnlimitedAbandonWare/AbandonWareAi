...
public class NoiseClipper {
    ...
    public String clip(String raw) {
        ...
        // Special-case: "X이/가 뭐야/어떤 곳..." 형태면 엔티티(X)만 추출
        var em = ENTITY_WHAT.matcher(s);
        if (em.matches()) {
            s = em.group(1).strip();
        }
