...
public class ContextAwareExecutorService implements ExecutorService {
    ...
    public void execute(Runnable command) {
        ...
        delegate.execute(wrap(command, mdc, guard));
    }
    ...
    public Future<?> submit(Runnable task) {
        Map<String, String> mdc = MDC.getCopyOfContextMap();
        GuardContext guard = captureGuardOrDefault();
        return delegate.submit(wrap(task, mdc, guard));
    }
    ...
    public <T> Future<T> submit(Runnable task, T result) {
        Map<String, String> mdc = MDC.getCopyOfContextMap();
        GuardContext guard = captureGuardOrDefault();
        return delegate.submit(wrap(task, mdc, guard), result);
    }
    ...
    public <T> List<Future<T>> invokeAll(Collection<? extends Callable<T>> tasks) throws InterruptedException {
        ...
        for (Callable<T> t : tasks) wrapped.add(wrap(t, mdc, guard));
    ...
    private Runnable wrap(Runnable task, Map<String, String> mdc, GuardContext guard) {
        return () -> {
