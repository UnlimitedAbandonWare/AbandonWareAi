...
public class PromptBuilder {
    ...
    public String buildInstructions(PromptContext ctx) {
        ...
        // Determine whether the context is effectively empty.  We treat the context as empty
        // only when there are no web or RAG documents and no file, history or memory text.
        boolean emptyCtx = (ctx == null)
                || ((ctx.web() == null || ctx.web().isEmpty())
                && (ctx.rag() == null || ctx.rag().isEmpty())
                && !StringUtils.hasText(ctx.fileContext())
                && !StringUtils.hasText(ctx.history())
                && !StringUtils.hasText(ctx.memory()));
        if (emptyCtx) {
            // When the context is truly empty instruct the model to respond with "정보 없음" when
            ...
            sys.append("- 답변은 제공된 컨텍스트와 문서 근거에 기반해야 합니다. 정보가 부족하면 '정보 없음'이라고 답하세요.\n");
        } else {
            // When some context exists, instruct the model to ground claims and avoid adding
            // unsupported information, but omit the '정보 없음' directive so that the model
            // attempts to synthesize an answer from the available evidence.
            sys.append("- Ground every claim in the provided sections; do not introduce information that isn't grounded in the context.\n");
            sys.append("- 답변은 제공된 컨텍스트와 문서 근거에 기반해야 합니다. 컨텍스트에 없는 새로운 정보나 추측을 답변에 포함하지 마세요.\n");
        }
