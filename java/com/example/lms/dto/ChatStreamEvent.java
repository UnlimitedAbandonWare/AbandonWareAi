...
/**
 * SSE 이벤트 페이로드.
 * type: "status" | "trace" | "token" | "final" | "error" | "thought" | "session" | "understanding"
 * data: 상태/토큰 문자열
...
public record ChatStreamEvent(
         ...
         * Useful for SSE clients that need to persist the session id before the
         * final event arrives.
         */
        ...
        /**
         * Emit a final event enriched with the resolved answer mode (e.g. EVIDENCE_ONLY).
         */
        ...
        /**
         * Emit a final event enriched with answer mode and the persisted trace turn id.
         *
         * <p>traceTurnId is the DB id of the TRACE meta system message so that UIs can
         * target-open the exact trace panel without reloading the session.</p>
         */
        public static ChatStreamEvent done(
                ...
                String answerMode,
                Long traceTurnId) {
                return new ChatStreamEvent("final", null, null, modelUsed, ragUsed, sessionId, answerMode, traceTurnId);
        }
        ...
        public static ChatStreamEvent thought(String msg) {
                return new ChatStreamEvent("thought", msg, null, null, null, null, null, null);
        }
