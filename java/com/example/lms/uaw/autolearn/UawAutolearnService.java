 ...
 *
 * <p>P2 (log-based sampling / plan DSL execution) can be layered on top later.
 */
@Service
    ...
    private List<String> buildSeeds() {
        ...
        try {
            ...
            for (int i = 0; i < gapTake; i++) {
                if (gapLogger == null) break;
                var evt = gapLogger.poll();
                if (evt == null || evt.isEmpty()) break;
        ...
        // 4) Sample seeds from real user chat history if enabled (also can fill with baseSeeds depending on cfg).
        try {
            if (seedSampler != null) {
