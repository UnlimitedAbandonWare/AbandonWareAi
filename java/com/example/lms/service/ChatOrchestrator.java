...
// 🔧 오케스트레이터 주입
    ...
    @Cacheable(
            ...
            key = "T(com.example.lms.service.ChatOrchestrator).cacheKey(#req)"
    )
    public ChatResult continueChat(ChatRequestDto req) {
        Function<String, List<String>> defaultProvider =
                q -> webSearchProvider.search(q, 5);    // WebSearchProvider Top-5
...
int plateLimit = Math.max(hybridTopK, Math.max(plate.webTopK(), plate.vecTopK()) * 3);
        ...
        if (lengthVerifier.isShort(out, vp.minWordCount())) {
            java.util.List<String> expandEvidence = new java.util.ArrayList<>();
            try {
                if (useWeb && topDocs != null) {
                    for (var c : topDocs) {
                        if (c != null && c.textSegment() != null && c.textSegment().text() != null) {
                            expandEvidence.add(c.textSegment().text());
                        }
                        if (expandEvidence.size() >= 5) break;
                    }
                }
                if (useRag && vectorDocs != null && expandEvidence.size() < 5) {
                    for (var c : vectorDocs) {
                        if (c != null && c.textSegment() != null && c.textSegment().text() != null) {
                            expandEvidence.add(c.textSegment().text());
                        }
                ...
                // evidence 수집 실패 시에도 확장 로직은 계속 진행
            }
            out = Optional.ofNullable(answerExpander.expandWithLc(out, vp, model, expandEvidence)).orElse(out);
        }
...
// 🔸 5) 단일 LLM 호출로 답변 생성
        ...
        try {
            ...
            String hintQuery = correctedMsg;
            if (StringUtils.hasText(draft)) {
                hintQuery = correctedMsg + "\n\n[model draft]\n" + draft;
            }
            List<String> hintSnippets = webSearchProvider.search(
                    hintQuery, 5); // 질문과 초안을 바탕으로 추가 정보 검색
...
// - private ConversationalRetrievalChain buildChain(ChatMemory mem) 변경
    ...
    private List<String> aggregateSearch(List<String> queries, int topKPerQuery) {
        ...
        for (String q : queries) {
            ...
            try {
                List<String> snippets = webSearchProvider.search(q, topKPerQuery);
