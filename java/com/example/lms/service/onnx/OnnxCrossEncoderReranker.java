...
public class OnnxCrossEncoderReranker {
  ...
  @org.springframework.beans.factory.annotation.Autowired(required = false) private LoggingSseEventPublisher sse;
    ...
    // HybridReranker 호환용 오버로드: scorer 없이 topN만 받는 경우
    public <T> List<T> rerank(String query, List<T> items, int topN) {
        if (items == null || items.isEmpty()) return items;
        if (topN > 0 && topN < items.size()) {
            return new ArrayList<>(items.subList(0, topN));
        }
        return items;
    }
