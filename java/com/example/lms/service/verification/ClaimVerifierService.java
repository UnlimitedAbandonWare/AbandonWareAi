...
try {
    if (temporalVerifier != null) {
        ...
        if (!temporalResult.isPass()) {
            log.warn("[ClaimVerifier] Temporal mismatch detected: {}", temporalResult.reason());
            finalAnswer = finalAnswer + "\n\n[시간 정합성 경고] " + temporalResult.reason();
        }
    }
} catch (Exception ignore) {
    // Temporal checker failures must never break the chat flow
    log.debug("[ClaimVerifier] temporal verification failed: {}", ignore.toString());
}
