...
public class EvidenceAwareGuard {
    ...
    public record GuardContext(
            String planName, // "hypernova", "brave", "zero_break", "safe" 등
            boolean aggressiveMode,
            double fusionScore, // GRANDAS / RRF 결과 스코어 (0.0~1.0)
            String profile, // SAFE / BRAVE / ZERO_BREAK / WILD
            String guardLevel, // HIGH / NORMAL / LOW
            double finalSigmoidScore, // Autonomous Resilience Engine의 최종 점수
            boolean braveMode, // Brave 플랜 활성화 여부
            boolean zeroBreakMode, // ZeroBreak 플랜 활성화 여부
            double irregularityScore, // IrregularityProfiler 출력 점수
            String memoryProfile, // deep_memory / wild_pro / projection
            double evidenceThreshold, // YAML에서 주입되는 동적 evidence 임계값
            int minCitationCount // YAML에서 주입되는 동적 citation 기준
    ) {
        ...
        /**
         * Aggressive plan detection used to relax guards in Brave / ZeroBreak /
         * Hypernova style plans. This method intentionally combines explicit
         * flags with profile / plan name heuristics so that callers can
         * progressively adopt the richer context model.
         */
        public boolean isAggressivePlan() {
            ...
            if (profile != null) {
                String p = profile.toUpperCase(java.util.Locale.ROOT);
                if ("BRAVE".equals(p) || "ZERO_BREAK".equals(p) || "WILD".equals(p)) {
                    return true;
                }
            }
            if (braveMode || zeroBreakMode) {
                return true;
            }
            if (planName != null) {
                String lower = planName.toLowerCase(java.util.Locale.ROOT);
                if (lower.contains("hypernova") || lower.contains("brave") || lower.contains("zero_break")) {
                    return true;
                }
            }
            return false;
        ...
        /**
         * [NEW] 프로필 기반 evidence 최소 임계값.
         * YAML 등에서 evidenceThreshold 값이 주입되면 우선 사용하고,
         * 값이 0 이하이면 플랜 aggressiveness 에 따라 기본값을 사용합니다.
         */
        public double getMinEvidenceScore() {
            if (this.evidenceThreshold > 0.0d) {
                return this.evidenceThreshold;
            }
            return isAggressivePlan() ? 0.30d : 0.35d;
        }

        /**
         * [NEW] 프로필 기반 citation 최소 개수.
         * minCitationCount 가 0 이상이면 그대로 사용하고,
         * 음수인 경우 aggressiveness 에 따라 기본값을 선택합니다.
         */
        public int getMinCitation() {
            if (this.minCitationCount >= 0) {
                return this.minCitationCount;
            }
            return isAggressivePlan() ? 1 : 2;
        }
    }
    ...
    public Result ensureCoverage(
        ...
        if (inconsistentTemplate) {
            log.warn(
                    "[guard] Inconsistent state: 'no-evidence' draft detected (count={}, coverage={}) - quality low, proceeding to standard logic.",
    ...
    public static boolean looksWeak(String draft) {
        // 기본 컨텍스트(SAFE/NORMAL)에서 판단
        return looksWeak(draft, GuardContext.defaultContext());
    }
    ...
    private GuardMode resolveMode(String draft, java.util.List<EvidenceDoc> safeEvidence) {
        ...
        boolean isDeviceRumor = q.contains("폴드7") || q.contains("fold7")
                || q.contains("갤럭시 z") || q.contains("galaxy z")
                || q.contains("폴드 7")
                || q.contains("플립7") || q.contains("flip7")
                || q.contains("s25") || q.contains("s26");
