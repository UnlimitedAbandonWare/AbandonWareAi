package com.example.lms.service.patch;
...
public class ChatServiceLegacyPatch {
    private static final Logger log = LoggerFactory.getLogger(ChatServiceLegacyPatch.class);
...
// 🔧 오케스트레이터 주입
    ...
    // `app.ai.default-model` property (e.g. gemma3:27b) and fall back to the
    // legacy `openai.api.model` setting when unspecified.  This model is
    // used when the router returns a null ChatModel.
    @Value("${app.ai.default-model:${openai.api.model:gemma3:27b}}")
    ...
    private void reinforce(String sessionKey, String query, String answer) {
        try {
            reinforceAssistantAnswer(sessionKey, query, answer);
        } catch (Throwable e) {
            log.warn("[Memory] reinforceAsync failed (sid={})", sessionKey, e);
        }
    }
    ...
    public ChatResult continueChat(ChatRequestDto req,
        ...
        try {
            ...
            // never block the chat flow, but log for diagnostics
            log.warn("[Chat] displaySearchQuery failed, continuing without search summary", e);
        }
...
// - private ConversationalRetrievalChain buildChain(ChatMemory mem) 변경
    ...
    private static String safeSnippet(dev.langchain4j.rag.content.Content c) {
        ...
        try {
            ...
            log.debug("[Chat] Failed to build short answer preview", e);
        }
        return "";
    }
