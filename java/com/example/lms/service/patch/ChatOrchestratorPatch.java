...
// 🔧 오케스트레이터 주입
    ...
    public ChatResult continueChat(ChatRequestDto req,
        ...
        // ── 확장 게이트: 증거 있고, 약하지 않고, 짧을 때만 확장 ──
        boolean haveEvidenceForExpansion = 
                (useWeb && topDocs != null && !topDocs.isEmpty())
             || (useRag && vectorDocs != null && !vectorDocs.isEmpty());
        boolean draftWeak = com.example.lms.service.guard.EvidenceAwareGuard.looksWeak(out);
        if (haveEvidenceForExpansion            // 증거 존재
                && !draftWeak                   // "정보 없음" 패턴 아님
                && model != null                // 모델 안전 체크
                && lengthVerifier.isShort(out, vp.minWordCount())) { // 길이 부족
            String expanded = answerExpander.expandWithLc(out, vp, model);
            if (expanded != null) {
                out = expanded;
            }
        }
