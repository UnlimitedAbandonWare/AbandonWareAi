...
// 🔧 오케스트레이터 주입
    ...
    public ChatResult continueChat(ChatRequestDto req,
        ...
        switch (queryDomain) {
            case STUDY, SENSITIVE -> guardProfile = GuardProfile.STRICT;
            case GAME, SUBCULTURE -> guardProfile = GuardProfile.SUBCULTURE;
            default -> guardProfile = GuardProfile.NORMAL;
        }
...
int plateLimit = Math.max(hybridTopK, Math.max(plate.webTopK(), plate.vecTopK()) * 3);
        ...
        if (useWeb && (topDocs == null || topDocs.isEmpty())) {
            log.debug("[ChatService] 웹 검색 모드이나 topDocs가 비어있음. HybridRetriever 필터링 확인 필요.");
        }
            ...
            try {
                ...
                if (!evidenceDocs.isEmpty()) {
                    ...
                    var res = guard.ensureCoverage(verified, evidenceDocs,
                            ...
                            2);
                    // Guard가 초안을 증거 리스트 기반 답변으로 degrade 했거나
                    // 재생성된 텍스트가 있는 경우, escalated 여부와 무관하게 우선 사용한다.
                    if (res.regeneratedText() != null) {
                        verified = res.regeneratedText();
                    }
                }
            } catch (Exception e) {
        ...
        if (hasAnyEvidence && isDefinitiveFailure(verified)) {
            log.info("[Rescue] 답변이 '정보 부족' 패턴으로 판별되었으나 증거가 존재함 (useWeb={}, topDocs={}, useRag={}, vectorDocs={}). EvidenceComposer로 강제 전환합니다. (query={})",
            ...
            try {
                ...
                if (verified != null) {
                    log.debug("[Rescue] 증거 기반 답변 생성 완료 (length={})", verified.length());
                }
            } catch (Exception e) {
                log.warn("[Rescue] EvidenceComposer 실패, Evidence 리스트로 Fallback 시도: {}", e.toString());
                ...
                try {
                    ...
                    // 최종 Fallback
                    log.warn("[Rescue] Evidence 리스트 생성도 실패: {}", e2.toString());
...
// - private ConversationalRetrievalChain buildChain(ChatMemory mem) 변경
    ...
    private boolean isDefinitiveFailure(String text) {
        ...
        boolean hasInsufficientInfoMarker =
                normalized.contains("충분한 증거를 찾지 못했습니다")
                        || normalized.contains("정보가 부족하여")
                        || normalized.contains("자료가 부족하여")
                        || normalized.contains("정보 없음");

        boolean hasSubstantialContent = normalized.length() > 80;

        // Only treat as definitive failure when it is almost pure "정보 부족" style
        // with no substantial explanatory content.
        return hasInsufficientInfoMarker && !hasSubstantialContent;
    }
