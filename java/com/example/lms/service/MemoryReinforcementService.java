...
import com.example.lms.service.guard.EvidenceAwareGuard.GuardContext;
@Service
@Transactional
...
// 에너지는 "낮을수록 좋음": 혜택항은 음수, 탐험항은 양수
    ...
    public void reinforceWithSnippet(TranslationMemory tm, QualityMetadata meta) {
        ...
        try {
            ...
            // 2) citation 이 0개이지만 aggressive 플랜(예: zero_break)이면서 evidenceScore 가 높은 경우 → 저장 허용
            String planId = meta.planId();
            if (!meta.hasCitation()
                    && planId != null
                    && "zero_break".equalsIgnoreCase(planId)
                    && meta.evidenceScore() >= 0.80) {
                log.info("[MEMORY_GATE] No citation but aggressive plan '{}' & evidenceScore={} → save (Projection Jammini)",
                        planId,
     ...
     * 기존 {@link #reinforceWithSnippet(TranslationMemory, QualityMetadata)} 을 그대로 사용해도 된다.
     */
...
/** Helper: 문자열을 안전하게 자르기 */
    ...
    public double adjustScoreByGuard(
        ...
        double strengthWeight =
                (decision.evidenceStrength() == com.example.lms.service.guard.EvidenceAwareGuard.EvidenceStrength.STRONG)
                        ? 1.0 : 0.7;
