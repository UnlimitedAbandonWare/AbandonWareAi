...
// 에너지는 "낮을수록 좋음": 혜택항은 음수, 탐험항은 양수
    ...
    public void reinforceWithSnippet(String sessionId,
        ...
        if (hasNegative && hasRecovery) {
            // 부정 서두 + 유용 정보가 이어지는 회복형 답변 → 감쇠 강화
            double penalty = 0.85;
            try {
                // HyperparameterService를 통해 튜닝 가능하게 열어둠
                penalty = hp.getDouble("memory.reinforce.recovered-penalty", penalty);
            } catch (Exception ignore) { }
            score = score * penalty;
            log.info("[MEMORY_GATE] Allowing recovered answer with penalty (x{}). snippet='{}'",
...
// ═══════════════════════════════════════════════════════════
        ...
        if ("ASSISTANT".equals(sourceTag) || "AI_GENERATED".equals(sourceTag)) {
            ...
            if ("STRICT".equalsIgnoreCase(mode)) {
                ...
                // SOFT 모드: URL/인용 마커가 아예 없으면 점수를 캡해서 약하게만 강화
                if (!hasCitationSignal) {
                    log.info("[MEMORY_GATE] SOFT mode: missing explicit citation, but allowing low-weight reinforcement");
                    // 너무 강하게 학습되지 않도록 상한을 둔다.
                    if (score > 0.35d) {
                        score = 0.35d;
                    }
                } else {
                    log.info("[MEMORY_GATE] SOFT mode: citation signal detected → normal scoring");
                }
            }
        }

        // ═══════════════════════════════════════════════════════════
        // GATE 3: Score Threshold (기존 로직 유지)
        // ═══════════════════════════════════════════════════════════
