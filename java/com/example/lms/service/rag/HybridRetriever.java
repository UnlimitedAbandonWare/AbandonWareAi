...
public class HybridRetriever implements ContentRetriever {
    ...
    public List<Content> retrieve(Query query) {
        ...
        switch (level) {
            case SIMPLE -> {
                /*
                 * Prefer results from the live web search. Only when the web
                 * search fails or returns no results do we fall back to the
                 * vector store. This prevents mixing a handful of web
                 * results with unrelated RAG snippets, and aligns the
                 * behaviour with the documented fallback strategy. See
                 * {스터프0}/{스터프1} for details.
                 */
                java.util.List<Content> webResults = java.util.Collections.emptyList();
                try {
                    webResults = webSearchRetriever.retrieve(query);
                } catch (Exception e) {
                    // Log and ignore; fallback will be triggered below
                    log.warn("[Hybrid] Web search failed ({}), fallback to Vector.", e.getMessage());
                }
                if (webResults != null && !webResults.isEmpty()) {
                    mergedContents.addAll(webResults);
                } else {
                    // No web results or failure → fallback to vector store
                    ContentRetriever pine = ragService.asContentRetriever(chosenIndex);
                    try {
                        List<Content> raw = pine.retrieve(query);
                        if (raw != null) {
                            mergedContents
                                    .addAll(raw.stream().filter(this::allowVectorChunk).collect(Collectors.toList()));
                        }
                    } catch (Exception ignore) {
                        // If vector retrieval also fails, leave mergedContents empty
                    }
                    // Optionally try Tavily retriever when available
                    if (mergedContents.size() < topK && tavilyWebSearchRetriever != null) {
                        try {
                            mergedContents.addAll(tavilyWebSearchRetriever.retrieve(query));
                        } catch (Exception e) {
            ...
            case AMBIGUOUS -> {
                ...
                if (mergedContents.size() < topK) {
                    ...
                    List<Content> raw = pine.retrieve(query);
                    if (raw != null) {
                        mergedContents.addAll(raw.stream().filter(this::allowVectorChunk).collect(Collectors.toList()));
                    }
                }
                if (mergedContents.size() < topK && tavilyWebSearchRetriever != null) {
                    try {
        ...
        try {
            ...
            for (String q : queries) {
                ...
                try {
                    ...
                    dev.langchain4j.rag.query.Query subQ = QueryUtils.buildQuery(q, sidForQuery, null);
        ...
        try {
            ...
            if (qualityEvaluator != null
                ...
                java.util.List<Content> out = finalizeResults(new java.util.ArrayList<>(local), "text",
                        java.util.Collections.emptyList(), question);
                return out.size() > top ? out.subList(0, top) : out;
            }
            ...
            for (int qi = 0; qi < queries.size(); qi++) {
                ...
                try {
                    ...
                    if (kSchedule != null && !kSchedule.isEmpty()) {
                        ...
                        if (k != null && k > 0)
                            subMd.put("webTopK", String.valueOf(k));
                    }
            ...
            combined.addAll(fused);
            java.util.List<Content> out = finalizeResults(combined, "text", java.util.Collections.emptyList(),
                    question);
            return out.size() > top ? out.subList(0, top) : out;
        } catch (Exception e) {
            log.error("[Hybrid] retrieveProgressive 실패(sid={}, q='{}')", sessionKey, question, e);
    ...
    public List<Content> retrieveAll(List<String> queries, int limit, Object sessionKey,
        ...
        try {
            ...
            if (debugSequential) {
                ...
                try {
                    results = pool.submit(() -> queries.parallelStream()
                            .map(q -> {
                                java.util.List<Content> acc = new java.util.ArrayList<>();
                                try {
                                    java.util.Map<String, Object> md = new java.util.HashMap<>();
