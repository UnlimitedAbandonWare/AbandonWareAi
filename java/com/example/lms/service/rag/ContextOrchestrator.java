...
public class ContextOrchestrator {
    ...
    /**
     * 여러 정보 소스를 바탕으로 최종 컨텍스트를 조율하고 생성합니다.
     * @param query 사용자 원본 쿼리
     * @param vectorResults Vector DB 검색 결과
     * @param webResults 웹 검색 결과
     * @return LLM에 전달될 최종 컨텍스트 문자열 또는 "정보 없음"
     */
    public String orchestrate(String query, List<Content> vectorResults, List<Content> webResults) {
        ...
        // 4. 안전장치: 가장 높은 점수조차 기준 미달이면, 신뢰할 수 없는 정보로 판단하고 차단
        double topScore = uniq.values().stream().mapToDouble(sc -> sc.score).max().orElse(0.0);
        ...
        // 5. 최종 프롬프트 생성
        return promptEngine.createPrompt(query, finalDocs);
    }
