...
public class AnalyzeWebSearchRetriever implements ContentRetriever {
    ...
    public List<Content> retrieve(Query query) {
        ...
        // Null-safe: if the planner is unavailable, fall back to using the original
        // query as-is.
        ...
        for (String q : queries) {
            tasks.add(ContextPropagation.wrapCallable(() -> {
                try {
                    return webSearchProvider.search(q, topK);
                } catch (Exception e) {
        ...
        try {
            java.util.List<Future<java.util.List<String>>> futures = searchIoExecutor.invokeAll(
                    tasks,
                    Math.max(250L, timeoutMs),
                    TimeUnit.MILLISECONDS);
            for (int i = 0; i < futures.size(); i++) {
                Future<java.util.List<String>> f = futures.get(i);
        ...
        if (dedupMap.isEmpty()) {
            try {
                log.warn(
                        ...
                        originalQuery);
                java.util.List<String> fallback = webSearchProvider.search(originalQuery, topK);
