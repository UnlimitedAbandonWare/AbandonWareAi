...
private static String normalize(String raw) {        /* 🔴 NEW */
    ...
    public List<Content> retrieve(Query query) {
        ...
        int k = Math.max(topK, MIN_SNIPPETS);
        ...
        List<String> first = searchWithAggressiveRetry(
                normalized,
                k * 2,
                must
        );
        ...
        List<String> fallback = ranked.size() >= MIN_SNIPPETS ? List.of()
                : searchSvc.searchSnippets(normalized.replace("교수님", "교수").replace("님",""), k);
    ...
    private List<String> searchWithAggressiveRetry(String query, int k, java.util.regex.Pattern mustVersion) {
        int maxAttempts = 3;      // 3초 * 3회 = 최대 약 9~10초
        long backoffMs = 300L;    // 시도 간 짧은 대기

        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            long start = System.currentTimeMillis();
            try {
                List<String> rawResults = searchSvc.searchSnippets(query, k);
                    ...
                    if (!valid.isEmpty()) {
                        if (attempt > 1) {
                            log.info("✅ [WebSearch] Retry success on attempt {}/{} ({}ms). Found {} valid items.",
                                    attempt, maxAttempts, System.currentTimeMillis() - start, valid.size());
                        }
                        return valid;
                    } else {
                        log.warn("⚠️ [WebSearch] Attempt {}/{} had results but all filtered out (Captcha/Version/Tag). Retrying...",
                                attempt, maxAttempts);
                    }
                } else {
                    log.warn("⚠️ [WebSearch] Attempt {}/{} returned 0 results. Retrying...", attempt, maxAttempts);
                }
            } catch (Exception e) {
