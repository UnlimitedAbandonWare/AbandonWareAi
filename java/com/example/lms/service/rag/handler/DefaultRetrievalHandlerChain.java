...
public class DefaultRetrievalHandlerChain implements RetrievalHandler {
    ...
    public void handle(Query query, List<Content> accumulator) {
        ...
        // 5. Web search (legacy) is intentionally skipped.  AdaptiveWebSearchHandler
        // already aggregates snippets from configured providers.  To avoid
        // duplicate web queries through the legacy WebSearchRetriever, the
        // invocation of web.retrieve(query) has been removed.  The evidence
        // accumulated by adaptiveWeb will be passed directly to downstream
        // stages.  Keeping this call commented out prevents redundant
        // external calls while preserving the injection of WebSearchRetriever
        // for other consumers (e.g. EvidenceRepairHandler).
