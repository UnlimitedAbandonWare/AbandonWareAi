...
public class DynamicRetrievalHandlerChain implements RetrievalHandler {
    ...
    public void handle(Query query, List<Content> accumulator) {
        ...
        if (memoryMode == MemoryMode.JAMMINI_MEMORY && sessionId != null) {
            try {
        ...
        // Apply alias-corrected query text to downstream retrievers (previously only used for heuristics)
        ...
        // Dynamic K-Allocation: fill per-source topK hints into Query metadata if missing.
        // This makes retrieval orchestration consistent with PlanHintApplier/OrchestrationHints.
        try {
            if (kallocEnabled && effectiveQuery != null) {
                ...
                if (!(hasWebK && hasVecK && hasKgK)) {
                    ...
                    try {
                        ...
                        if (it == null) it = md.get("intent");
                        if (it != null) intent = String.valueOf(it);
                    } catch (Exception ignore2) {
                    }
                    boolean officialOnly = metaBool(md, "officialSourcesOnly", false) || metaBool(md, "officialOnly", false);

                    com.abandonware.ai.agent.integrations.service.rag.kalloc.KAllocator.KPlan planK = __decideKPlan(intent,
                            qText, officialOnly);
                    if (planK != null) {
                        if (!hasWebK) {
                        ...
                        if (changed) {
                            ...
                            effectiveQuery = new Query(effectiveQuery.text(), dev.langchain4j.data.document.Metadata.from(md));
                        }
                    }
                }
            }
        ...
        // [Lore] Inject YAML-based domain knowledge using LLM NER before heavy
        // web/vector retrieval.
        ...
        if (allowLoreNer && nerExtractor != null && loreRegistry != null) {
            try {
        ...
        try {
            ...
            if (!enableAnalyzeHint || nightmareMode || auxLlmDown || strikeMode || compressionMode) {
                needAnalyze = false;
            }
        } catch (Exception ignore) { }
        if (needAnalyze) {
            ...
            try {
                switch (src) {
                    ...
                    case VECTOR -> {
                        ...
                        // this gives a fast, stable, high-signal context chunk before broader vector/web.
                        ...
                        try {
                            ...
                            TraceStore.put("uaw.thumb.recall.domain",
                                    uawThumbnailProperties != null ? uawThumbnailProperties.getKnowledgeDomain() : null);
            ...
            try {
                ...
                if (v instanceof java.util.Map<?, ?> m3) {
                    ...
                    for (java.util.Map.Entry<?, ?> e2 : m3.entrySet()) {
                        if (e2.getKey() != null) out.put(String.valueOf(e2.getKey()), e2.getValue());
                    }
                    return out;
                }
            } catch (Exception ex) {
    ...
    private com.abandonware.ai.agent.integrations.service.rag.kalloc.KAllocator.KPlan __decideKPlan(String intent,
            String query, boolean officialOnly) {
        com.abandonware.ai.agent.integrations.service.rag.kalloc.KAllocator.Settings ks =
                new com.abandonware.ai.agent.integrations.service.rag.kalloc.KAllocator.Settings();
        ...
        com.abandonware.ai.agent.integrations.service.rag.kalloc.KAllocator allocator =
                new com.abandonware.ai.agent.integrations.service.rag.kalloc.KAllocator(ks);
        return allocator.decide(new com.abandonware.ai.agent.integrations.service.rag.kalloc.KAllocator.Input(intent,
                query, officialOnly));
    }
    ...
    private static java.util.List<Source> parseOrderOverride(java.util.Map<String, Object> md) {
        ...
        if (v == null) v = md.get("retrievalOrder");
        if (v == null) v = md.get("order");
