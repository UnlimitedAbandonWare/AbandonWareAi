...
public class GenericDocClassifier {
     ...
     */
        ...
        // GENERAL/EDUCATION 도메인은 범용 패널티를 적용하지 않는다.
        if ("GENERAL".equals(d) || "EDUCATION".equals(d)) {
            return 0.0;
        }
        ...
        // 백과/개요 키워드는 항상 예외 처리
        if (WHITELIST.matcher(t).find()) {
            return 0.0;
        }

        boolean isGame = "GENSHIN".equals(d) || "GAME".equals(d);

        // 게임/원신 도메인: 캐릭터 스펙/성유물 가이드는 정답의 일부이므로 최소 패널티만 부여
        if (isGame) {
            if (t.contains("기본 정보") || t.contains("성유물")
                    || t.contains("스펙") || t.contains("무기")
                    || t.contains("파티")) {
                return 0.1;
            }
        }

        // 범용 패턴에 해당하는 경우 도메인에 따라 패널티 크기를 조절
        if (GENERIC.matcher(t).find()) {
            return isGame ? 0.15 : 0.25;
        }
