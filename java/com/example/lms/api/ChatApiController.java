...
import com.example.lms.service.rag.HybridRetriever; import com.example.lms.web.OwnerKeyResolver;
...
public class ChatApiController {
        ...
        Disposable d = Mono.fromRunnable(() -> {
            try {
                ...
                ChatSession session = (req.getSessionId() == null)
                        ? historyService.startNewSession(dto.getMessage(), _username, _clientIp, preResolvedOwnerKey, dto.getMemoryProfile())
                ...
                if (allowWeb) {
                    ...
                    if (sr.trace() != null) {
                        ...
                        try {
                            ...
                            meta.put("webTopK",
                                    String.valueOf((dto.getPrecisionTopK() != null && dto.getPrecisionTopK() > 0)
                                            ? dto.getPrecisionTopK()
                                            : topKParam));
                            meta.put("officialSourcesOnly",
                                    String.valueOf(java.lang.Boolean.TRUE.equals(dto.getOfficialSourcesOnly())));
                            ...
                            List<dev.langchain4j.rag.content.Content> curated = hybridRetriever.retrieveProgressive(
                                    dto.getMessage(),
                            ...
                            List<String> curSnips = (sr.snippets() != null && !sr.snippets().isEmpty())
                                    ...
                                    : curated.stream()
                                            ...
                                            .map(s -> s.length() > 240
                                                    ? String.format("%s/* ... *&#47;", s.substring(0, 240))
                                                    : s)
                                            .toList();
    ...
    // ===== internal =====
    /**
     * Overload that wires Jammini / guard headers into a GuardContext and exposes it
    ...
    private ChatResponseDto handleChat(ChatRequestDto uiReq, String username, String clientIp, String preResolvedOwnerKey) {
        ...
        ChatSession session = (uiReq.getSessionId() == null)
                ? historyService.startNewSession(dto.getMessage(), username, clientIp, preResolvedOwnerKey, dto.getMemoryProfile())
                        .orElseThrow(() -> new IllegalStateException("세션 생성 실패"))
        ...
        if (session == null && uiReq.getSessionId() != null) {
            ...
            session = historyService.startNewSession(dto.getMessage(), username, clientIp, preResolvedOwnerKey, dto.getMemoryProfile())
    ...
    private static String resolveModelUsed(String fromLlm, String requested) {
        ...
        if (cand != null && !isWrapperLabel(cand))
            return cand;
...
public record SessionDetail(Long id, String title, LocalDateTime createdAt, List<MessageDto> messages, String modelUsed, java.util.Map<String, Object> settings) {
    }
