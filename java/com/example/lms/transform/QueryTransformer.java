...
public class QueryTransformer {
    ...
    private static final org.slf4j.Logger log =
            org.slf4j.LoggerFactory.getLogger(QueryTransformer.class);
    ...
    // Debug-only breadcrumbs (no secrets). Helps diagnose "model is required" errors.
    ...
    /**
     * Cheap/deterministic query expansion should remain available even when the LLM-backed
     * QueryTransformer is disabled or temporarily blocked (breaker-open / aux.blocked).
     *
     * This is intentionally separated from {@code nova.orch.query-transformer.enabled}.
     */
    ...
    // TIMEOUT -> breaker OPEN can cascade into starvation. Keep a small cap for this optional stage.
    @org.springframework.beans.factory.annotation.Value("${query-transformer.llm.timeout-open-hint-ms:5000}")
    ...
    private String cachedLlm(String prompt) {
        ...
        if (bd.bypass()) {
            ...
            try {
                ...
                // QueryTransformer is an *optional* enhancer. Treat bypass as cheap "degraded" (not "blocked")
        ...
        long timeoutMs = Math.max(0L, (hint > 0 ? hint : 800L));
            ...
            try {
                ...
                CompletableFuture.runAsync(ContextPropagation.wrap(() -> {
                    ...
                    if (nightmareBreaker != null) {
                        ...
                        // otherwise we can create cancellation-toxicity cascades (cancel -> breaker-open -> aux blank).
                        try {
    ...
    private BypassDecision computeBypassDecision() {
        ...
        if (!disabled && !breakerOpen && failurePatterns != null) {
            try {
                failureCooldown = failurePatterns.isCoolingDown("llm");
            } catch (Exception ignore) {
                // fail-soft
            }
        }
        ...
        boolean bypass = disabled || breakerOpen || failureCooldown || stagePolicyClamped || ctxBypass;

        // NoiseGate: in COMPRESSION-mode bypass (false positives), allow a small escape probability
        ...
        if (bypass && !disabled && !breakerOpen && !failureCooldown && !stagePolicyClamped
                && ctxBypass && ctxReason == AuxBlockedReason.COMPRESSION) {
            try {
                boolean stageNoiseEnabled = Boolean.parseBoolean(System.getProperty("orch.noiseGate.qtx.compression.enabled", "true"));
                if (stageNoiseEnabled) {
                    double irr = (ctx != null) ? ctx.getIrregularityScore() : 0.0;
                    double max = Double.parseDouble(System.getProperty("orch.noiseGate.qtx.compression.escapeP.max", "0.18"));
                    double min = Double.parseDouble(System.getProperty("orch.noiseGate.qtx.compression.escapeP.min", "0.04"));
        ...
        try {
            ...
            if (text == null || text.isBlank()) {
                ...
                try {
                    ...
                    emitQtx(
                            DebugEventLevel.WARN,
                            "qtx.blank",
                            "QueryTransformer blank response (fail-soft)",
                            "QueryTransformer.runLLM",
                            dd,
                            null);
                } catch (Throwable ignore) {
            ...
            if (FriendShieldPatternDetector.looksLikeSilentFailure(text)) {
                ...
                try {
                    ...
                    emitQtx(
                            DebugEventLevel.WARN,
                            "qtx.friendshield",
                            "QueryTransformer silent-failure pattern (friendshield)",
                            "QueryTransformer.runLLM",
                            dd,
                            null);
                } catch (Throwable ignore) {
            ...
            if (kind == NightmareBreaker.FailureKind.CONFIG) {
                try {
                    ...
                    log.error("[QueryTransformer] LLM rejected request (model is required). fastBaseUrl={} fastModel={} curlHint={}",
                            baseUrl,
                            model,
                            curlHint);
            ...
            if (kind == NightmareBreaker.FailureKind.INTERRUPTED) {
                ...
                try {
                    ...
                    dd.put("key", breakerKey);
                    dd.put("kind", kind.name());
                    dd.put("rootType", (unwrap(e) != null ? unwrap(e).getClass().getSimpleName() : e.getClass().getSimpleName()));
                    dd.put("promptLen", (prompt != null ? prompt.length() : 0));
                    dd.put("promptSha1", com.abandonware.ai.agent.integrations.TextUtils.sha1(prompt));
                    emitQtx(
                            DebugEventLevel.WARN,
    ...
    private static NightmareBreaker.FailureKind classifyLlmFailure(Throwable t) {
        ...
        // Prefer shared classifier for HttpException + common OpenAI-compatible error bodies.
        try {
    ...
    public List<String> transform(String context, String normalizedQuery) {
        // NOTE: List.of(..) rejects null elements. For null/blank input we must fail-soft
