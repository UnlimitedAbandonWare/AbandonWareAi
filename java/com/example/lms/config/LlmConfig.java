...
public class LlmConfig {
    ...
    public ChatModel miniModel(
            ...
            @Value("${llm.mini.max-retries:1}") int maxRetries
    ) {
        ModelGuard.assertConfigured("openai-compatible", apiKey, model);
    ...
    public ChatModel fastChatModel(
            ...
            @Value("${llm.fast.timeout-seconds:3}") long timeoutSeconds,
        ...
        var builder = OpenAiChatModel.builder()
                ...
                .timeout(Duration.ofSeconds(timeoutSeconds));
        if (maxTokens != null) {
            if (OpenAiTokenParamCompat.shouldSendLegacyMaxTokens(model, baseUrl)) {
    ...
    public ChatModel highModel(
            ...
            @Value("${llm.high.max-retries:2}") int maxRetries
    ) {
        ModelGuard.assertConfigured("openai-compatible", apiKey, model);
