...
public class FederatedEmbeddingStore implements EmbeddingStore<TextSegment> {
    ...
    private final ExecutorService pool = new ContextAwareExecutorService(
            Executors.newCachedThreadPool(r -> {
                Thread t = new Thread(r, "vec-federated-" + System.nanoTime());
                t.setDaemon(true);
                return t;
            }));
    @PostConstruct
    void logInitialization() {
        if (stores == null || stores.isEmpty()) {
            log.warn("FederatedEmbeddingStore initialized with NO underlying stores. " +
                    "Vector search will always return empty results. " +
                    "Check FederatedVectorStorePatchConfig / EmbeddingStore beans.");
        } else {
            List<String> ids = stores.stream().map(NamedStore::id).toList();
            log.info("FederatedEmbeddingStore initialized with {} store(s): {}", ids.size(), ids);
        }
    }
        ...
        for (NamedStore ns : stores) {
            try {
                ...
                try {
                    ...
                    ok++;
                    log.warn("Federated addAll degraded on store {}: addAll(ids,..) unsupported -> fallback used", ns.id());
                } catch (Exception e2) {
                    failed.add(ns.id() + ":" + e2.getClass().getSimpleName());
                    log.warn("Federated addAll failed on store {} (fallback also failed): {}", ns.id(), e2.toString());
                }
            } catch (Exception e) {
                failed.add(ns.id() + ":" + e.getClass().getSimpleName());
                log.warn("Federated addAll failed on store {}: {}", ns.id(), e.toString());
            }
        }
    ...
    public EmbeddingSearchResult<TextSegment> search(EmbeddingSearchRequest req) {
        ...
        if (merged.isEmpty()) {
            ...
            log.info("ROUTE_LABEL topic={} weights={} split={} k={} stores={}",
                    topic, weights, split, k,
                    stores.stream().map(NamedStore::id).toList());
            return new EmbeddingSearchResult<>(Collections.emptyList());
        }
