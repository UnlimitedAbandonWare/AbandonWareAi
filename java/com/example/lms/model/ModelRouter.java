...
@org.springframework.context.annotation.Profile("legacy-router")
...
public class ModelRouter implements com.example.lms.service.routing.ModelRouter {
    ...
    /**
     * When true the router will always pick the upper tier chat model regardless of heuristics.
     * This flag is injected from the {@code ai.moe.force-upper-tier} property.  Defaults to
     * false when the property is unset.  See application.yml for details.
     */
    @org.springframework.beans.factory.annotation.Value("${ai.moe.force-upper-tier:false}")
    private boolean forceUpperTier;

    /**
     * The model identifier to use when {@link #forceUpperTier} is enabled.  Injected from
     * {@code ai.moe.upper-tier-chat-model}.  When null or blank the router falls back to
     * the configured MOE model name.  See application.yml for details.
     */
    @org.springframework.beans.factory.annotation.Value("${ai.moe.upper-tier-chat-model:}")
    private String upperTierChatModel;
    ...
    public ChatModel route(RouteSignal sig) {
        // When forceUpperTier is enabled always return the configured upper tier model.
        if (forceUpperTier) {
            return resolveUpperTier();
        }
        if (sig == null) {
    ...
    public ChatModel route(@Nullable String intent,
        ...
        // When forceUpperTier is enabled disregard all heuristics and use the configured upper tier model.
        if (forceUpperTier) {
            return resolveUpperTier();
        }
    ...
    private ChatModel resolveMoe() {
        // If forceUpperTier is set always delegate to resolveUpperTier.
        if (forceUpperTier) {
            return resolveUpperTier();
        }
    ...
    /**
     * Resolve the explicitly configured upper tier chat model.  When a custom model name is provided
     * via the {@code ai.moe.upper-tier-chat-model} property the DynamicChatModelFactory is used
     * to instantiate it on demand.  If no factory is available but a preconfigured MOE model bean
     * exists, the bean is returned.  As a last resort the base model is returned, ensuring that
     * forceUpperTier never results in a null model.
     *
     * @return the resolved upper tier ChatModel
     */
    private ChatModel resolveUpperTier() {
        String modelName = (upperTierChatModel != null && !upperTierChatModel.isBlank())
                ? upperTierChatModel
                : moeModelName;
        // Prefer the dynamic factory when available
        if (factory != null) {
            try {
                return factory.lc(modelName, 0.3, 1.0, null);
            } catch (Exception ex) {
                log.warn("resolveUpperTier: failed to create model {} via factory", modelName, ex);
            }
        }
        // Fall back to injected MOE model
        if (moe != null) {
            return moe;
        }
        // Fall back to injected base model
        if (utility != null) {
            return utility;
        }
        // If all else fails throw to indicate misconfiguration
        throw new IllegalStateException(
                "No ChatModel beans nor DynamicChatModelFactory present while forceUpperTier=true. " +
