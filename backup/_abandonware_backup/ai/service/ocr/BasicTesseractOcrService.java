...
// auto-patch: gating knobs
private boolean ocrEnabled = true;
private double minConfidence = 0.78;



private java.util.List<?> filterByConfidence(java.util.List<?> spans, double minC) {
  try {
    java.util.List<Object> out = new java.util.ArrayList<>();
    for (Object s : spans) {
      // best-effort: reflect 'getConfidence' if present
      try {
        java.lang.reflect.Method m = s.getClass().getMethod("getConfidence");
        Object v = m.invoke(s);
        double c = (v instanceof Number)? ((Number)v).doubleValue() : 1.0;
        if (c >= minC) out.add(s);
      } catch (Throwable t) {
        out.add(s);
      }
