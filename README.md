...
Netty 도입 Netty는 비동기 이벤트 기반 네트워크 애플리케이션 프레임워크로, 고성능 프로토콜 서버 및 클라이언트를 신속하게 개발하는 데 사용됩니다. Mina Core와 Netty는 동일한 개발자에 의해 만들어졌지만, Netty는 Mina Core의 후속작으로 더 개선된 아키텍처를 제공합니다. Mina Core 사용 시 발생하는 반복적인 패킷 전송 버그를 해결하기 위해 Netty 소켓을 도입하여 안정적인 통신을 구현했습니다.
