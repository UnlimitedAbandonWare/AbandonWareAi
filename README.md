...
* [필수 사전 설치 안내](#필수-사전-설치-안내)
* [빠른 시작 (Build/Run)](#빠른-시작-buildrun)
* [시작 전 cURL 테스트](#시작-전-curl-테스트)
* [데모: 업로드 → 채팅 → 스트리밍](#데모-업로드--채팅--스트리밍)
* [API 개발 문서 / Postman](#api-개발-문서--postman)
* [핵심 기능(요약)](#핵심-기능요약)
* [필수 설정(버전 순도/프로퍼티/시크릿)](#필수-설정버전-순도프로퍼티시크릿)
* [프로필/포트/운영 팁](#프로필포트운영-팁)
* [운영 헬스/메트릭](#운영-헬스메트릭)
* [파일/디렉터리 설명](#파일디렉터리-설명)
* [개발 환경 & 사용 라이브러리](#개발-환경--사용-라이브러리)
* [TODO](#todo)
* [트러블슈팅](#트러블슈팅)
* [보안/비밀/감사](#보안비밀감사)
* [검증 태스크](#검증-태스크)
* [라이선스 / 문서 메타](#라이선스--문서-메타)
* [개발자/문의](#개발자문의)
...
* MUST\_INCLUDE(1\~4 키워드) 자동 추출, 위치정보 제공 시 `### LOCATION CONTEXT` 추가.
* **문자열 concat 금지**, 널 검사/토큰 클리핑 내장.

### 3) 세션 격리 & 메모리

* `rag.safety.enforceSid=true` 로 세션 간 증거 혼용 방지.
* `MemoryReinforcementService`가 장기메모리 요약/주입, TTL 관리.
...
> **주의:** 기존 `gpt-4o`/`gpt-4o-mini` 등 **모두 제거**. `application.yml`에도 동일 키 동기화.
...
* **ONNX 모델 없음** → `abandonware.reranker.backend=embedding-model`로 임시 전환 또는 모델 배치.
