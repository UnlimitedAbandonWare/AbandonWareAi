...
KR: 웹 검색과 벡터 DB를 융합한 하이브리드 RAG 챗봇 서버입니다. SSE 스트리밍, 메모리 강화 루프, 사실 검증 가드를 제공합니다. 모든 프롬프트는 PromptBuilder.build(ctx)로만 생성되며 LangChain4j 1.0.1 순도를 강제합니다.
...
ACCEPTANCE_PROOF
DESKTOP_ONLY=YES
LC4J_PURITY=ONLY 1.0.1 (혼입 없음; 증빙: gradle/libs.versions.toml 또는 build.gradle)
...
위 README는 **{스터프1}**의 내용을 **{스터프2}**의 구성/문맥/톤에 맞춰 재서술했습니다. 바로 README.md로 복사-붙여넣기 하시면 됩니다.
