...
export async function apiCall(url, options = {}) {
  ...
  // Spring Security: _csrf_header 메타가 있으면 그 이름으로 전달
  const headerName = document.querySelector(CSRF_HEADER_META_SELECTOR)?.content || 'X-CSRF-TOKEN';
  if (!opts.headers[headerName]) {
    const meta = document.querySelector(CSRF_META_SELECTOR);
    if (meta) opts.headers[headerName] = meta.content;
  }
