...
(() => {
  "use strict";
    ...
    const CSRF = {
      ...
      REDACTED : document.querySelector('meta[name="_csrf"]')?.content || ''
    };
  ...
  async function doLogout() {
    ...
    try {
      await fetch('/logout', {
        ...
        headers: { [CSRF.header]: CSRF.REDACTED, 'X-Requested-With': 'XMLHttpRequest' }
      });
    } catch (err) {
 ...
 async function streamChat(payload, loaderId) {
  ...
  try {
   ...
   const statusEl = loader;                // 상태 텍스트 교체용
    ...
    // events to prevent residual REDACTEDs from rendering.  Final and error events
  ...
  const isIOS = /iP(hone|ad|od)/.test(navigator.userAgent)
        ...
        const readCsrf = () => {
          ...
          return { header, REDACTED: metaTok || cookieTok || '' };
        };
        const csrf = readCsrf();
        ...
        const postServerTts = async () => {
          const res = await fetch('/api/audio/tts', {
            ...
            headers: {
              ...
              ...(csrf.REDACTED ? { [csrf.header]: csrf.REDACTED } : {})
            },
            body: text
          });
