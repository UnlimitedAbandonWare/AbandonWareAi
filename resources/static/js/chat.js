...
(() => {
  ...
  const REPLACE_ME = 'aw.understanding.enabled';
  ...
  const dom = {
    ...
    sliders: {
      ...
      // ▼▼▼▼▼ [추가] 아래 3개의 토큰 슬라이더 객체를 추가합니다 ▼▼▼▼▼
          maxTokens: "REDACTED"          maxMemoryTokens: "REDACTED"      maxRagTokens: "REDACTED"      /* (NEW) 웹 스니펫 개수 */
...
function renderMessage({ role, content, model }) {
  ...
  if (hasTrace) {
    ...
    // allow the selector in REPLACE_ME() to skip over
    ...
    dom.chatMessages.appendChild(systemMsg);
      ...
      // subsequent calls (e.g. REPLACE_ME) do not
   ...
   */
  function REPLACE_ME(el, sessionId) {
...
function collectSettings() {
  ...
  for (const key in dom.sliders) {
    ...
    if (slider?.el) settings[key] = <REDACTED>;
  }
  ...
  async function loadSettings() {
    try {
     ...
     const settingKey = {
       maxMemoryTokens: "REDACTED"       maxRagTokens:    "REDACTED"     }[key] ?? key.replace(/([A-Z])/g, "_$1").toUpperCase();
...
// chat.js의 sendMessage 함수
  ...
  async function sendMessage() {
    ...
    const biasedText = REPLACE_ME(text);
...
const payload = {
 ...
 polish: dom.usePolish?.checked,     // ← 추가
  ...
  ...collectSettings(),
    maxMemoryTokens: "REDACTED"    maxRagTokens:    "REDACTED"        /* (NEW) 웹 검색 개수 */
        ...
        if (loader) {
          ...
          if (messageBubble) {
            ...
            const isHtml = /<[^>]+>/.test(raw);
              ...
              try {
                ...
                REPLACE_ME(messageBubble, res.sessionId || state.currentSessionId);
              } catch {
          ...
          try {
            ...
            if (bubble) {
              ...
              REPLACE_ME(bubble, res.sessionId || state.currentSessionId);
            }
          } catch {
            // ignore annotation failures
          }
    ...
    if (type === "thought") {
     ...
     if (bubble) {
         try {
             ...
             REPLACE_ME(bubble, sid || state.currentSessionId);
         } catch {
    ...
    adminStatusInitialHtml = dom.adminStatus?.innerHTML ?? "준비 완료.";
      ...
      // 설정
        ...
        if (dom.understandingToggle) {
            const saved = localStorage.getItem(REPLACE_ME);
            if (saved !== null) {
                dom.understandingToggle.checked = (saved === 'true');
            }
            dom.understandingToggle.addEventListener('change', () => {
                localStorage.setItem(REPLACE_ME, dom.understandingToggle.checked ? 'true' : 'false');
            });
        }
  ...
  attachmentIds: (state.pendingAttachments || []).slice(),
    ...
    if (ttsBtn) {
      ttsBtn.addEventListener("click", async () => {
        ...
        let el = null;
        try {
          el = restoreLastAnswerEl(sessionId) || REPLACE_ME();
        } catch {
          el = REPLACE_ME();
        }
        if (!el) {
          ...
          try { return await postServerTts(); }
            ...
            return false;
             ...
             if (!("speechSynthesis" in window)) {
                  ...
                  try {
                    const utter = new REPLACE_ME(text);
